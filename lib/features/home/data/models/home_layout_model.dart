...
abstract final class HomeLayoutModel {
  ...
  static HomeLayout fromJson(Map<String, dynamic> json) {
    final int version = _requiredPositiveInt(json, 'version');

    final String page = _requiredString(json, 'page');

    final String locale = _requiredString(json, 'locale');

    final DateTime updatedAt = _requiredDateTime(json, 'updated_at');

    final List<HomeBlock> blocks = _parseBlocks(json, 'blocks');
  ...
  static List<HomeBlock> _parseBlocks(Map<String, dynamic> json, String key) {
    final dynamic value = json[key];

    if (value is! List) {
      throw FormatException('Missing or invalid home layout list field: $key');
    }
    ...
    for (int index = 0; index < value.length; index++) {
      ...
      final Map<String, dynamic> blockJson = Map<String, dynamic>.from(
        rawBlock,
      );
      final dynamic rawType = blockJson['type'];

      if (rawType is String && HomeBlockType.tryParse(rawType) == null) {
        // Future server block types remain forward-compatible.
        continue;
      }

      try {
        blocks.add(HomeBlockModel.fromJson(blockJson));
