  ...
  Widget build(BuildContext context) {
	...
	for (final String item in items) {
	  ...
	  if (item == 'share' && productAction) {
		...
		footerItems.add(_FooterAction(icon: _footerActionIcon('like', footer.string('like_icon_variant', 'heart'), isLiked), label: footer.boolean('show_labels', true) ? footer.string('like_label', 'Like') : '', color: isLiked ? Colors.red : _cmsColor(footer.string('like_color', '#1F2933'), Colors.black87), size: footer.number('like_icon_size', footer.number('icon_size', 24)), labelSize: footer.number('label_size', 11), iconLabelGap: footer.number('icon_label_gap', 3), onPressed: onLike));
...
class _FooterAction extends StatelessWidget { const _FooterAction({required this.icon,required this.label,required this.color,required this.size,required this.labelSize,required this.iconLabelGap,required this.onPressed}); final IconData icon; final String label; final Color color; final double size; final double labelSize; final double iconLabelGap; final VoidCallback? onPressed; @override Widget build(BuildContext context)=>InkWell(onTap:onPressed,child:Padding(padding:const EdgeInsets.symmetric(horizontal:8),child:Column(mainAxisSize:MainAxisSize.min,children:[Icon(icon,color:color,size:size),if(label.isNotEmpty)...[SizedBox(height:iconLabelGap.clamp(0,12)),Text(label,style:TextStyle(color:color,fontSize:labelSize))]]))); }
