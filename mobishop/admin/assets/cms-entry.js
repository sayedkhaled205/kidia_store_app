(function () {
	...
	const config = window.mobishopCMSEntry || {};
	...
	function pluginPage(url) {
		try {
			...
			return target.origin === window.location.origin &&
				...
				target.searchParams.get('page') === 'mobishop';
		} catch (_error) {
			return false;
		}
	}
	...
	function applyCmsClasses(payload) {
		...
		document.body.classList.add('mobishop-cms-plugin-page', 'mobishop');
		document.body.classList.toggle('mobishop-cms-builder-screen', Boolean(payload.builderScreen));
		document.documentElement.classList.toggle('mobishop-cms-builder-screen', Boolean(payload.builderScreen));
	}

	async function enterCms(url) {
		if (window.mobishopCmsEntryInFlight) return;
		...
		window.mobishopCmsEntryInFlight = true;
		const target = new URL(url, window.location.href);
		target.hash = '';
		try {
			...
			history.pushState({ mobishopCmsPage: true }, '', target.href);
			const loadedNewScript = await loadScripts(payload);
			await stylesReady;
			if (loadedNewScript) document.dispatchEvent(new Event('DOMContentLoaded'));
			document.dispatchEvent(new CustomEvent('mobishop:cms-page-ready', { detail: { url: target.href } }));
		} catch (_error) {
			window.location.href = target.href;
		} finally {
			window.mobishopCmsEntryInFlight = false;
		}
	}

	document.addEventListener('click', function (event) {
		if (document.body.classList.contains('mobishop-cms-plugin-page')) return;
	...
	window.addEventListener('popstate', function () {
		if (document.body.classList.contains('mobishop-cms-plugin-page') && !pluginPage(window.location.href)) {
