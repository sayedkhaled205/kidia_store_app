(function () {
	...
	var builder = document.getElementById("mobishop-home-builder");
	...
	window.mobishopHomeBuilderBooted = true;

	var config = window.mobishopHomeBuilder || {};
	var labels = config.labels || {};
	var picker = document.getElementById("mobishop-element-picker");
	var searchInput = document.getElementById("mobishop-element-picker-search");
	var blocksPayload = document.getElementById("mobishop-home-builder-payload");
	var previewContent = document.getElementById("mobishop-preview-content");
	var phoneScreen = document.querySelector(".mobishop-preview__screen");
	...
	document.addEventListener("click", function (event) {
		if (event.target.closest("#mobishop-add-element")) {
			openPicker();
			return;
		}
		if (event.target.closest("#mobishop-collapse-all")) {
			collapseAll(true);
			return;
		}
		if (event.target.closest("#mobishop-expand-all")) {
			collapseAll(false);
			return;
		}
	});
	...
	function makeActionValueControl(typeField) {
		...
		var field = current ? current.closest(".mobishop-builder-field") : null;
	...
	function getBlocks() {
		return toArray(builder.querySelectorAll(".mobishop-builder-block"));
	}
	...
	function markDirty() {
		form.dispatchEvent(new window.CustomEvent("mobishop:dirty", { bubbles: true }));
	}
	...
	function updateIndexes() {
		getBlocks().forEach(function (block, index) {
			...
			var orderInput = block.querySelector(".mobishop-block-order");
			if (orderInput) {
	...
	function serializeBlocks() {
		return getBlocks().map(function (block, index) {
			...
			return {
				id: value(".mobishop-block-id"),
				library_id: value(".mobishop-block-library-id"),
				source_library_id: value(".mobishop-block-source-library-id"),
				create_intent: value(".mobishop-block-create-intent"),
				type: value(".mobishop-block-type"),
				name: value(".mobishop-block-name-input"),
				enabled: enabled && enabled.checked ? "1" : "",
				status: value(".mobishop-block-status"),
				order: index + 1,
				settings: settings
			};
		});
	...
	function blockHeading(settings, fallback, runtimeData) {
		...
		return '<div class="mobishop-preview-section-heading"><div><strong>' + title + "</strong>" + (subtitle ? "<small>" + subtitle + "</small>" : "") + "</div>" + (showViewAll ? "<span>" + viewAllLabel + " ‹</span>" : "") + "</div>";
	}
	...
	function sampleCards(count, className, label) {
		...
		for (index = 0; index < count; index += 1) {
			cards.push('<div class="' + className + '"><span class="mobishop-preview-sample-image"></span>' + (label ? "<strong>" + escapeHtml(label) + "</strong>" : "") + "</div>");
		}
		return cards.join("");
	}
	...
	function renderCategoryCard(item, showName, settings) {
		...
		return '<article class="mobishop-preview-category-card"><span class="mobishop-preview-category-card__image" style="width:' + numberInRange(settings.image_size, 78, 32, 160) + 'px;height:' + numberInRange(settings.image_size, 78, 32, 160) + 'px;border-radius:' + radius + '">' + (image ? '<img src="' + image + '" alt="' + name + '">' : '<span class="mobishop-preview-image-fallback"></span>') + "</span>" + (showName ? '<strong style="color:' + safeColor(settings.label_color, "#1F2933") + ';font-size:' + numberInRange(settings.label_size, 13, 10, 22) + 'px">' + name + "</strong>" : "") + "</article>";
	}
	...
	function renderProductCard(item, settings) {
		...
		var quickShell = numberInRange(settings && settings.quick_add_background_size, 40, 10, 64);
		...
		return '<article class="mobishop-preview-product-card is-' + escapeHtml(style) + '" style="border-radius:' + numberInRange(settings && settings.card_radius, 20, 0, 40) + 'px"><div class="mobishop-preview-product-card__image" style="aspect-ratio:' + numberInRange(settings && settings.image_ratio, 1, 0.6, 1.8) + '">' + (image ? '<img src="' + image + '" alt="' + name + '">' : '<span class="mobishop-preview-image-fallback"></span>') + (status && (!settings || settings.show_badge !== "") ? '<span class="mobishop-preview-product-card__badge' + (!inStock ? " is-out-of-stock" : "") + '">' + status + "</span>" : "") + (settings && settings.show_wishlist !== "" ? '<span class="mobishop-preview-product-card__wishlist is-' + escapeHtml(settings.product_wishlist_icon_style || "outline") + '" ' + wishlistStyle + '>' + wishlistIcon + '</span>' : "") + (inStock && (!settings || settings.quick_add_enabled !== "") ? '<span class="mobishop-preview-product-card__quick-add is-' + escapeHtml(settings && settings.quick_add_icon_style || "outline") + '" ' + quickStyle + '>' + quickIcon + '</span>' : "") + '</div><div class="mobishop-preview-product-card__body">' + (!settings || settings.show_name !== "" ? "<strong>" + name + "</strong>" : "") + (settings && settings.show_rating !== "" && rating > 0 ? '<span class="mobishop-preview-product-card__rating">★ ' + rating.toFixed(1) + "</span>" : "") + (!settings || settings.show_price !== "" ? '<div class="mobishop-preview-product-card__prices"><b>' + price + (currency ? " " + currency : "") + "</b>" + (discounted && (!settings || settings.show_regular_price !== "") ? "<del>" + regularPrice + (currency ? " " + currency : "") + "</del>" : "") + "</div>" : "") + "</div></article>";
	}
	...
	function renderBrandCard(item, itemWidth, settings) {
		...
		return '<article class="mobishop-preview-brand-card" style="--mobishop-preview-brand-width:' + width + 'px"><span class="mobishop-preview-brand-card__image" style="border-radius:' + radius + '">' + (image ? '<img src="' + image + '" alt="' + name + '">' : '<span class="mobishop-preview-image-fallback"></span>') + "</span>" + (settings.show_names === "" ? "" : "<strong>" + name + "</strong>") + "</article>";
	}
	...
	function renderQuickLink(item, settings) {
		...
		return '<article class="mobishop-preview-quick-link"><span style="width:' + numberInRange(settings.item_size, 78, 32, 160) + 'px;height:' + numberInRange(settings.item_size, 78, 32, 160) + 'px;border-radius:' + shape + '">' + (image ? '<img src="' + image + '" alt="' + label + '">' : '<span class="mobishop-preview-image-fallback"></span>') + "</span>" + (settings.show_labels === "" ? "" : '<strong style="color:' + safeColor(settings.label_color, "#1F2933") + ';font-size:' + numberInRange(settings.label_size, 13, 10, 22) + 'px">' + label + "</strong>" + (subtitle ? "<small>" + subtitle + "</small>" : "")) + "</article>";
	}
	...
	function renderBannerTile(item, settings) {
		...
		return '<article class="mobishop-preview-banner-tile" style="aspect-ratio:' + numberInRange(settings.aspect_ratio, 1, 0.45, 5) + ";border-radius:" + numberInRange(settings.border_radius, 16, 0, 48) + 'px">' + (image ? '<img src="' + image + '" alt="" style="object-fit:' + (settings.image_fit === "contain" ? "contain" : "cover") + '">' : '<span class="mobishop-preview-image-fallback"></span>') + ((title || subtitle || button) ? '<div style="background:linear-gradient(0deg,rgba(0,0,0,' + overlay + '),transparent);color:' + safeColor(settings.text_color, "#FFFFFF") + '"><strong>' + title + "</strong><small>" + subtitle + "</small>" + (button ? "<b>" + button + "</b>" : "") + "</div>" : "") + "</article>";
	}
	...
	function previewIcon(type) {
		...
		return '<span class="mobishop-preview-header__icon" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false">' + drawing + "</svg></span>";
	}

	function renderAppHeader(settings, name) {
		var image = safeImage(settings.logo_url);
		var title = escapeHtml(settings.title || name || "MobiShop Store");
		...
		if (settings.show_account) {
			icons += '<span class="mobishop-preview-header__account is-' + escapeHtml(settings.account_style || "icon") + '" style="--account-icon-size:' + numberInRange(settings.account_icon_size, 24, 16, 40) + 'px">' + styledIcon("account") + (settings.show_account_label !== "" ? '<small>' + escapeHtml(settings.account_label || "Account") + '</small>' : "") + '</span>';
		}
		...
		var search = settings.show_search && settings.search_style === "bar" ? '<span class="mobishop-preview-header__search" style="height:' + numberInRange(settings.search_height, 40, 32, 64) + 'px;border:' + numberInRange(settings.search_border_width, 0, 0, 6) + 'px solid ' + safeColor(settings.search_border_color, "#DDE3E8") + ';border-radius:' + numberInRange(settings.search_radius, 14, 0, 32) + 'px;background:' + safeColor(settings.search_background, "#F1F3F4") + ';color:' + safeColor(settings.search_text_color, "#5F6368") + '">' + styledIcon("search", safeColor(settings.search_icon_color, "#5F6368")) + escapeHtml(settings.search_placeholder || "Search products") + (settings.show_voice_search ? " ♪" : "") + '</span>' : "";
		return '<div class="mobishop-preview-header' + (settings.search_style === "bar" ? " has-search-bar" : "") + '" style="min-height:' + numberInRange(settings.height, 64, 48, 120) + 'px;padding-inline:' + numberInRange(settings.horizontal_padding, 12, 0, 32) + 'px;border-radius:' + numberInRange(settings.border_radius, 0, 0, 40) + 'px;color:' + safeColor(settings.title_color, "#1F2933") + ';background:' + safeColor(settings.background_color, "#FFFFFF") + ';box-shadow:' + shadow + '"><span class="mobishop-preview-header__identity">' + identity + '</span><span class="mobishop-preview-header__icons" style="gap:' + numberInRange(settings.icon_gap, 4, 0, 24) + 'px">' + icons + '</span>' + search + '</div>';
	}
	...
	function renderBlock(block) {
		...
		case "hero_slider":
			...
			return '<div class="mobishop-preview-block mobishop-preview-hero is-' + escapeHtml(settings.overlay_position || "start") + (settings.indicator_position === "below" ? " has-indicators-below" : "") + '" style="aspect-ratio:' + ratio + ";margin-inline:" + numberInRange(settings.horizontal_padding, 16, 0, 32) + "px;border-radius:" + numberInRange(settings.border_radius, 24, 0, 48) + 'px">' + (image ? '<img src="' + image + '" alt="" style="object-fit:' + (settings.image_fit === "contain" ? "contain" : "cover") + '">' : '<div class="mobishop-preview-placeholder">' + name + "</div>") + ((item.title || item.subtitle || item.button_label) ? '<div class="mobishop-preview-hero__copy" style="background:rgba(17,24,39,' + numberInRange(settings.overlay_strength, 72, 0, 95) / 100 + ");color:" + safeColor(settings.text_color, "#FFFFFF") + '"><strong>' + escapeHtml(item.title || "") + "</strong><small>" + escapeHtml(item.subtitle || "") + "</small>" + (item.button_label ? "<b>" + escapeHtml(item.button_label) + "</b>" : "") + "</div>" : "") + (items.length > 1 && settings.show_indicators !== "" ? '<div class="mobishop-preview-dots is-' + escapeHtml(settings.indicator_style || "pill") + '">' + items.map(function (_, index) { return "<i" + (index === 0 ? ' class="is-active"' : "") + "></i>"; }).join("") + "</div>" : "") + "</div>";
		...
		case "category_grid":
			...
			return '<section class="mobishop-preview-section">' + blockHeading(settings, name, runtimeData) + '<div class="mobishop-preview-category-grid is-' + escapeHtml(settings.layout || "grid") + ' is-align-' + escapeHtml(settings.items_alignment || "right") + (settings.layout === "carousel" ? " is-carousel" : "") + '" style="--mobishop-preview-columns:' + columns + ";--mobishop-preview-gap:" + categoryGap + "px;column-gap:" + categoryGap + "px;row-gap:" + categoryRowGap + 'px">' + (items.length ? items.map(function (category) { return renderCategoryCard(category, settings.show_names !== "", settings); }).join("") : sampleCards(count, "mobishop-preview-category-card", settings.show_names === "" ? "" : "Category")) + "</div></section>";

		case "image_banner":
			image = safeImage(settings.image_url);
			ratio = numberInRange(settings.aspect_ratio, 2.4, 0.45, 5);
			return '<div class="mobishop-preview-block mobishop-preview-banner" style="aspect-ratio:' + ratio + ";border-radius:" + numberInRange(settings.border_radius, 20, 0, 60) + 'px">' + (image ? '<img src="' + image + '" alt="' + escapeHtml(settings.semantic_label || "") + '" style="object-fit:' + (settings.image_fit === "contain" ? "contain" : "cover") + '">' : '<div class="mobishop-preview-placeholder">' + name + "</div>") + ((settings.title || settings.subtitle || settings.button_label) ? '<div class="mobishop-preview-banner__overlay" style="background:linear-gradient(0deg,rgba(0,0,0,' + numberInRange(settings.overlay_strength, 0, 0, 95) / 100 + '),transparent);color:' + safeColor(settings.text_color, "#FFFFFF") + '"><strong>' + escapeHtml(settings.title || "") + "</strong><small>" + escapeHtml(settings.subtitle || "") + "</small>" + (settings.button_label ? "<b>" + escapeHtml(settings.button_label) + "</b>" : "") + "</div>" : "") + "</div>";

		case "product_carousel":
			count = Math.round(numberInRange(settings.limit, 10, 1, 12));
			items = limitedItems(runtimeData.items, count, 12);
			return '<section class="mobishop-preview-section mobishop-preview-section--products">' + blockHeading(settings, name, runtimeData) + '<div class="mobishop-preview-product-row" style="--mobishop-preview-product-width:' + numberInRange(settings.item_width, 168, 110, 260) * 0.7 + 'px">' + (items.length ? items.map(function (product) { return renderProductCard(product, settings); }).join("") : sampleCards(Math.min(count, 3), "mobishop-preview-product-card", "Product")) + "</div></section>";
		...
		case "product_grid":
			...
			return '<section class="mobishop-preview-section mobishop-preview-section--products">' + blockHeading(settings, name, runtimeData) + '<div class="mobishop-preview-product-grid" style="--mobishop-preview-columns:' + columns + '">' + (items.length ? items.map(function (product) { return renderProductCard(product, settings); }).join("") : sampleCards(count, "mobishop-preview-product-card", "Product")) + "</div></section>";

		case "bundle_collection":
			count = Math.round(numberInRange(settings.limit, 6, 1, 12));
			items = limitedItems(runtimeData.items, count, 12);
			return '<section class="mobishop-preview-section mobishop-preview-section--bundles">' + blockHeading(settings, name, runtimeData) + '<div class="mobishop-preview-bundle-row is-' + escapeHtml(settings.layout || "carousel") + '">' + (items.length ? items.map(function (bundle) {
				return '<article class="mobishop-preview-bundle-card" style="border-radius:' + numberInRange(settings.card_radius, 16, 0, 40) + 'px"><span class="dashicons dashicons-products"></span><strong>' + escapeHtml(bundle.name || "Bundle") + '</strong><small>' + escapeHtml(bundle.description || "") + '</small>' + (settings.show_discount === "" || !bundle.discount_value ? "" : '<b>' + escapeHtml(String(bundle.discount_value)) + '% off</b>') + '<em>' + escapeHtml(bundle.cta_label || "Customize bundle") + '</em></article>';
			}).join("") : sampleCards(Math.min(count, 3), "mobishop-preview-bundle-card", "Bundle")) + "</div></section>";

		case "section_header":
			return '<div class="mobishop-preview-section-heading mobishop-preview-section-heading--standalone"><div><strong>' + escapeHtml(settings.title || name) + "</strong>" + (settings.subtitle ? "<small>" + escapeHtml(settings.subtitle) + "</small>" : "") + "</div>" + (settings.show_view_all === "" ? "" : "<span>" + escapeHtml(settings.view_all_label || "View all") + " ‹</span>") + "</div>";

		case "brand_carousel":
			count = Math.round(numberInRange(settings.limit, 12, 1, 5));
			items = limitedItems(runtimeData.items, count, 5);
			return '<section class="mobishop-preview-section">' + blockHeading(settings, name, runtimeData) + '<div class="mobishop-preview-brand-row' + (settings.layout === "grid" ? " is-grid" : "") + '" style="--mobishop-preview-columns:' + numberInRange(settings.columns, 4, 2, 6) + ";gap:" + numberInRange(settings.gap, 12, 0, 32) + 'px">' + (items.length ? items.map(function (brand) { return renderBrandCard(brand, runtimeData.item_width || settings.item_width, settings); }).join("") : sampleCards(count, "mobishop-preview-brand-card", "Brand")) + "</div></section>";

		case "quick_links":
			items = Array.isArray(settings.items) ? settings.items : [];
			columns = Math.round(numberInRange(settings.columns, 4, 2, 6));
			return '<section class="mobishop-preview-section">' + blockHeading(settings, name, runtimeData) + '<div class="mobishop-preview-quick-links ' + (settings.layout === "grid" ? "is-grid" : "is-row") + '" style="--mobishop-preview-columns:' + columns + ";gap:" + numberInRange(settings.gap, 12, 0, 32) + 'px">' + (items.length ? items.map(function (link) { return renderQuickLink(link, settings); }).join("") : sampleCards(columns, "mobishop-preview-quick-link", "Link")) + "</div></section>";
		...
		case "banner_grid":
			...
			return '<section class="mobishop-preview-section">' + blockHeading(settings, name, runtimeData) + '<div class="mobishop-preview-banner-grid is-' + escapeHtml(settings.layout || "equal") + '" style="--mobishop-preview-columns:' + columns + ";gap:" + numberInRange(settings.gap, 10, 0, 32) + 'px">' + (items.length ? items.map(function (banner) { return renderBannerTile(banner, settings); }).join("") : sampleCards(columns + 1, "mobishop-preview-banner-tile", "")) + "</div></section>";
		...
		case "promo_strip":
			...
			return '<div class="mobishop-preview-promo is-' + escapeHtml(settings.transition_effect || "fade") + '" style="width:' + promoWidth + '%;min-height:' + promoHeight + ';background:' + background + ";color:" + textColor + '"><span>✦</span><strong>' + escapeHtml(promoText) + "</strong></div>";

		case "coupon_banner":
			image = safeImage(settings.image_url);
			return '<div class="mobishop-preview-coupon' + (image ? " has-image" : "") + '" style="background:' + safeColor(settings.background_color, "#DCEEE8") + ";color:" + safeColor(settings.text_color, "#1F2933") + ";border-radius:" + numberInRange(settings.border_radius, 20, 0, 48) + 'px">' + (image ? '<img src="' + image + '" alt="">' : "") + '<div class="mobishop-preview-coupon__overlay"><strong>' + escapeHtml(settings.title || name) + "</strong><small>" + escapeHtml(settings.description || "") + "</small>" + (settings.coupon_code ? '<code style="color:' + safeColor(settings.accent_color, "#2F806E") + '">▣ ' + escapeHtml(settings.coupon_code) + "</code>" : "") + "</div></div>";
		...
		case "countdown":
			...
			if (remainingSeconds === 0 && settings.ends_at) {
				return '<div class="mobishop-preview-countdown is-expired"><strong>' + escapeHtml(settings.expired_text || "Offer ended") + '</strong></div>';
			}
			var countdownDays = Math.floor(remainingSeconds / 86400);
			var visibleUnits = ["days", "days_hours", "days_hours_minutes", "days_hours_minutes_seconds"].indexOf(settings.visible_units) >= 0
				? settings.visible_units
				: settings.show_seconds !== ""
					? "days_hours_minutes_seconds"
					: settings.show_minutes !== ""
						? "days_hours_minutes"
						: settings.show_hours !== ""
							? "days_hours"
							: "days";
			var showCountdownDays = visibleUnits.indexOf("days") === 0;
			var showCountdownHours = visibleUnits.indexOf("hours") >= 0;
			var showCountdownMinutes = visibleUnits.indexOf("minutes") >= 0;
			var showCountdownSeconds = visibleUnits.indexOf("seconds") >= 0;
			var countdownHours = showCountdownDays
			...
			var countdownUnits = [];
			if (showCountdownDays) { countdownUnits.push('<span>' + countdownDays + '<small>يوم</small></span>'); }
			if (showCountdownHours) { countdownUnits.push('<span>' + countdownHours + '<small>ساعة</small></span>'); }
			if (showCountdownMinutes) { countdownUnits.push('<span>' + countdownMinutes + '<small>دقيقة</small></span>'); }
			if (showCountdownSeconds) { countdownUnits.push('<span>' + countdownSeconds + '<small>ثانية</small></span>'); }
			return '<div class="mobishop-preview-countdown is-' + escapeHtml(settings.layout_style || "cards") + '" style="background:' + safeColor(settings.background_color, "#FFFFFF") + ";color:" + safeColor(settings.text_color, "#1F2933") + '"><strong>' + escapeHtml(settings.title || name) + '</strong><div style="--mobishop-countdown-box:' + safeColor(settings.box_color, "#E9EEEC") + '">' + countdownUnits.join('') + '</div></div>';

		case "video_banner":
			image = safeImage(settings.poster_url);
			ratio = numberInRange(settings.aspect_ratio, 1.8, 0.45, 4);
			var videoUrl = safeImage(settings.video_url);
			return '<div class="mobishop-preview-block mobishop-preview-video" style="aspect-ratio:' + ratio + '">' + (videoUrl ? '<video src="' + videoUrl + '" poster="' + image + '" ' + (settings.auto_play !== "" ? "autoplay " : "") + (settings.muted !== "" ? "muted " : "") + (settings.loop !== "" ? "loop " : "") + 'playsinline></video>' : (image ? '<img src="' + image + '" alt="">' : '<div class="mobishop-preview-placeholder">' + name + "</div>")) + (settings.auto_play !== "" ? "" : '<span class="mobishop-preview-video__play">▶</span>') + '</div>';

		case "text_block":
			background = safeColor(settings.background, "#ffffff");
			textColor = safeColor(settings.text_color, "#111111");
			return '<div class="mobishop-preview-text" style="background:' + background + ";color:" + textColor + ";text-align:" + (settings.alignment === "left" || settings.alignment === "center" ? settings.alignment : "right") + ";font-weight:" + (settings.font_weight === "bold" ? "700" : settings.font_weight === "medium" ? "500" : "400") + '"><strong style="font-size:' + numberInRange(settings.title_size, 22, 12, 48) + 'px">' + escapeHtml(settings.title || name) + '</strong><p style="font-size:' + numberInRange(settings.content_size, 15, 10, 32) + 'px">' + escapeHtml(settings.content || "") + "</p></div>";

		case "divider":
			return '<div class="mobishop-preview-divider" style="margin-block:' + numberInRange(settings.margin, 16, 0, 64) + "px;border-top:" + numberInRange(settings.thickness, 1, 1, 12) + "px solid " + safeColor(settings.color, "#e5e7eb") + '"></div>';

		case "spacer":
			return '<div class="mobishop-preview-spacer" style="height:' + numberInRange(settings.height, 24, 0, 160) + 'px"><span>' + numberInRange(settings.height, 24, 0, 160) + "px</span></div>";

		default:
			return '<div class="mobishop-preview-block"><div class="mobishop-preview-block__label">' + name + '</div><div class="mobishop-preview-placeholder">' + escapeHtml(replaceEvery(block.type, "_", " ")) + "</div></div>";
		}
	}
	...
	function renderBlockFrame(block) {
		...
		return '<div class="mobishop-preview-focus-target mobishop-preview-block-frame" data-preview-block="' +
			escapeHtml(block.id) +
			...
			'px;background:' + background + ';--mobishop-preview-content-scale:' + scale + '">' +
			'<div class="mobishop-preview-block-frame__content">' + renderBlock(block) + '</div></div>';
	}
	...
	function renderPreviewNow() {
		...
		window.mobishopHomePreviewBlocks = blocks;
		document.dispatchEvent(new CustomEvent("mobishop:home-preview-state", { detail: { blocks: blocks } }));

		header = renderFixedChrome("header");
		footer = renderFixedChrome("footer");
		previewContent.innerHTML = header + '<div class="mobishop-preview-home-body">' + (blocks.length ? blocks.map(renderBlockFrame).join("") : '<div class="mobishop-preview-empty">Add or enable an element to preview the Home Page.</div>') + '</div>' + footer;
		if(activePreviewBlock){var active=activePreviewBlock==="header"?previewContent.querySelector(".mobishop-app-header"):activePreviewBlock==="footer"?previewContent.querySelector(".mobishop-app-footer"):previewContent.querySelector('[data-preview-block="'+activePreviewBlock.replace(/"/g,"\\\"")+'"]');if(active){active.classList.add("is-editor-focused");}}
	}
	...
	function focusPreviewBlock(block) {
		...
		var id=(block.querySelector(".mobishop-block-id")||{}).value||block.dataset.libraryId||"";
		activePreviewBlock=block.dataset.chromePart||id;var target=block.dataset.chromePart?previewContent.querySelector(block.dataset.chromePart==="header"?".mobishop-app-header":".mobishop-app-footer"):previewContent.querySelector('[data-preview-block="'+id.replace(/"/g,"\\\"")+'"]');
		document.dispatchEvent(new CustomEvent("mobishop:home-preview-focus", {
	...
	function renderFixedChrome(part) {
		...
		if (window.MobiShopChromePreview) { return part === "header" ? window.MobiShopChromePreview.renderHeader(card, "MobiShop", { collapseProgress: previewCollapseProgress, page: "home" }) : window.MobiShopChromePreview.renderFooter(card, { page: "home" }); }
		...
		if (part === "header") {
			...
			return '<header class="mobishop-app-header" style="height:' + Number(chromeValue(card, "height", 64)) + 'px;background:' + chromeValue(card, "background_color", "#FFFFFF") + ';color:' + chromeValue(card, "title_color", "#1F2933") + '"><span class="mobishop-app-header__leading"></span><div class="mobishop-app-header__title">' + (searchBar ? '<div class="mobishop-app-search" style="height:' + Number(chromeValue(card, "search_height", 40)) + 'px;border-radius:' + Number(chromeValue(card, "search_radius", 14)) + 'px;background:' + chromeValue(card, "search_background", "#F1F3F4") + '"><span>⌕ ' + escapeHtml(chromeValue(card, "search_placeholder", "Search products")) + '</span></div>' : '<strong>' + escapeHtml(chromeValue(card, "title", "MobiShop")) + '</strong>') + '</div><div class="mobishop-app-header__actions">' + (chromeChecked(card, "show_search", true) && !searchBar ? '<span class="mobishop-app-icon mobishop-app-icon--search"></span>' : '') + (chromeChecked(card, "show_cart", true) ? '<span class="mobishop-app-icon mobishop-app-icon--bag"></span>' : '') + '</div></header>';
		}
		return '<footer class="mobishop-app-footer" style="height:' + Number(chromeValue(card, "height", 72)) + 'px;background:' + chromeValue(card, "background_color", "#FFFFFF") + ';color:' + chromeValue(card, "inactive_color", "#6B7280") + '"><span class="is-active" style="color:' + chromeValue(card, "active_color", "#1F6F61") + '"><span class="mobishop-app-icon mobishop-app-icon--home"></span><b>Home</b></span><span><span class="mobishop-app-icon mobishop-app-icon--categories"></span><b>Categories</b></span><span><span class="mobishop-app-icon mobishop-app-icon--heart"></span><b>Wishlist</b></span><span><span class="mobishop-app-icon mobishop-app-icon--person"></span><b>Account</b></span></footer>';
	}
	...
	function loadRuntimePreview() {
		...
		if (document.getElementById("mobishop-flutter-preview")) {
			return;
		}
		...
		window.fetch(String(config.previewEndpoint), {
			...
			if (window.console && window.console.warn) {
				window.console.warn("MobiShop Home preview kept its local fallback because runtime data could not be loaded.", error);
			}
		});
	}
	...
	function requestLiveRuntimePreview() {
		if (document.getElementById("mobishop-flutter-preview")) { return; }
	...
	function setCollapsed(block, collapsed) {
		...
		toggle = block.querySelector(".mobishop-toggle-block-settings");
		if (toggle) {
	...
	function openPicker() {
		...
		document.body.classList.add("mobishop-picker-open");
	...
	function closePicker() {
		...
		document.body.classList.remove("mobishop-picker-open");
	}

	function removeEmptyState() {
		var empty = document.getElementById("mobishop-builder-empty");
		if (empty) {
			empty.remove();
		}
	}
	...
	function ensureEmptyState() {
		...
		if (getBlocks().length || document.getElementById("mobishop-builder-empty")) {
			return;
		}

		empty = document.createElement("div");
		empty.id = "mobishop-builder-empty";
		empty.className = "mobishop-builder-empty";
		empty.innerHTML = '<span class="dashicons dashicons-screenoptions"></span><h2></h2><p></p><button type="button" class="button button-primary" data-mobishop-open-picker></button>';
	...
	function appendTemplate(templateId, type, libraryId, name, isNew) {
		...
		removeEmptyState();
		if (activeInsertionBlock && activeInsertionBlock.isConnected) {
			activeInsertionBlock.insertAdjacentHTML("afterend", html);
			block = activeInsertionBlock.nextElementSibling;
		} else {
		...
		if (block) {
			activeInsertionBlock = block;
			nameInput = block.querySelector(".mobishop-block-name-input");
			nameLabel = block.querySelector(".mobishop-block-name");
			createIntentInput = block.querySelector(".mobishop-block-create-intent");
	...
	function setDraftStatus(block) {
		var statusInput = block.querySelector(".mobishop-block-status");
		var statusSelect = block.querySelector(".mobishop-block-status-select");
		var statusBadge = block.querySelector(".mobishop-builder-status");
		...
		if (statusBadge) {
			statusBadge.classList.remove("mobishop-builder-status--published");
			statusBadge.classList.add("mobishop-builder-status--draft");
	...
	function duplicateBlock(block) {
		...
		var typeField = clone.querySelector(".mobishop-block-type");
		var type = typeField ? typeField.value : "block";
		var newId = generateId(type);
		var idInput = clone.querySelector(".mobishop-block-id");
		var libraryInput = clone.querySelector(".mobishop-block-library-id");
		var sourceLibraryInput = clone.querySelector(".mobishop-block-source-library-id");
		var createIntentInput = clone.querySelector(".mobishop-block-create-intent");
		var editButton = clone.querySelector(".mobishop-edit-library-item");
		var nameInput = clone.querySelector(".mobishop-block-name-input");
		var nameLabel = clone.querySelector(".mobishop-block-name");
	...
	function reindexHeroItems(block) {
		var items = block.querySelectorAll(".mobishop-hero-block-item");
	...
	function reindexRepeatableItems(block) {
		var items = block.querySelectorAll(".mobishop-repeatable-item");
		toArray(items).forEach(function (item, itemIndex) {
			toArray(item.querySelectorAll('[name*="[settings][items]"]')).forEach(function (input) {
				input.name = input.name.replace(/(\[settings\]\[items\])\[[^\]]*\]/, "$1[" + itemIndex + "]");
			});
	...
	function addRepeatableItem(block) {
		var container = block.querySelector(".mobishop-repeatable-items");
		var template = block.querySelector(".tmpl-mobishop-repeatable-item");
		var itemIndex;
		var html;
		if (!container || !template) { return; }
		itemIndex = container.querySelectorAll(".mobishop-repeatable-item").length;
	...
	function addHeroSlide(block) {
		var container = block.querySelector(".mobishop-hero-block-items");
		var template = block.querySelector(".tmpl-mobishop-hero-block-item");
		var itemIndex;
		var html;
		...
		itemIndex = container.querySelectorAll(".mobishop-hero-block-item").length;
		html = replaceEvery(template.innerHTML, "__ITEM_INDEX__", String(itemIndex));
		container.insertAdjacentHTML("beforeend", html);
		reindexHeroItems(block);
		markDirty();
	...
	document.addEventListener("click", function (event) {
		...
		if (target.closest("[data-mobishop-open-picker]")) {
			openPicker();
			return;
		}
		if (target.closest("[data-mobishop-close-picker]")) {
			closePicker();
			return;
		}
		createButton = target.closest(".mobishop-create-element, .mobishop-element-group[data-block-type]");
		if (createButton) {
			var type = createButton.dataset.blockType || "";
			var label = createButton.dataset.blockLabel || "";
			if (type) {
				appendTemplate("tmpl-mobishop-block-" + type, type, generateId(type), label, true);
			}
			return;
		}

		libraryButton = target.closest(".mobishop-add-library-element");
		if (libraryButton) {
	...
	builder.addEventListener("click", function (event) {
		...
		block = target.closest(".mobishop-builder-block");
		if (!block) {
			return;
		}

		if (target.closest(".mobishop-add-promo-message")) {
			var messages = block.querySelector(".mobishop-promo-messages");
			...
			if (messages && blockMatch) {
				...
				row.className = "mobishop-promo-message-row";
				var input = document.createElement("input");
				input.type = "text";
				input.name = "blocks[" + blockMatch[1] + "][settings][messages][" + nextIndex + "]";
				var remove = document.createElement("button");
				remove.type = "button";
				remove.className = "button mobishop-remove-promo-message";
		...
		if (target.closest(".mobishop-remove-promo-message")) {
			var promoRow = target.closest(".mobishop-promo-message-row");
			if (promoRow) { promoRow.remove(); markDirty(); renderPreview(); requestLiveRuntimePreview(); }
			return;
		}

		if (target.closest(".mobishop-toggle-block-settings")) {
		...
		if (target.closest(".mobishop-delete-block")) {
			name = block.querySelector(".mobishop-block-name");
		...
		if (target.closest(".mobishop-duplicate-block")) {
			duplicateBlock(block);
			return;
		}

		if (target.closest(".mobishop-add-hero-block-item")) {
			addHeroSlide(block);
			return;
		}

		if (target.closest(".mobishop-add-repeatable-item")) {
			addRepeatableItem(block);
			return;
		}

		if (target.closest(".mobishop-remove-repeatable-item")) {
			item = target.closest(".mobishop-repeatable-item");
			if (item) {
				item.remove();
				reindexRepeatableItems(block);
				markDirty();
		...
		if (target.closest(".mobishop-remove-hero-block-item")) {
			item = target.closest(".mobishop-hero-block-item");
			if (item) {
				item.remove();
				reindexHeroItems(block);
				markDirty();
		...
		if (target.closest(".mobishop-select-hero-block-image, .mobishop-hero-block-image-preview")) {
			item = target.closest(".mobishop-hero-block-item");
			openMediaPicker({ title: "Choose slide image", button: { text: "Use image" }, multiple: false }, function (frame) {
				...
				var input = item ? item.querySelector(".mobishop-hero-block-image-url") : null;
				var preview = item ? item.querySelector(".mobishop-hero-block-image-preview") : null;
		...
		action = target.closest(".mobishop-select-banner-image, .mobishop-select-media, .mobishop-select-app-header-logo, .mobishop-banner-image-preview, .mobishop-media-preview");
		if (action) {
			openMediaPicker({ title: "Choose image", button: { text: "Use image" }, multiple: false }, function (frame) {
				var selection = frame.state().get("selection").first();
				var attachment = selection ? selection.toJSON() : null;
				var field = action.closest(".mobishop-builder-field");
				var repeatableItem = action.closest(".mobishop-banner-editor-item");
				var input = field ? field.querySelector(".mobishop-banner-image-url, .mobishop-media-url, .mobishop-app-header-logo-url") : null;
				var preview = field ? field.querySelector(".mobishop-banner-image-preview, .mobishop-media-preview") : null;
				input = input || (repeatableItem ? repeatableItem.querySelector(".mobishop-media-url") : null);
				preview = preview || (repeatableItem ? repeatableItem.querySelector(".mobishop-media-preview") : null);
				if (!attachment || !attachment.url) {
					return;
				}
				if (input) {
	...
	function applyElementPickerFilters() {
		var activeFilter = picker ? picker.querySelector(".mobishop-element-category-filter [data-mobishop-element-category].is-active") : null;
		var selectedCategory = activeFilter ? activeFilter.dataset.mobishopElementCategory || "all" : "all";
		var searchValue = searchInput ? searchInput.value.trim().toLocaleLowerCase() : "";
		var visibleGroups = 0;
		var noResults;

		toArray(picker ? picker.querySelectorAll(".mobishop-element-group") : []).forEach(function (group) {
		...
		noResults = document.getElementById("mobishop-element-picker-no-results");
		if (noResults) {
	...
	if (picker) {
		picker.addEventListener("click", function (event) {
			var filterButton = event.target.closest(".mobishop-element-category-filter [data-mobishop-element-category]");
			if (!filterButton || !picker.contains(filterButton)) {
				return;
			}
			toArray(picker.querySelectorAll(".mobishop-element-category-filter [data-mobishop-element-category]")).forEach(function (button) {
	...
	builder.addEventListener("change", function (event) {
		...
		if (target.classList.contains("mobishop-block-status-select")) {
			block = target.closest(".mobishop-builder-block");
			hidden = block ? block.querySelector(".mobishop-block-status") : null;
			badge = block ? block.querySelector(".mobishop-builder-status") : null;
			if (hidden) {
				hidden.value = target.value;
			}
			if (badge) {
				badge.textContent = target.value === "published" ? labels.published || "Published" : labels.draft || "Draft";
				badge.className = "mobishop-builder-status mobishop-builder-status--" + target.value;
			}
		}
		...
		if (target.classList.contains("mobishop-promo-transition-toggle")) {
			var promoBlock = target.closest(".mobishop-builder-block");
			var transitionFields = promoBlock && promoBlock.querySelector(".mobishop-promo-transition-fields");
			if (transitionFields) { transitionFields.hidden = !target.checked; }
		}
		if (target.classList.contains("mobishop-countdown-layout")) {
			var layoutPreview = target.closest(".mobishop-builder-field").querySelector(".mobishop-countdown-layout-preview");
			if (layoutPreview) { layoutPreview.className = "mobishop-countdown-layout-preview is-" + target.value; }
		}
		if (target.classList.contains("mobishop-countdown-visible-units")) {
			var countdownBlock = target.closest(".mobishop-builder-block");
			var countdownUnitPreview = countdownBlock && countdownBlock.querySelector(".mobishop-countdown-layout-preview");
			var selectedUnits = {
				days: ["days"],
				days_hours: ["days", "hours"],
				days_hours_minutes: ["days", "hours", "minutes"],
				days_hours_minutes_seconds: ["days", "hours", "minutes", "seconds"]
			}[target.value] || ["days", "hours", "minutes", "seconds"];
			if (countdownUnitPreview) {
				toArray(countdownUnitPreview.querySelectorAll("[data-countdown-unit]")).forEach(function (unit) {
					unit.hidden = selectedUnits.indexOf(unit.dataset.countdownUnit) < 0;
				});
			}
		}
	...
	builder.addEventListener("input", function (event) {
		...
		if (target.classList.contains("mobishop-block-name-input")) {
			block = target.closest(".mobishop-builder-block");
			name = block ? block.querySelector(".mobishop-block-name") : null;
			if (name) {
				name.textContent = target.value.trim() || labels.untitled || "Untitled Element";
			}
		}
		if (target.classList.contains("mobishop-block-background-picker")) {
			var backgroundValue = target.parentNode.querySelector(".mobishop-block-background-value");
			if (backgroundValue) { backgroundValue.value = target.value; }
		}
		if (target.classList.contains("mobishop-block-background-value") && /^#[0-9a-f]{6}$/i.test(target.value.trim())) {
			var backgroundPicker = target.parentNode.querySelector(".mobishop-block-background-picker");
	...
	form.addEventListener("input", function (event) {
		if (event.target && event.target.closest && event.target.closest(".mobishop-fixed-chrome-card")) { markDirty(); renderPreview(); }
	});
	form.addEventListener("change", function (event) {
		if (event.target && event.target.closest && event.target.closest(".mobishop-fixed-chrome-card")) { markDirty(); renderPreview(); }
	});
	form.addEventListener("click", function(event){
		var block=event.target.closest(".mobishop-builder-block,.mobishop-fixed-chrome-card");
		if(block && event.target.closest(".mobishop-builder-block__header,.mobishop-page-card__header")){
			focusPreviewBlock(block);
			if (block.classList.contains("mobishop-builder-block")) {
				toArray(builder.querySelectorAll(".is-insertion-target")).forEach(function (item) { item.classList.remove("is-insertion-target"); });
				activeInsertionBlock = block;
				block.classList.add("is-insertion-target");
			}
		}
	});
	...
	builder.addEventListener("pointerdown", function (event) {
		...
		handle = target.closest(".mobishop-builder-drag");
		block = handle ? handle.closest(".mobishop-builder-block") : null;
		if (block) {
	...
	builder.addEventListener("dragstart", function (event) {
		...
		dragPreview.classList.add("mobishop-builder-drag-preview");
	...
	builder.addEventListener("dragover", function (event) {
		...
		targetBlock = target.closest(".mobishop-builder-block");
		...
		toArray(builder.querySelectorAll(".mobishop-builder-block:not(.is-dragging)")).forEach(function (item) {
	...
	builder.addEventListener("pointerdown", function (event) {
		...
		var interactive = target && target.closest && target.closest("input, select, textarea, button, a, label, [contenteditable='true'], .mobishop-builder-drag");
	...
	if (phoneScreen) {
		phoneScreen.addEventListener("scroll", function () {
			...
			if (window.MobiShopChromePreview) {
				window.MobiShopChromePreview.updateHeaderProgress(previewContent.querySelector(".mobishop-app-header"), previewCollapseProgress);
			}
		}, { passive: true });
	}
