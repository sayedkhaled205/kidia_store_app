(function () {
	...
	const config = window.mobishopSavedThemePreview || {};
	...
	function buildPayload(snapshot, page) {
		if (page === 'splash') {
			return Promise.resolve({ type: 'mobishop-preview-layout', page: page, layout: {}, splash: snapshot.splash || {} });
		}
		...
		if (page === 'home') {
			return Promise.all([
				...
				return { type: 'mobishop-preview-layout', page: page, layout: payloads[0], home: payloads[1], splash: snapshot.splash || {} };
			});
		}
		...
		if (page === 'category') {
			...
			return Promise.all([
				...
				return { type: 'mobishop-preview-layout', page: page, layout: payloads[0], category: payloads[1], splash: snapshot.splash || {} };
			});
		}

		return layoutRequest.then(function (layout) {
			return { type: 'mobishop-preview-layout', page: page, layout: layout, splash: snapshot.splash || {} };
		});
	}
	...
	document.querySelectorAll('.mobishop-theme-file input[type="file"]').forEach(function (input) {
		input.addEventListener('change', function () {
			const label = input.closest('.mobishop-theme-file');
