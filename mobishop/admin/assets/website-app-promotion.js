(() => {
  ...
  const initializePromotionAdmin = () => {
    ...
    const state = admin.querySelector(".mobishop-app-promotion-state");
    ...
    const updateCampaignStatuses = () => {
      ...
      typeCards.forEach((card) => {
        ...
        if (statusNode) {
          statusNode.className = `mobishop-app-promotion-type__status is-${status}`;
    ...
    const appendQrPlaceholder = (holder) => {
      const placeholder = create("span", "mobishop-app-promo__qr-placeholder");
    ...
    const renderPreview = () => {
      ...
      const promo = create(
        promoTag,
        `mobishop-app-promo mobishop-app-promo--${selectedCampaign}`,
      );
      promo.dataset.mobishopAppPromotion = selectedCampaign;
      promo.style.setProperty("--mobishop-app-promo-brand", primary);
      promo.style.setProperty("--mobishop-app-promo-ink", ink);
      promo.style.setProperty("--mobishop-app-promo-surface", surface);
      ...
      const logo = create("span", "mobishop-app-promo__icon");
      if (logoUrl) {
        ...
        logo.append(create("span", "mobishop-app-promo__phone-icon"));
      }
      promo.append(logo);

      const copy = create("span", "mobishop-app-promo__copy");
      copy.append(
        create(
          "strong",
          "",
      ...
      if (selectedCampaign !== "floating_button") {
        copy.append(create("span", "mobishop-app-promo__tagline", tagline));
        if (
          ...
          copy.append(
            create("span", "mobishop-app-promo__description", description),
          );
        }
        if (offer || coupon) {
          ...
          if (coupon) {
            offerNode.append(
              create("b", "mobishop-app-promo__coupon", coupon),
            );
          }
          copy.append(offerNode);
        } else if (!target) {
          copy.append(
            create("small", "mobishop-app-promo__status", "Coming soon"),
          );
        }
      }
      promo.append(copy);
      ...
      if (selectedCampaign === "desktop_qr") {
        const qr = create("span", "mobishop-app-promo__qr");
      ...
      if (target && selectedCampaign !== "floating_button") {
        promo.append(create("a", "mobishop-app-promo__action", buttonLabel));
      }
      if (["bottom_sheet", "popup"].includes(selectedCampaign)) {
        promo.append(
          create("button", "mobishop-app-promo__later", dismissLabel),
        );
      }
      if (!["floating_button", "inline_banner"].includes(selectedCampaign)) {
        promo.append(create("button", "mobishop-app-promo__dismiss", "×"));
      }
      if (selectedCampaign === "popup") {
        output.append(create("div", "mobishop-app-promo-backdrop"));
      }
      output.append(promo);
    ...
    admin
      ...
      ?.addEventListener("click", async (event) => {
        ...
        try {
          await navigator.clipboard.writeText("[mobishop_app_promo]");
  ...
  document.addEventListener("mobishop:cms-page-ready", initializePromotionAdmin);
})();
