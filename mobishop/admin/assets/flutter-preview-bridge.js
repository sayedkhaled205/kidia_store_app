(function () {
	...
	var form = root && root.querySelector("form.mobishop-page-editor");
	if (!root || !frame || !form) { return; }

	var page = root.dataset.page || "catalog";
	var config = window.mobishopFlutterPreview || {};
	var ready = false;
	var controller = null;
	var requestNumber = 0;
	...
	function sendLayout(layout) {
		...
		frame.contentWindow.postMessage(JSON.stringify({ type: "mobishop-preview-layout", page: page, layout: layout }), frameOrigin);
	}
	...
	window.addEventListener("message", function (event) {
		...
		if (message && message.type === "mobishop-flutter-preview-ready") { ready = true; showFlutter(); refresh(); }
	});
	form.addEventListener("input", refresh);
	form.addEventListener("change", refresh);
	document.addEventListener("mobishop:page-layout-changed", refresh);
