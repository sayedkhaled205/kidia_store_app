(function () {
	'use strict';
	document.documentElement.classList.add('mobishop-cms-js');
	const shell = document.querySelector('.mobishop-cms-shell');
	const completedNoticeStorageKey = 'mobishopCompletedBackgroundJobNotices';
	const backgroundJobStackPositionKey = 'mobishopBackgroundJobStackPositionV1';
	const backgroundJobStackSafeInset = 8;

	function backgroundJobStack() {
		return document.querySelector('[data-mobishop-background-job-stack]');
	}
	...
	function positionBackgroundJobStack(stack, left, top, remember) {
		...
		stack.dataset.mobishopStackPositioned = '1';
	...
	function restoreBackgroundJobStackPosition(stack) {
		if (!stack || stack.dataset.mobishopStackPositioned === '1') return;
		try {
	...
	function bindBackgroundJobStackDrag(stack) {
		if (!stack || stack.dataset.mobishopStackDragBound === '1') return;
		stack.dataset.mobishopStackDragBound = '1';
		...
		stack.addEventListener('pointerdown', function (event) {
			...
			const card = eventTarget ? eventTarget.closest('[data-mobishop-background-job]') : null;
			if (
				!card ||
				!stack.contains(card) ||
			...
			const stop = function (stopEvent) {
				...
				stack.dataset.mobishopSuppressDragClick = '1';
				window.setTimeout(function () { delete stack.dataset.mobishopSuppressDragClick; }, 0);
			};
		...
		stack.addEventListener('click', function (event) {
			if (stack.dataset.mobishopSuppressDragClick !== '1') return;
			const eventTarget = event.target && typeof event.target.closest === 'function' ? event.target : null;
			if (!eventTarget || !eventTarget.closest('[data-mobishop-background-job]')) return;
		...
		if (typeof window.ResizeObserver === 'function') {
			...
			stack.mobishopBackgroundJobResizeObserver = observer;
		}
	}
	...
	function appendBackgroundJobCard(card) {
		...
		const job = String(card.dataset.mobishopBackgroundJob || '');
		const existing = job
			? Array.from(stack.querySelectorAll('[data-mobishop-background-job]')).find(function (candidate) {
				return candidate !== card && String(candidate.dataset.mobishopBackgroundJob || '') === job;
			})
			: null;
		if (existing) {
			card.remove();
	...
	function completedNoticeKey(card) {
		const job = String(card.dataset.mobishopBackgroundJob || 'background-job');
	...
	function syncBuilderScreen(enabled) {
		...
		document.body.classList.toggle('mobishop-cms-builder-screen', active);
		document.documentElement.classList.toggle('mobishop-cms-builder-screen', active);
	}

	syncBuilderScreen(document.body.classList.contains('mobishop-cms-builder-screen'));

	/* One permanent CMS shell; the server returns view fragments only. */
	function installPersistentCmsNavigation() {
		if (window.mobishopCmsNavigatorInstalled) return;
		window.mobishopCmsNavigatorInstalled = true;
		const config = window.mobishopCMSNavigation || {};
		...
		const legacyViews = {
			...
			'mobishop-saved-themes': 'saved-themes',
			'mobishop-store-data': 'store-data',
			'mobishop-ai-insights': 'ai-insights',
			'mobishop-bundles': 'bundles',
			'mobishop-push-notifications': 'push',
			'mobishop-website-app-promotion': 'website-promotion'
		};
		...
		const pluginPage = function (url) {
			try {
				...
				return target.origin === window.location.origin &&
					...
					(target.searchParams.get('page') === 'mobishop' || Boolean(legacyViews[target.searchParams.get('page')]));
			} catch (_error) {
				return false;
			}
		};
		const keyFor = function (url) {
			...
			if (legacyView) {
				target.searchParams.set('page', 'mobishop');
		...
		const pluginAsset = function (asset) {
			if (/^mobishop(?:-|$)/.test(String(asset.handle || ''))) {
				return true;
			}
			try {
				return new URL(asset.src, window.location.href).pathname.indexOf('/plugins/mobishop/') !== -1;
			} catch (_error) {
				return false;
			}
		};
		...
		const hardNavigate = function (url) {
			viewCache.clear();
			if (typeof window.mobishopCmsHardNavigate === 'function') {
				window.mobishopCmsHardNavigate(url);
				return;
			}
		...
		const updateNavigation = function (payload) {
			const sidebar = document.querySelector('[data-mobishop-cms-sidebar]');
			const shell = document.querySelector('[data-mobishop-cms-shell]');
			sidebar.querySelectorAll('[data-mobishop-sidebar-view]').forEach(function (link) {
				link.classList.toggle('is-active', link.dataset.mobishopSidebarView === payload.activeSidebar);
			});
			shell.hidden = !payload.showPageTabs;
			shell.querySelectorAll('[data-mobishop-page-view]').forEach(function (link) {
				link.classList.toggle('is-active', link.dataset.mobishopPageView === (payload.view === 'pages' ? 'splash' : payload.view));
			});
		};
		const persistentShellNode = function (node, sidebar, shell) {
			return node === sidebar ||
				node === shell ||
				(node.nodeType === 1 && (
					node.matches('[data-mobishop-background-job-stack]') ||
					node.matches('[data-mobishop-background-job]') ||
					node.matches('.mobishop-ai-progress-overlay.is-global')
				));
		};
		const promoteBackgroundJobCards = function (content) {
			Array.from(content.querySelectorAll('[data-mobishop-background-job]')).forEach(function (card) {
				if (card.closest('[data-mobishop-background-job-stack]')) return;
		...
		const loadPage = async function (url, pushState, navigationOptions) {
			...
			const focusState = options.preserveFocus && activeElement && currentContent.contains(activeElement) && activeElement.name
				...
				: null;
			if (window.mobishopCmsNavigationController) window.mobishopCmsNavigationController.abort();
			const controller = new AbortController();
			window.mobishopCmsNavigationController = controller;
			try {
				let payload;
				...
				document.dispatchEvent(new CustomEvent('mobishop:cms-before-page-change', {
					detail: { url: cacheKey }
				}));
				...
				if (pushState) {
					if (options.replaceState) {
						history.replaceState({ mobishopCmsPage: true }, '', cacheKey);
					} else {
						history.pushState({ mobishopCmsPage: true }, '', cacheKey);
					}
				}
				if (focusState) {
				...
				document.dispatchEvent(new CustomEvent('mobishop:cms-page-ready', { detail: { url: window.location.href } }));
			} catch (error) {
				if (error.name === 'AbortError') return;
				const message = document.createElement('div');
				message.className = 'notice notice-error mobishop-cms-navigation-error';
				...
				currentContent.classList.remove('is-mobishop-page-loading');
			}
		};
		window.mobishopCmsNavigate = function (url, options) {
	...
	function initThemeModal() {
		const saveForm = document.querySelector('[data-mobishop-save-theme]');
		const modal = document.querySelector('[data-mobishop-theme-modal]');
		if (!saveForm || !modal) return;
		const input = modal.querySelector('[data-mobishop-theme-name]');
		const error = modal.querySelector('[data-mobishop-theme-error]');
		const close = function () { modal.hidden = true; document.body.classList.remove('mobishop-modal-open'); };
		saveForm.addEventListener('submit', function (event) {
			if (saveForm.dataset.confirmed === '1') return;
			event.preventDefault();
			modal.hidden = false;
			document.body.classList.add('mobishop-modal-open');
			input.value = '';
			error.hidden = true;
			window.setTimeout(function () { input.focus(); }, 30);
		});
		modal.querySelectorAll('[data-mobishop-theme-cancel]').forEach(function (button) { button.addEventListener('click', close); });
		modal.querySelector('[data-mobishop-theme-confirm]').addEventListener('click', function () {
			const name = input.value.trim();
			if (!name) { error.hidden = false; input.focus(); return; }
			const editorAction = document.querySelector('input[name="action"][value^="mobishop_save_"]');
			const editorForm = editorAction ? editorAction.form : null;
			if (editorForm && editorForm !== saveForm) {
				const field = document.createElement('input');
				field.type = 'hidden'; field.name = 'mobishop_save_theme_name'; field.value = name;
		...
		input.addEventListener('keydown', function (event) {
			if (event.key === 'Enter') { event.preventDefault(); modal.querySelector('[data-mobishop-theme-confirm]').click(); }
		});
	}
	...
	initThemeModal();
	document.querySelectorAll('.mobishop-theme-file input[type="file"]').forEach(function (input) {
		input.addEventListener('change', function () {
			const name = input.closest('.mobishop-theme-file').querySelector('[data-theme-file-name]');
	...
	const initCustomDateFilters = function (root) {
		(root || document).querySelectorAll('.mobishop-date-filter').forEach(function (filter) {
			...
			if (datePreset.dataset.mobishopCustomDatesBound !== '1') {
				datePreset.dataset.mobishopCustomDatesBound = '1';
	...
	const navigateFreshStoreData = function (url, options) {
		if (typeof window.mobishopCmsNavigate === 'function') {
			return window.mobishopCmsNavigate(url, Object.assign({
				fresh: true,
				replaceState: true,
				preserveScroll: true
			}, options || {}));
	...
	const applyInstantFilter = function (form, trigger) {
		...
		form.classList.add('is-mobishop-loading');
		...
		return Promise.resolve(navigateFreshStoreData(storeDataUrlFromForm(form), {
			...
			form.classList.remove('is-mobishop-loading');
	...
	document.addEventListener('input', function (event) {
		const search = event.target.closest('form[data-mobishop-instant-filter] input[type="search"]');
		if (!search) return;
		scheduleInstantFilter(search.form, search, 350);
	});
	document.addEventListener('change', function (event) {
		const field = event.target.closest('form[data-mobishop-instant-filter] select, form[data-mobishop-instant-filter] input[type="date"]');
		if (!field) return;
		if (field.form.matches('[data-mobishop-reporting-filter]') && field.form.dataset.mobishopReportingReady !== '1') return;
		scheduleInstantFilter(field.form, field, 0);
	});
	document.addEventListener('submit', function (event) {
		const form = event.target.closest('form[data-mobishop-instant-filter]');
		if (!form) return;
		event.preventDefault();
		if (form.matches('[data-mobishop-reporting-filter]') && form.dataset.mobishopReportingReady !== '1') return;
		void applyInstantFilter(form, document.activeElement);
	});
	document.addEventListener('click', async function (event) {
		const button = event.target.closest('[data-mobishop-generate-store-reporting]');
		if (!button || button.disabled) return;
		const form = button.closest('form[data-mobishop-reporting-filter]');
		...
		form.classList.add('is-mobishop-loading');
		form.setAttribute('aria-busy', 'true');
		form.querySelectorAll('.mobishop-reporting-error').forEach(function (notice) { notice.remove(); });
		try {
			const config = window.mobishopCMSBackground || {};
			const data = new FormData(form);
			data.set('action', 'mobishop_generate_store_reporting');
	...
	const setInstantActionBusy = function (form, busy) {
		form.classList.toggle('is-mobishop-loading', busy);
		if (busy) {
			form.setAttribute('aria-busy', 'true');
		} else {
			form.removeAttribute('aria-busy');
		...
		form.querySelectorAll('button,select').forEach(function (control) {
			if (busy) {
				control.dataset.mobishopWasDisabled = control.disabled ? '1' : '0';
				control.disabled = true;
			} else if (control.dataset.mobishopWasDisabled !== '1') {
				control.disabled = false;
			}
			if (!busy) delete control.dataset.mobishopWasDisabled;
		});
	};
	document.addEventListener('change', function (event) {
		const channel = event.target.closest('form[data-mobishop-instant-action="coupon-channel"] select[name="coupon_channel"]');
	...
	document.addEventListener('submit', async function (event) {
		const form = event.target.closest('form[data-mobishop-instant-action]');
		if (!form) return;
		event.preventDefault();
		...
		try {
			...
			if (channel && channel.dataset.mobishopSavedValue) {
				channel.value = channel.dataset.mobishopSavedValue;
			}
			form.classList.add('is-mobishop-error');
			window.setTimeout(function () { form.classList.remove('is-mobishop-error'); }, 1800);
		} finally {
			if (form.isConnected) setInstantActionBusy(form, false);
		}
	});
	const aiBackgroundConfig = window.mobishopCMSBackground || {};
	const aiDockPositionKey = 'mobishop_ai_progress_position_v1';
	...
	const createAiDock = function () {
		...
		dock.className = 'mobishop-ai-progress-overlay is-docked is-global';
		dock.dataset.aiProgressOverlay = '';
		dock.dataset.mobishopBackgroundJob = 'generate-offers';
		dock.setAttribute('aria-live', 'polite');
		dock.innerHTML =
			'<div class="mobishop-ai-progress-card">' +
			'<div class="mobishop-ai-progress-ring" data-ai-progress-ring style="--mobishop-ai-progress:0"><strong data-ai-progress-value>0%</strong></div>' +
			'<h2>Store analysis</h2><p data-ai-progress-stage>Reading store data in the background…</p>' +
			'<strong class="mobishop-ai-progress-count" data-ai-progress-count>Checking progress…</strong>' +
			'<div class="mobishop-ai-progress-track"><i data-ai-progress-bar></i></div>' +
			'<small data-ai-progress-note>You can continue using every CMS page while this runs.</small>' +
			'<div class="mobishop-ai-progress-actions">' +
			'<a class="button button-primary" data-ai-view-results hidden>View results</a>' +
			'<button class="button mobishop-ai-cancel-button" type="button" data-ai-cancel-button>Cancel analysis</button>' +
			'</div></div>';
	...
	const positionAiDock = function (overlay, left, top, remember) {
		...
		if (overlay.closest('[data-mobishop-background-job-stack]')) {
	...
	const bindAiDockDrag = function (overlay) {
		...
		const card = overlay.querySelector('.mobishop-ai-progress-card');
		if (!card) return;
		overlay.dataset.aiDragBound = '1';
		card.addEventListener('pointerdown', function (event) {
			if (
				!overlay.classList.contains('is-docked') ||
				overlay.closest('[data-mobishop-background-job-stack]') ||
				event.button !== 0 ||
	...
	const persistAiProgressAcrossNavigation = function (overlay, jobId) {
		...
		overlay.dataset.mobishopBackgroundJob = 'generate-offers';
	...
	if (window.__MOBISHOP_AI_PROGRESS_TEST__) {
		window.MobiShopAiProgressTest = {
	...
	const pollBackgroundJob = async function (jobId, overlay) {
		...
		while (active && document.body.contains(overlay) && overlay.dataset.aiPollingJob === jobId) {
			try {
				const payload = await aiRequest('mobishop_ai_analysis_status', {job_id: jobId, advance: '1'});
	...
	const bindAiOverlayActions = function (overlay, getJobId, setForeground) {
		...
		if (background) {
			background.addEventListener('click', async function () {
				...
				try {
					const payload = await aiRequest('mobishop_background_ai_analysis', {job_id: jobId});
					setForeground(false);
			...
			cancel.addEventListener('click', async function () {
				...
				try {
					if (overlay.dataset.aiComplete === '1') {
						await aiRequest('mobishop_dismiss_ai_analysis', {job_id: jobId});
					} else {
						setForeground(false);
						await aiRequest('mobishop_cancel_ai_analysis', {job_id: jobId});
					}
	...
	if (overlay) {
		...
		document.addEventListener('mobishop:cms-before-page-change', function () {
			...
			aiRequest('mobishop_background_ai_analysis', {job_id: activeJobId})
	...
	document.addEventListener('submit', async function (event) {
		...
		if (!aiGenerateForm) return;
			...
			try {
				...
				let payload = await aiRequest('mobishop_start_ai_analysis', {
				...
				while (!payload.done && foreground) {
					try {
						payload = await aiRequest('mobishop_ai_analysis_status', {job_id: activeJobId, advance: '1'});
						renderAiProgress(overlay, payload);
						failures = 0;
					} catch (stepError) {
						failures += 1;
	...
	if (configuredJob && !viewingAiResult) {
		...
		aiRequest('mobishop_dismiss_ai_analysis', {job_id: configuredJob}).catch(function () {});
	}
	...
	document.addEventListener('mobishop:cms-page-ready', function () { syncRecoveryControls(document); });
	document.addEventListener('change', function (event) {
	...
	document.addEventListener('click', async function (event) {
		...
		const content = row ? row.querySelector('.mobishop-cart-details-content') : null;
		...
		content.innerHTML = '<p class="mobishop-cart-details-loading">Loading order details…</p>';
		...
		try {
			const config = window.mobishopCMSBackground || {};
			const body = new URLSearchParams({
				action: 'mobishop_abandoned_cart_details',
			...
			const historyHtml = orders.length
				? '<ul>' + orders.map(function (order) {
					const openOrder = order.edit_url
						? '<a class="button mobishop-open-order-button" href="' + escapeHtml(order.edit_url) + '" target="_blank" rel="noopener">Open order</a>'
						: '';
			...
			const alternativeHtml = alternative
				? '<div class="mobishop-cart-alternative-order"><strong>#' + escapeHtml(alternative.id) + '</strong><span>' + escapeHtml(alternative.date) + '</span><em>' + escapeHtml(alternative.total) + ' ' + escapeHtml(alternative.currency) + '</em>' + (alternative.edit_url ? '<a class="button mobishop-open-order-button" href="' + escapeHtml(alternative.edit_url) + '" target="_blank" rel="noopener">Open order</a>' : '') + '</div>'
			...
			const customerHtml =
				'<dl class="mobishop-customer-summary">' +
			...
			content.innerHTML =
				...
				'<div class="mobishop-cart-details-stack">' +
			...
			const marker = document.querySelector('[data-abandoned-cart-row="' + CSS.escape(cartId) + '"] .mobishop-cart-segment');
			if (marker) marker.className = 'mobishop-cart-segment is-' + segment;
		} catch (error) {
			content.innerHTML = '<p class="mobishop-cart-details-error">' + escapeHtml(error.message || 'Order details could not be loaded.') + '</p>';
		} finally {
	...
	const initLiveStoreData = function (root) {
		stopLiveStoreData();
		const liveStoreRegions = Array.from((root || document).querySelectorAll('[data-mobishop-live-store-data]'));
		...
		const refreshLiveStoreData = async function () {
			...
			try {
				const url = new URL(window.location.href);
				url.searchParams.set('mobishop_live', String(Date.now()));
				const response = await fetch(url.toString(), {
					credentials: 'same-origin',
					cache: 'no-store',
					headers: {'X-MobiShop-Live-Data': '1'},
					signal: controller.signal
				});
				if (!response.ok) throw new Error('Live Store Data request failed');
				const page = new DOMParser().parseFromString(await response.text(), 'text/html');
				const freshRegions = Array.from(page.querySelectorAll('[data-mobishop-live-store-data]'));
				liveStoreRegions.forEach(function (region) {
					const key = region.dataset.mobishopLiveStoreData;
					const fresh = freshRegions.find(function (candidate) {
						return candidate.dataset.mobishopLiveStoreData === key;
					});
					...
					const expandedCartDetails = Array.from(
						...
						const detailsContent = detailsRow ? detailsRow.querySelector('.mobishop-cart-details-content') : null;
						return {
							cartId: cartId,
	...
	document.addEventListener('mobishop:cms-before-page-change', stopLiveStoreData);
	document.addEventListener('mobishop:cms-page-ready', function () { initLiveStoreData(document); });
	...
	document.querySelectorAll('.wrap > .notice-success').forEach(function (notice) {
		notice.classList.add('mobishop-global-save-toast');
