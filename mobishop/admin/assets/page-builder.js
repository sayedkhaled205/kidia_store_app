(function ($) {
	"use strict";
	var root = document.querySelector(".mobishop-page-builder");
	if (!root) { return; }
	var list = document.getElementById("mobishop-page-elements");
	var preview = document.getElementById("mobishop-page-live-preview");
	var form = root.querySelector("form");
	var phoneScreen = root.querySelector(".mobishop-page-phone__screen");
	var dragged = null;
	var frame = 0;
	var activePreviewElement = "";
	var config = window.mobishopPageBuilder || {};
	var products = Array.isArray(config.products) ? config.products : [];
	var previewCollapseProgress = 0;
	var wishlistPicker = document.getElementById("mobishop-wishlist-element-picker");
	...
	function icon(name) { return '<span class="mobishop-app-icon mobishop-app-icon--' + name + '" aria-hidden="true"></span>'; }
	...
	function syncProductTabs(editor) {
		if (!editor) { return; }
		var rows = array(editor.querySelectorAll(".mobishop-product-tab-row")).map(function (row) {
			return {
				label: (row.querySelector(".mobishop-product-tab-label").value || "").trim(),
				target: row.querySelector(".mobishop-product-tab-target").value || "overview",
				enabled: row.querySelector(".mobishop-product-tab-enabled input").checked
			};
		}).filter(function (tab) { return tab.label; }).slice(0, 10);
		var input = editor.querySelector(".mobishop-product-tabs-json");
		input.value = JSON.stringify(rows);
		input.dispatchEvent(new window.Event("input", { bubbles: true }));
	}
	function productTabRow(editor) {
		var row = document.createElement("div");
		row.className = "mobishop-product-tab-row";
		row.innerHTML = '<input type="text" class="mobishop-product-tab-label" value="New tab" placeholder="Tab label"><select class="mobishop-product-tab-target"><option value="overview">Overview / product information</option><option value="variations">Variations</option><option value="description">Description</option><option value="reviews">Reviews</option><option value="recommend">Related products</option></select><label class="mobishop-product-tab-enabled"><input type="checkbox" checked>Show</label><button type="button" class="button mobishop-product-tab-remove" aria-label="Remove tab"><span class="dashicons dashicons-trash"></span></button>';
		editor.querySelector(".mobishop-product-tabs-rows").appendChild(row);
	...
	function applyWishlistPreviewState(options) {
		...
		array(list.querySelectorAll("[data-wishlist-state]")).forEach(function (card) {
			...
			if (active && options && options.open) {
				var body = card.querySelector(":scope > .mobishop-page-card__body");
				card.classList.add("is-open");
				if (body) { body.hidden = false; }
				var expand = card.querySelector(".mobishop-page-expand");
	...
	function openWishlistPicker() {
		...
		document.body.classList.add("mobishop-wishlist-picker-open");
	...
	function closeWishlistPicker() {
		...
		document.body.classList.remove("mobishop-wishlist-picker-open");
	}

	function updateIndexes() {
		array(list.querySelectorAll(".mobishop-page-card")).forEach(function (card, index) {
	...
	function duplicateWishlistElement(card) {
		...
		var idInput = clone.querySelector(".mobishop-page-element-id");
		if (idInput) { idInput.value = instance; }
		var remove = clone.querySelector(".mobishop-page-remove");
		if (remove) { remove.hidden = false; }
		clone.classList.add("is-open");
		var body = clone.querySelector(":scope > .mobishop-page-card__body");
		if (body) { body.hidden = false; }
	...
	function productCards(card, forcedLimit) {
		...
		return '<div class="mobishop-app-products" style="--columns:' + columns + ';gap:' + gap + 'px">' + items.map(function (item) {
			var image = item.image_url ? '<img src="' + escapeHtml(item.image_url) + '" alt="">' : '<span class="mobishop-app-product__fallback">K</span>';
			var showQuickAdd = field(card, "show_quick_add") ? checked(card, "show_quick_add", true) : checked(card, "quick_add_enabled", true);
			var copy = (checked(card, "show_name", true) ? '<strong>' + escapeHtml(item.name || "MobiShop product") + '</strong>' : "") + (checked(card, "show_rating", false) ? '<small class="mobishop-app-rating">★ 4.8</small>' : "") + (checked(card, "show_price", true) ? '<b>' + money(item) + '</b>' : "") + (checked(card, "show_regular_price", false) ? '<del>520 ج.م</del>' : "");
			return '<article class="mobishop-app-product is-' + style + '" style="border-radius:' + number(card, "card_radius", 0) + 'px"><div class="mobishop-app-product__image" style="aspect-ratio:' + number(card, "image_ratio", 1) + '">' + image + (checked(card, "show_wishlist", false) ? productWishlist(card) : "") + (showQuickAdd ? quickAdd(card) : "") + (checked(card, "show_badge", false) ? '<b class="mobishop-app-badge">SALE</b>' : "") + '</div>' + (copy ? '<div class="mobishop-app-product__copy">' + copy + '</div>' : "") + '</article>';
		}).join("") + '</div>';
	}
	...
	function wishlistMessage(card, signIn) {
		...
		var illustration = imageUrl
			? '<img class="mobishop-app-wishlist-illustration" src="' + escapeHtml(imageUrl) + '" alt="">'
			: '<span class="mobishop-app-wishlist-bag" aria-hidden="true"><i></i></span>';
		var buttonStyle = value(card, "button_style", "outline");
		return '<section class="mobishop-app-empty mobishop-app-wishlist-message" style="padding-top:' + number(card, "top_spacing", 56) + 'px;padding-bottom:' + number(card, "bottom_spacing", 96) + 'px;--wishlist-illustration-size:' + number(card, "illustration_size", 104) + 'px;--wishlist-content-gap:' + number(card, "content_gap", 16) + 'px">' +
			illustration +
	...
	function renderHeader(card) {
		...
		if (window.MobiShopChromePreview) { return window.MobiShopChromePreview.renderHeader(card, titles[root.dataset.page] || "المنتجات", { collapseProgress: previewCollapseProgress, page: root.dataset.page }); }
	...
	function previewElement(card) {
		...
		if (id === "size_chart_content") {
			...
			return '<section class="mobishop-app-section mobishop-app-size-chart is-'+escapeHtml(style)+'" style="color:'+color(card,"text_color","#1F2933")+'"><h2 style="font-size:'+number(card,"title_size",24)+'px">'+escapeHtml(value(card,"title","Size chart"))+'</h2>'+(checked(card,"show_description",true)?'<p>'+escapeHtml(value(card,"description","Choose the size that fits you best."))+'</p>':'')+'<div class="mobishop-app-size-chart__rows">'+sizes.map(function(size,index){return '<div style="min-height:'+number(card,"row_height",52)+'px;background:'+rowColor+';border-color:'+(index===2?accent:"transparent")+'">'+size+'</div>';}).join("")+'</div></section>';
		}
		if (id === "filter_bar") {
			var buttons = [];
			...
			return '<div class="mobishop-app-filter' + (checked(card, "sticky", false) ? " is-sticky" : "") + '" style="width:' + number(card, "block_width", 100) + '%;height:' + number(card, "block_height", 56) + 'px;gap:' + number(card, "button_gap", 8) + 'px;background:' + color(card, "background_color", "#FFFFFF") + ';--icon-size:' + number(card, "icon_size", 22) + 'px;--icon-color:' + color(card, "icon_color", "#1F2933") + ';--icon-offset-y:' + number(card, "filter_icon_offset_y", -2) + 'px">' + buttons.map(function (button) { return '<button style="border-color:' + color(card, "border_color", "#DDE3E8") + ';border-radius:' + number(card, "button_radius", 12) + 'px">' + button + '</button>'; }).join("") + (checked(card, "show_result_count", false) ? '<small>24 منتج</small>' : "") + '</div>';
		}
		if (id === "image_gallery") {
			var image = first.image_url ? '<img src="' + escapeHtml(first.image_url) + '" alt="">' : '<span class="mobishop-app-gallery__fallback">MOBISHOP</span>';
			return '<section class="mobishop-app-gallery" style="aspect-ratio:' + number(card, "aspect_ratio", .75) + ';--gallery-fit:' + escapeHtml(value(card, "fit", "contain")) + ';background:' + color(card, "background_color", "#F4F2F3") + '">' + image + (checked(card, "show_counter", true) ? '<span class="mobishop-app-gallery__count" style="background:' + color(card, "counter_background", "#8A8585") + ';color:' + color(card, "counter_text_color", "#FFFFFF") + '">1 / 4</span>' : "") + (checked(card, "show_thumbnails", false) ? '<span class="mobishop-app-gallery__thumbnails">● ● ●</span>' : "") + (checked(card, "enable_zoom", false) ? icon("zoom") : "") + '</section>';
		}
		if (id === "product_summary") {
			return '<section class="mobishop-app-summary">' + (checked(card, "show_badge", false) ? '<span class="mobishop-app-sale">خصم</span>' : "") + '<div style="display:flex;justify-content:space-between;align-items:center">' + (checked(card, "show_price", true) ? '<div class="mobishop-app-price"><b>' + money(first) + '</b>' + (checked(card, "show_regular_price", true) ? '<del>520 ج.م</del>' : "") + '</div>' : "") + (checked(card, "show_rating", true) ? '<div class="mobishop-app-stars">★★★★★ ' + (checked(card, "show_review_count", true) ? '<u>252</u>' : '') + '</div>' : "") + '</div>' + (checked(card, "show_name", true) ? '<h2>' + escapeHtml(first.name || "Toddler Girl Dress") + '</h2>' : "") + (checked(card, "show_sku", false) ? '<small>SKU: MOBISHOP-001</small>' : "") + (checked(card, "show_stock", false) ? '<small class="mobishop-app-stock">متوفر</small>' : "") + '</section>';
		}
		if (id === "variations") {
			return '<section class="mobishop-app-variations"><h3>اللون <span>وردي</span></h3><div><button class="is-selected">وردي</button><button>أصفر</button></div><h3>المقاس</h3><div><button>2Y</button><button class="is-selected">3Y</button><button>4Y</button></div></section>';
		}
		if (id === "purchase_bar") {
			return checked(card, "show_quantity", true) ? '<section class="mobishop-app-purchase-inline"><strong>الكمية</strong><div class="mobishop-app-quantity"><button>−</button><b>1</b><button>＋</button></div></section>' : '';
		}
		if (id === "description") {
			return '<section class="mobishop-app-details"><h3>الوصف والتفاصيل</h3>' + (checked(card, "show_description", true) ? '<p>خامات مريحة وجودة مناسبة للأطفال.</p>' : "") + (checked(card, "show_attributes", true) ? '<div><span>الخامة</span><b>قطن</b></div><div><span>اللون</span><b>وردي</b></div>' : "") + (checked(card, "show_shipping", true) ? '<div><span>الشحن</span><b>2–5 أيام</b></div>' : "") + '</section>';
		}
		if (id === "reviews") { return '<section class="mobishop-app-reviews"><h3>' + escapeHtml(value(card, "title", "Reviews")) + ' (252) ›</h3><b>4.5</b><span>★★★★☆</span><small>Small 1% · True to size 99% · Large 0%</small></section>'; }
		if (id === "sign_in_state") { return wishlistMessage(card, true); }
		if (id === "empty_state") { return wishlistMessage(card, false); }
		if (id === "account_summary") { return '<section class="mobishop-app-account-summary is-' + value(card, "card_style", "elevated") + '"><span class="mobishop-app-avatar" style="width:' + number(card, "avatar_size", 66) + 'px;height:' + number(card, "avatar_size", 66) + 'px">B</span><div><strong>' + escapeHtml(value(card, "guest_title", "بسمة زيدان")) + '</strong>' + (checked(card, "show_email", true) ? '<small>customer@example.com</small>' : "") + (checked(card, "show_addresses", true) ? '<small>العناوين</small>' : "") + (checked(card, "show_profile", true) ? '<small>الملف الشخصي</small>' : "") + '</div></section>'; }
		if (id === "account_menu") {
			...
			return '<section class="mobishop-app-account-menu is-' + value(card, "style", "list") + '" style="--menu-color:' + color(card, "icon_color", "#1F6F61") + '">' + menu.map(function (item) { return '<div>' + icon(item[0]) + '<b>' + item[1] + '</b>' + icon("chevron") + '</div>'; }).join("") + '</section>';
		}
		if (id === "logout_button") { return '<button class="mobishop-app-logout is-' + value(card, "style", "outline") + '" style="--logout-color:' + color(card, "color", "#B42318") + '">' + icon("logout") + escapeHtml(value(card, "label", "تسجيل الخروج")) + '</button>'; }
		return "";
	}

	function renderFooter(card) {
		if (window.MobiShopChromePreview) { return window.MobiShopChromePreview.renderFooter(card, { page: root.dataset.page }); }
		if (!card || !checked(card, "enabled", true)) { return ""; }
		if (value(card, "style", "navigation") === "product_action") {
			return '<footer class="mobishop-app-footer mobishop-app-footer--product" style="height:' + number(card, "height", 84) + 'px;background:' + color(card, "background_color", "#FFFFFF") + '">' + (checked(card, "show_share", true) ? '<span>' + icon("share") + '<b>' + escapeHtml(value(card, "share_label", "مشاركة")) + '</b></span>' : "") + (checked(card, "show_like", true) ? '<span>' + icon("heart") + '<b>' + escapeHtml(value(card, "like_label", "إعجاب")) + '</b></span>' : "") + (checked(card, "show_add_to_cart", true) ? '<button style="background:' + color(card, "button_color", "#1F2933") + ';color:' + color(card, "button_text_color", "#FFFFFF") + ';border-radius:' + number(card, "button_radius", 28) + 'px">' + escapeHtml(value(card, "add_to_cart_label", "أضف للحقيبة")) + '</button>' : "") + '</footer>';
		}
		var nav = [];
	...
	function renderPreview() {
		...
		var html = renderHeader(header) + '<main class="mobishop-app-page mobishop-app-page--' + escapeHtml(root.dataset.page || "page") + '"' + (pageBackground ? ' style="background:' + escapeHtml(pageBackground) + '"' : '') + '>';
		array(list.querySelectorAll(".mobishop-page-card")).forEach(function (card) { if (!card.hidden && checked(card, "enabled", true)) { var background=value(card,"background_color","").trim()||"transparent",mergeUp=number(card,"margin_top",0),mergeDown=number(card,"margin_bottom",0),legacySpace=number(card,"padding_vertical",0);html += '<div class="mobishop-page-element-frame" style="margin:0;transform:translateY('+(mergeDown-mergeUp)+'px);padding:'+number(card,"space_up",legacySpace)+'px '+number(card,"padding_horizontal",0)+'px '+number(card,"space_down",legacySpace)+'px;background:'+escapeHtml(background)+'">'+previewElement(card)+'</div>'; } });
		html += '</main>' + renderFooter(footer);
		preview.innerHTML = html;
		var headerNode = preview.querySelector(".mobishop-app-header");
		var footerNode = preview.querySelector(".mobishop-app-footer");
		if (headerNode) { headerNode.classList.add("mobishop-page-preview-header"); }
		if (footerNode) { footerNode.classList.add("mobishop-page-preview-footer"); }
		array(preview.querySelectorAll(".mobishop-app-product")).forEach(function (node) { node.classList.add("mobishop-page-preview-product"); });
		array(list.querySelectorAll(".mobishop-page-card:not([hidden])")).forEach(function (card, index) {
			var rendered = preview.querySelectorAll(".mobishop-app-page > *")[index];
			if (rendered) { rendered.classList.add("mobishop-page-preview-element"); rendered.dataset.previewElement = card.dataset.instanceId || card.dataset.element || ""; }
		});
		if(activePreviewElement){var active=activePreviewElement==="header"?preview.querySelector(".mobishop-page-preview-header"):activePreviewElement==="footer"?preview.querySelector(".mobishop-page-preview-footer"):preview.querySelector('[data-preview-element="'+activePreviewElement+'"]');if(active){active.classList.add("is-editor-focused");}}
	}
	function focusPreview(card) {
		if (!card || !preview) { return; }
		var part = card.dataset.instanceId || card.dataset.element || "";
		activePreviewElement=part;
		var target = part === "header" ? preview.querySelector(".mobishop-page-preview-header") : part === "footer" ? preview.querySelector(".mobishop-page-preview-footer") : preview.querySelector('[data-preview-element="' + part + '"]');
	...
	root.addEventListener("click", function (event) {
		var collapseAll = event.target.closest("#mobishop-collapse-all");
		var expandAll = event.target.closest("#mobishop-expand-all");
		if (root.dataset.page === "wishlist" && event.target.closest("#mobishop-add-element")) {
			openWishlistPicker();
			return;
		}
		if (event.target.closest("[data-mobishop-close-wishlist-picker]")) {
			closeWishlistPicker();
			return;
		}
		...
		if (collapseAll || expandAll) {
			...
			array(root.querySelectorAll(".mobishop-page-card:not(.mobishop-fixed-chrome-card)")).forEach(function (card) {
				var body = card.querySelector(":scope > .mobishop-page-card__body");
				var toggle = card.querySelector(":scope > .mobishop-page-card__header .mobishop-page-expand, :scope > .mobishop-page-card__header .mobishop-fixed-chrome-expand");
		...
		var duplicate = event.target.closest(".mobishop-page-duplicate");
		if (duplicate) {
			duplicateWishlistElement(duplicate.closest(".mobishop-page-card"));
			return;
		}
		var copyRecommendation = event.target.closest(".mobishop-recommendation-copy");
		if (copyRecommendation) {
			var copyCard = copyRecommendation.closest(".mobishop-page-card");
			recommendationClipboard = {};
			array(copyCard.querySelectorAll(".mobishop-page-card__body [name]")).forEach(function (control) {
		...
		var pasteRecommendation = event.target.closest(".mobishop-recommendation-paste");
		if (pasteRecommendation && recommendationClipboard) {
			var pasteCard = pasteRecommendation.closest(".mobishop-page-card");
			array(pasteCard.querySelectorAll(".mobishop-page-card__body [name]")).forEach(function (control) {
				var match = control.name.match(/\[settings\]\[([^\]]+)\]$/);
		...
		var addTab = event.target.closest(".mobishop-product-tab-add");
		if (addTab) {
			var editor = addTab.closest(".mobishop-product-tabs-editor");
			if (editor.querySelectorAll(".mobishop-product-tab-row").length < 10) {
				productTabRow(editor);
				markDirty();
			}
			return;
		}
		var removeTab = event.target.closest(".mobishop-product-tab-remove");
		if (removeTab) {
			var tabEditor = removeTab.closest(".mobishop-product-tabs-editor");
			if (tabEditor.querySelectorAll(".mobishop-product-tab-row").length > 1) {
				removeTab.closest(".mobishop-product-tab-row").remove();
				syncProductTabs(tabEditor);
				markDirty();
			}
			return;
		}
		var remove = event.target.closest(".mobishop-page-remove");
		if (remove) {
			var removableCard = remove.closest(".mobishop-page-card");
		...
		var button = event.target.closest(".mobishop-page-expand");
		var media = event.target.closest(".mobishop-page-media-choose, .mobishop-page-media-preview");
		if (button && !button.closest(".mobishop-fixed-chrome-card")) { var card = button.closest(".mobishop-page-card"); var body = card.querySelector(".mobishop-page-card__body"); card.classList.toggle("is-open"); body.hidden = !card.classList.contains("is-open"); return; }
		if (media && !media.closest(".mobishop-fixed-chrome-card") && window.wp && wp.media) { var mediaField = media.closest(".mobishop-page-field--image"); var mediaFrame = wp.media({title:"Choose image",button:{text:"Use image"},multiple:false}); mediaFrame.on("select", function () { var attachment = mediaFrame.state().get("selection").first().toJSON(); var input = mediaField.querySelector(".mobishop-page-media-url"); var image = mediaField.querySelector(".mobishop-page-media-preview"); input.value = attachment.url || ""; image.src = attachment.url || ""; image.hidden = !attachment.url; markDirty(); schedulePreview(); }); mediaFrame.open(); }
	});
	...
	root.addEventListener("input", function (event) {
		var editor = event.target.closest && event.target.closest(".mobishop-product-tabs-editor");
		if (editor && !event.target.classList.contains("mobishop-product-tabs-json")) { syncProductTabs(editor); }
	});
	...
	root.addEventListener("click", function (event) {
		var option = event.target.closest(".mobishop-wishlist-access-option, .mobishop-wishlist-preview-option");
	...
	root.addEventListener("change", function (event) {
		if (event.target.matches('[name="layout[settings][wishlist_preview_state]"]')) {
			applyWishlistPreviewState();
			markDirty();
			document.dispatchEvent(new window.CustomEvent("mobishop:page-layout-changed", {
	...
	root.addEventListener("pointerdown", function (event) {
		...
		var interactive = target && target.closest && target.closest("input, select, textarea, button, a, label, [contenteditable='true'], .mobishop-page-drag, .mobishop-builder-drag");
		if (!interactive && active && root.contains(active) && /^(INPUT|SELECT|TEXTAREA|BUTTON)$/.test(active.tagName) && typeof active.blur === "function") {
			active.blur();
		}
	});
	root.addEventListener("click", function (event) {
		var card=event.target.closest(".mobishop-page-card");
		if(card && event.target.closest(".mobishop-page-card__header")){focusPreview(card);}
	});
	list.addEventListener("pointerdown", function (event) { var handle = event.target.closest(".mobishop-page-drag"); var card = handle ? handle.closest(".mobishop-page-card") : null; if (card) { card.draggable = true; } });
	list.addEventListener("dragstart", function (event) { var card = event.target.closest(".mobishop-page-card"); if (!card || !card.draggable) { event.preventDefault(); return; } dragged = card; card.classList.add("is-dragging"); });
	list.addEventListener("dragover", function (event) { if (!dragged) { return; } event.preventDefault(); var target = event.target.closest(".mobishop-page-card"); if (!target || target === dragged) { return; } var rect = target.getBoundingClientRect(); target.insertAdjacentElement(event.clientY > rect.top + rect.height / 2 ? "afterend" : "beforebegin", dragged); });
	...
	if (phoneScreen) { phoneScreen.addEventListener("scroll", function () { previewCollapseProgress=Math.max(0,Math.min(1,phoneScreen.scrollTop/64));if(window.MobiShopChromePreview){window.MobiShopChromePreview.updateHeaderProgress(preview.querySelector(".mobishop-app-header"),previewCollapseProgress);} }, {passive:true}); }
	if ($ && $.fn && $.fn.sortable) { $(list).sortable({handle:".mobishop-page-drag",items:"> .mobishop-page-card",update:function(){updateIndexes();markDirty();schedulePreview();}}); }
