(function () {
	'use strict';
	const form = document.querySelector('.mobishop-setup-form');
	if (!form) return;
	const steps = Array.from(form.querySelectorAll('.mobishop-setup-step'));
	const progress = document.querySelector('.mobishop-setup-progress');
	const next = form.querySelector('.mobishop-setup-next');
	const back = form.querySelector('.mobishop-setup-back');
	const apply = form.querySelector('.mobishop-setup-apply');
	let current = 0;
	...
	function updateActionTheme() {
		form.style.setProperty('--mobishop-setup-theme-color', '#2f806e');
	}
	...
	function selectedThemeCard() {
		...
		return selected ? selected.closest('.mobishop-theme-card') : null;
	}
	...
	function show(index, shouldScroll) {
		...
		if (shouldScroll !== false) {
			const hero = document.querySelector('.mobishop-setup-hero');
	...
	form.addEventListener('change', function (event) {
		...
		if (event.target.matches('.mobishop-theme-card input, [name="setup[primary_color]"], [name="setup[secondary_color]"]')) {
			updateActionTheme();
		}
	});
	...
	const modal = document.querySelector('.mobishop-theme-modal');
	const previewConfig = window.mobishopSetupThemePreview || {};
	...
	function buildPreviewPayload(snapshot, page) {
		...
		return Promise.all([
			...
			return { type: 'mobishop-preview-layout', page: page, layout: layouts[page] || {}, layouts: layouts, home: payloads[1], category: payloads[2], splash: snapshot.splash || {}, demo_catalog: demoCatalog };
		});
	}
	...
	function closeThemeModal() {
		...
		modal.hidden = true;
		document.body.classList.remove('mobishop-theme-preview-open');
	}
	function openThemeModal(card) {
		...
		modal.hidden = false;
		document.body.classList.add('mobishop-theme-preview-open');
		modal.querySelector('.mobishop-theme-modal__close').focus();
		selectPreviewPage('splash');
	}
	form.querySelectorAll('.mobishop-theme-preview-button').forEach(function (button) {
		button.addEventListener('click', function (event) {
			event.preventDefault();
			event.stopPropagation();
			openThemeModal(button.closest('.mobishop-theme-card'));
		});
	});
	if (modal) {
		...
		modal.querySelector('.mobishop-theme-modal__select').addEventListener('click', function () {
	...
	const chooseLogo = form.querySelector('.mobishop-setup-choose-logo');
	if (chooseLogo && window.wp && wp.media) {
		chooseLogo.addEventListener('click', function () {
			...
			frame.on('select', function () {
				...
				form.querySelector('.mobishop-setup-logo-preview').innerHTML = image.url ? '<img src="' + image.url + '" alt="">' : '';
