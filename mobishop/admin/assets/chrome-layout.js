(function () {
  "use strict";
  var dragged = null;
  var previewConfig = window.mobishopChromePreviewConfig || {};
  ...
  function icon(card, item, filled) {
    ...
    return (
      '<span class="mobishop-app-icon mobishop-app-icon--material mobishop-app-icon--' +
      visual +
      " mobishop-app-icon--" +
      item +
      "-" +
  ...
  function headerItem(card, item, defaultTitle) {
    if (item === "logo") {
      var url = value(card, "logo_url", ""),
        ...
        text = esc(value(card, "logo_text", "MobiShop")),
        textColor = value(
          card,
          "logo_text_color",
          value(card, "icon_color", "#1F2933"),
        ...
        logo = url
          ? '<img class="mobishop-app-header-logo" src="' +
            esc(url) +
            '" alt="" style="width:' +
            ...
            'px">'
          : '<strong class="mobishop-app-header-logo-text" style="color:' +
            textColor +
            '">' +
            text +
            "</strong>";
      return (
        '<span class="mobishop-app-header-brand" style="' +
        positionStyle(card, item) +
        '">' +
        logo +
        (subtitle
          ? '<small class="mobishop-app-header-subtitle" style="color:' +
            textColor +
            '">' +
            subtitle +
            "</small>"
    ...
    if (item === "title") {
      var title = String(value(card, "title", defaultTitle || "MobiShop")),
      ...
      return (
        '<strong class="mobishop-app-header-title" style="display:block;max-width:' +
    ...
    if (item === "search_bar") {
      return (
        '<div class="mobishop-app-search" style="' +
        positionStyle(card, item) +
        "width:" +
    ...
    return (
      '<span class="mobishop-app-header-item mobishop-app-header-item--' +
      item +
      " is-" +
      esc(style) +
      '" style="' +
      ...
      (item === "cart" && checked(card, "show_cart_badge", false)
        ? '<i class="mobishop-app-icon-badge" style="' +
          badgeStyle(card) +
          '">1</i>'
        : "") +
      "</span>"
  ...
  function headerRows(card, defaultTitle, layout) {
    ...
    return layout.rows
      .map(function (row, index) {
        ...
        return (
          '<div class="mobishop-app-header-row mobishop-app-layout-row" style="height:' +
          fixedHeight +
          ";align-items:" +
          align +
          ...
          row.columns
            .map(function (column) {
              return (
                '<div class="mobishop-app-header-slot is-' +
                column.align +
                '" style="align-self:' +
                align +
                '">' +
  ...
  function smoothHeaderLayers(
    ...
    if (!morphSearch) {
      return (
        '<div class="mobishop-app-header-transition-layer mobishop-app-header-transition-layer--regular" style="opacity:' +
        (1 - progress) +
        '">' +
        headerRows(card, defaultTitle, regularLayout) +
        '</div><div class="mobishop-app-header-transition-layer mobishop-app-header-transition-layer--compact" style="opacity:' +
        progress +
        '">' +
    ...
    return (
      ...
      '</div><div class="mobishop-app-header-morphing-search" style="top:' +
      searchTop +
      "px;width:" +
      searchWidth +
      '%">' +
      headerItem(card, "search_bar", defaultTitle) +
      '</div><div class="mobishop-app-header-fixed-actions" style="top:' +
      fixedTop +
      'px">' +
      persistent
        .map(function (item) {
  ...
  function updateHeaderProgress(header, progress) {
    ...
    var regularLayer = header.querySelector(
        ".mobishop-app-header-transition-layer--regular",
      ),
      compactLayer = header.querySelector(
        ".mobishop-app-header-transition-layer--compact",
      ),
      search = header.querySelector(".mobishop-app-header-morphing-search");
    if (regularLayer) {
  ...
  function renderHeader(card, defaultTitle, options) {
    ...
    return (
      '<header class="mobishop-app-header mobishop-app-header--composed is-transition-' +
      transition +
      ...
      'px">' +
      layers.replace(
        'class="mobishop-app-header-morphing-search"',
        'class="mobishop-app-header-morphing-search" data-start-top="' +
          startTop +
          '" data-start-width="' +
          startWidth +
          '" data-end-width="' +
  ...
  function footerItem(card, item, metrics, activeItem, itemColumnWidth) {
    ...
    if (item === "add_to_cart") {
      ...
      return (
        '<button class="mobishop-app-footer-add is-' +
        esc(buttonStyle) +
        '" style="width:' +
        buttonWidth +
        "%;height:" +
    ...
    return (
      '<span class="mobishop-app-footer-item mobishop-app-footer-item--' +
      item +
      " is-" +
      esc(style) +
      (active ? " is-active" : "") +
      ...
      ";--item-icon-size:" +
      metrics.iconSize +
      'px"><i class="mobishop-app-footer-icon-shell" style="background:' +
  ...
  function renderFooter(card, options) {
    ...
    var rows = placements.length
      ? '<div class="mobishop-app-footer-row mobishop-app-layout-row" style="grid-template-columns:' +
        placements
        ...
        placements
          .map(function (placement) {
            return (
              '<div class="mobishop-app-footer-column">' +
              footerItem(
                card,
                placement.item,
                metrics,
      ...
      : "";
    return (
      '<footer class="mobishop-app-footer mobishop-app-footer--composed" style="height:' +
      footerHeight +
      "px;margin:" +
      number(card, "margin_top", 0, 0, 80) +
      "px 0 " +
  ...
  function transferStorageKey(card) {
    return (
      "mobishop_" + (card.dataset.chromePart || "chrome") + "_settings"
    );
  }
  function transferStatus(card, message, error) {
    var status = card.querySelector(".mobishop-chrome-transfer-status");
    if (!status) {
      return;
    }
  ...
  function refreshIconChoices(card) {
    Array.prototype.forEach.call(
      card.querySelectorAll(".mobishop-chrome-icon-select"),
      function (select) {
        var choice = select.closest(".mobishop-chrome-icon-choice");
        if (!choice) {
          return;
        }
        Array.prototype.forEach.call(
          choice.querySelectorAll(".mobishop-chrome-icon-option"),
          function (option) {
  ...
  function applyTransferPayload(card, payload) {
    ...
    Array.prototype.forEach.call(
      card.querySelectorAll(".mobishop-chrome-composer"),
      function (composer) {
        composer.dispatchEvent(new Event("mobishop:chrome-layout-replaced"));
      },
    );
    return true;
  }
  function syncCollapsedVisibility(card) {
    if (
      ...
      (card.dataset.chromePart !== "header" &&
        !card.querySelector(".mobishop-collapsed-header-enabled"))
    ) {
      return;
    }
    var toggle = card.querySelector(".mobishop-collapsed-header-enabled"),
      visible = !!(toggle && toggle.checked);
    var composer = card.querySelector(".mobishop-chrome-composer--collapsed"),
      settings = card.querySelector(".mobishop-collapsed-header-settings");
    if (composer) {
  ...
  document.addEventListener("click", function (event) {
    var button =
      ...
      event.target.closest(".mobishop-fixed-chrome-expand");
    if (!button) {
      return;
    }
    var card = button.closest(".mobishop-fixed-chrome-card"),
      body = card && card.querySelector(":scope > .mobishop-page-card__body");
    if (!body) {
      return;
    }
  ...
  document.addEventListener("click", function (event) {
    var trigger =
      event.target.closest &&
      event.target.closest("#mobishop-collapse-all,#mobishop-expand-all");
    if (!trigger) {
      return;
    }
    var opening = trigger.id === "mobishop-expand-all";
    Array.prototype.forEach.call(
      document.querySelectorAll(".mobishop-fixed-chrome-card"),
      function (card) {
        var body = card.querySelector(":scope > .mobishop-page-card__body"),
          button = card.querySelector(".mobishop-fixed-chrome-expand");
        if (body) {
          body.hidden = !opening;
        }
        card.classList.toggle("is-open", opening);
  ...
  document.addEventListener("change", function (event) {
    if (
      ...
      !event.target.matches(
        ".mobishop-collapsed-header-enabled,[name$='[collapse_transition]']",
      )
    ) {
      return;
    }
    syncCollapsedVisibility(event.target.closest(".mobishop-fixed-chrome-card"));
  });
  document.addEventListener("click", function (event) {
    var button =
      event.target.closest &&
  ...
  document.addEventListener("click", function (event) {
    var trigger =
      event.target.closest &&
      event.target.closest(
        ".mobishop-fixed-chrome-card .mobishop-page-media-choose, .mobishop-fixed-chrome-card .mobishop-page-media-preview",
      );
    if (!trigger || !window.wp || !wp.media) {
      return;
    }
    var mediaField = trigger.closest(".mobishop-page-field--image"),
      card = trigger.closest(".mobishop-fixed-chrome-card"),
      input = mediaField && mediaField.querySelector(".mobishop-page-media-url"),
      preview =
        mediaField && mediaField.querySelector(".mobishop-page-media-preview"),
      clear = card && card.querySelector(".mobishop-page-media-clear");
  ...
  document.addEventListener("click", function (event) {
    var clear =
      event.target.closest &&
      event.target.closest(".mobishop-fixed-chrome-card .mobishop-page-media-clear");
    if (!clear) {
      return;
    }
    var card = clear.closest(".mobishop-fixed-chrome-card"),
      mediaField =
        card &&
        (card.querySelector('.mobishop-page-field[data-setting="logo_url"]') ||
          clear.closest(".mobishop-page-field--image")),
      input = mediaField && mediaField.querySelector(".mobishop-page-media-url"),
      preview =
        mediaField && mediaField.querySelector(".mobishop-page-media-preview"),
      textInput =
        card &&
        card.querySelector(
          '.mobishop-page-field[data-setting="logo_text"] input[type="text"]',
        );
    if (!input || !preview) {
      return;
    }
  ...
  document.addEventListener("input", function (event) {
    if (
      !event.target.matches ||
      !event.target.matches(".mobishop-fixed-chrome-card .mobishop-page-media-url")
    ) {
      return;
    }
    var fieldNode = event.target.closest(".mobishop-page-field--image"),
      card = event.target.closest(".mobishop-fixed-chrome-card"),
      preview =
        fieldNode && fieldNode.querySelector(".mobishop-page-media-preview"),
      clear = card && card.querySelector(".mobishop-page-media-clear"),
  ...
  function syncHeaderRowFields(card, layout, part, variant) {
    ...
    single.forEach(function (key) {
      var input = field(card, key),
        wrap = input && input.closest(".mobishop-page-field");
      if (wrap) {
        wrap.hidden = multi;
      }
    });
  ...
  function boot(composer) {
    if (composer.dataset.mobishopChromeBooted === "1") {
      return;
    }
    var part = composer.dataset.part,
      variant = composer.dataset.variant || "original",
      input = composer.querySelector(".mobishop-chrome-layout-json"),
      canvas = composer.querySelector(".mobishop-chrome-layout"),
      palette = composer.querySelector(".mobishop-chrome-palette__items"),
      reset = composer.querySelector(".mobishop-chrome-reset"),
      presets = composer.querySelector(".mobishop-header-presets"),
      card = composer.closest(".mobishop-fixed-chrome-card");
    if (!input || !canvas || !palette || !card) {
      return;
    }
    composer.dataset.mobishopChromeBooted = "1";
    ...
    function tile(item) {
      ...
      node.type = "button";
      node.draggable = true;
      node.className = "mobishop-chrome-item is-active";
      node.dataset.item = item;
      node.innerHTML =
        '<span class="dashicons dashicons-move"></span>' +
        (source ? source.textContent.trim() : item) +
        '<span class="mobishop-chrome-remove">×</span>';
      return node;
    }
    ...
    function render() {
      ...
      layout.rows.forEach(function (row, rowIndex) {
        ...
        wrap.className = "mobishop-chrome-row";
        var toolbar = document.createElement("div");
        toolbar.className = "mobishop-chrome-row-toolbar";
        toolbar.innerHTML =
          "<strong>Row " +
          (rowIndex + 1) +
          '</strong><label>Columns <select class="mobishop-row-column-count" data-row="' +
          rowIndex +
          '">' +
          [1, 2, 3, 4, 5, 6]
            .map(function (count) {
            ...
            .join("") +
          '</select></label><span class="mobishop-row-total">Total: ' +
          Math.round(
            row.columns.reduce(function (total, column) {
          ...
          (rowIndex === layout.rows.length - 1 && layout.rows.length < 3
            ? '<button type="button" class="button mobishop-chrome-add-row">+ Add row</button>'
            : "") +
          (layout.rows.length > 1
            ? '<button type="button" class="mobishop-chrome-remove-row" data-row="' +
              rowIndex +
              '">×</button>'
            : "");
        wrap.appendChild(toolbar);
        var grid = document.createElement("div");
        grid.className = "mobishop-chrome-row-grid";
        grid.style.gridTemplateColumns = columnsTemplate(row);
        row.columns.forEach(function (column, columnIndex) {
          var cell = document.createElement("div");
          cell.className = "mobishop-chrome-column";
          cell.innerHTML =
            '<label>Width % <input class="mobishop-column-width" type="number" min="1" max="100" step="0.01" data-row="' +
            rowIndex +
            '" data-column="' +
            columnIndex +
            '" value="' +
          ...
          zone.className = "mobishop-chrome-zone";
      ...
      Array.prototype.forEach.call(
        card.querySelectorAll(".mobishop-chrome-composer"),
        function (other) {
          var otherInput = other.querySelector(".mobishop-chrome-layout-json");
      ...
      Array.prototype.forEach.call(
        palette.querySelectorAll(".mobishop-chrome-item"),
        function (item) {
    ...
    function fillAutomaticWidth(target) {
      if (
        !target ||
        !target.matches(".mobishop-column-width") ||
        String(target.value).trim() !== ""
      ) {
        return;
      }
      var rowNode = target.closest(".mobishop-chrome-row"),
        total = 0;
      if (!rowNode) {
        return;
      }
      Array.prototype.forEach.call(
        rowNode.querySelectorAll(".mobishop-column-width"),
        function (control) {
          if (control !== target) {
    ...
    composer.addEventListener("dragstart", function (event) {
      var item = event.target.closest(".mobishop-chrome-item");
      if (!item) {
        return;
      }
    ...
    composer.addEventListener("dragover", function (event) {
      if (event.target.closest(".mobishop-chrome-zone,.mobishop-chrome-palette")) {
        event.preventDefault();
      }
    });
    composer.addEventListener("drop", function (event) {
      var paletteTarget = event.target.closest(".mobishop-chrome-palette");
      if (paletteTarget) {
        event.preventDefault();
      ...
      var target = event.target.closest(".mobishop-chrome-zone");
      if (!target) {
        return;
      }
      event.preventDefault();
      var before = event.target.closest(".mobishop-chrome-item");
      insert(
    ...
    composer.addEventListener("change", function (event) {
      if (event.target.matches(".mobishop-row-column-count")) {
    ...
    composer.addEventListener("click", function (event) {
      var remove = event.target.closest(".mobishop-chrome-remove");
      if (remove) {
        removeItem(remove.closest(".mobishop-chrome-item").dataset.item);
        save();
        render();
        return;
      }
      var paletteItem = event.target.closest(
        ".mobishop-chrome-palette .mobishop-chrome-item",
      );
      if (paletteItem) {
        var target = canvas.querySelector(".mobishop-chrome-zone");
        if (target) {
          insert(paletteItem.dataset.item, target, null);
        }
        return;
      }
      var add = event.target.closest(".mobishop-chrome-add-row");
      if (add) {
      ...
      var removeRow = event.target.closest(".mobishop-chrome-remove-row");
      if (removeRow) {
    ...
    card.addEventListener("click", function (event) {
      var option = event.target.closest(".mobishop-chrome-icon-option");
      if (!option) {
        return;
      }
      var choice = option.closest(
        ".mobishop-chrome-icon-choice,.mobishop-chrome-footer-icon-row",
      );
      if (!choice) {
        return;
      }
      var select = choice.querySelector(".mobishop-chrome-icon-select");
      if (!select) {
        return;
      }
    ...
    composer.addEventListener("mobishop:chrome-layout-replaced", function () {
      ...
      form.addEventListener(
        ...
        function (event) {
          if (!valid()) {
            ...
            Array.prototype.forEach.call(
              canvas.querySelectorAll(".mobishop-chrome-row"),
              function (node, index) {
  ...
  function bootChromeComposers() {
    ...
    Array.prototype.forEach.call(
      document.querySelectorAll(".mobishop-chrome-composer"),
      boot,
    );
  }
