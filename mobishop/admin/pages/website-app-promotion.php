...
foreach ( array_keys( $campaigns ) as $campaign_key ) {
	$campaign_statuses[ $campaign_key ] = MobiShop_Website_App_Promotion::campaign_status( $campaign_key, $promotion_settings );
...
$click_rate = $promotion_metrics['views'] > 0
	...
	: 0;
$preview_url  = MobiShop_Website_App_Promotion::preview_url();
$preview_host = (string) wp_parse_url( home_url( '/' ), PHP_URL_HOST );
?>
<div class="wrap mobishop-app-promotion-admin" data-promotion-admin>
	<header class="mobishop-app-promotion-hero">
		<div class="mobishop-app-promotion-hero__copy">
		...
		<span class="mobishop-app-promotion-state <?php echo ! empty( $promotion_settings['enabled'] ) ? 'is-live' : ''; ?>">
	...
	<?php if ( isset( $_GET['promotion_saved'] ) ) : ?>
		<div class="mobishop-app-promotion-notice"><span class="dashicons dashicons-yes-alt"></span><?php esc_html_e( 'Website app promotion settings saved.', 'mobishop' ); ?></div>
	<?php endif; ?>

	<section class="mobishop-app-promotion-stats">
	...
	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-promotion-form>
		<input type="hidden" name="action" value="mobishop_save_website_app_promotion">
		<?php wp_nonce_field( 'mobishop_save_website_app_promotion', 'mobishop_promotion_nonce' ); ?>

		<section class="mobishop-app-promotion-master">
			<div>
			...
			<label class="mobishop-promotion-switch"><input type="checkbox" name="promotion[enabled]" value="1" <?php checked( ! empty( $promotion_settings['enabled'] ) ); ?> data-promotion-master><span></span><b data-promotion-master-label><?php echo ! empty( $promotion_settings['enabled'] ) ? esc_html__( 'On', 'mobishop' ) : esc_html__( 'Off', 'mobishop' ); ?></b></label>
		</section>

		<section class="mobishop-app-promotion-choose">
			<div class="mobishop-app-promotion-heading">
			...
			<div class="mobishop-app-promotion-types">
				<?php $first = true; foreach ( $campaigns as $key => $meta ) : $saved_campaign = $campaign( $key ); $campaign_status = $campaign_statuses[ $key ]; ?>
					<article class="<?php echo esc_attr( trim( ( $first ? 'is-selected ' : '' ) . ( in_array( $campaign_status, array( 'live', 'announcement' ), true ) ? 'is-enabled' : '' ) ) ); ?>" data-promotion-type="<?php echo esc_attr( $key ); ?>" data-promotion-status="<?php echo esc_attr( $campaign_status ); ?>" tabindex="0">
						<div class="mobishop-app-promotion-type__top">
							<span class="dashicons <?php echo esc_attr( $meta[1] ); ?>"></span>
							<label class="mobishop-promotion-switch"><input type="checkbox" name="promotion[<?php echo esc_attr( $key ); ?>][enabled]" value="1" <?php checked( ! empty( $saved_campaign['enabled'] ) ); ?>><span></span></label>
						</div>
						<strong><?php echo esc_html( $meta[0] ); ?></strong>
						<p><?php echo esc_html( $meta[2] ); ?></p>
						<div class="mobishop-app-promotion-type__meta"><small><?php echo esc_html( $meta[3] ); ?></small><span class="mobishop-app-promotion-type__status is-<?php echo esc_attr( $campaign_status ); ?>" data-promotion-status><?php echo esc_html( $status_labels[ $campaign_status ] ); ?></span></div>
						<div class="mobishop-app-promotion-type__actions"><button type="button" data-edit-campaign="<?php echo esc_attr( $key ); ?>"><?php esc_html_e( 'Edit & preview', 'mobishop' ); ?></button><a href="<?php echo esc_url( MobiShop_Website_App_Promotion::test_url( $key ) ); ?>" target="_blank" rel="noopener" data-test-campaign="<?php echo esc_attr( $key ); ?>"><?php esc_html_e( 'Test on live site', 'mobishop' ); ?></a></div>
					</article>
				<?php $first = false; endforeach; ?>
			</div>
		</section>

		<div class="mobishop-app-promotion-workspace">
			<main>
				<section class="mobishop-app-promotion-panel">
					<div class="mobishop-app-promotion-heading">
					...
					<div class="mobishop-promotion-fields is-three">
						...
						<label class="mobishop-promotion-media"><span><?php esc_html_e( 'App icon URL', 'mobishop' ); ?></span><div><input type="url" name="promotion[logo_url]" value="<?php echo esc_attr( $value( 'logo_url' ) ); ?>" data-preview-field="logo_url"><button type="button" class="button" data-promotion-media><?php esc_html_e( 'Choose', 'mobishop' ); ?></button></div></label>
						...
						<label><span><?php esc_html_e( 'App deep link (optional)', 'mobishop' ); ?></span><input type="text" name="promotion[deep_link]" value="<?php echo esc_attr( $value( 'deep_link' ) ); ?>" placeholder="mobishop://home"></label>
					...
					<div class="mobishop-promotion-tip"><span class="dashicons dashicons-lightbulb"></span><p><strong><?php esc_html_e( 'Best setup:', 'mobishop' ); ?></strong> <?php esc_html_e( 'Use one smart link if available. Otherwise Android, iOS and Huawei visitors are routed to their matching store automatically.', 'mobishop' ); ?></p></div>
					<?php if ( ! $has_destination ) : ?>
						<div class="mobishop-promotion-warning is-link-status"><span class="dashicons dashicons-info-outline"></span><p><strong><?php esc_html_e( 'Safe announcement mode is active.', 'mobishop' ); ?></strong> <?php esc_html_e( 'Every enabled campaign stays customer-facing without a dead action. Until an app link is added, QR uses an honest placeholder and the floating shortcut remains visibly disabled.', 'mobishop' ); ?></p></div>
					<?php else : ?>
						<div class="mobishop-promotion-ready"><span class="dashicons dashicons-yes-alt"></span><p><?php esc_html_e( 'App destination connected. Every enabled format can open or download the app.', 'mobishop' ); ?></p></div>
					<?php endif; ?>
				</section>
				...
				<section class="mobishop-app-promotion-panel">
					...
					<div class="mobishop-promotion-fields is-three">
						...
						<div class="mobishop-promotion-colors">
				...
				<section class="mobishop-app-promotion-panel">
					...
					<div class="mobishop-promotion-campaign-settings is-active" data-promotion-campaign-panel="smart_banner">
					...
					<div class="mobishop-promotion-campaign-settings" data-promotion-campaign-panel="bottom_sheet" hidden>
					...
					<div class="mobishop-promotion-campaign-settings" data-promotion-campaign-panel="popup" hidden>
						...
						<div class="mobishop-promotion-warning"><span class="dashicons dashicons-shield"></span><p><?php esc_html_e( 'To protect mobile UX and search visibility, exit intent automatically falls back to a delayed bottom-style card on touch devices.', 'mobishop' ); ?></p></div>
					</div>

					<?php $qr = $campaign( 'desktop_qr' ); ?>
					<div class="mobishop-promotion-campaign-settings" data-promotion-campaign-panel="desktop_qr" hidden>
					...
					<div class="mobishop-promotion-campaign-settings" data-promotion-campaign-panel="floating_button" hidden>
					...
					<div class="mobishop-promotion-campaign-settings" data-promotion-campaign-panel="inline_banner" hidden>
						...
						<div class="mobishop-promotion-fields is-three">
							...
							<div class="mobishop-promotion-shortcode"><span><?php esc_html_e( 'Shortcode', 'mobishop' ); ?></span><code>[mobishop_app_promo]</code><button type="button" class="button" data-copy-shortcode><?php esc_html_e( 'Copy', 'mobishop' ); ?></button></div>
						</div>
					</div>
				</section>
				...
				<details class="mobishop-app-promotion-native">
					...
					<div class="mobishop-promotion-fields is-three">
						<label class="mobishop-promotion-check"><input type="checkbox" name="promotion[native_ios_enabled]" value="1" <?php checked( ! empty( $promotion_settings['native_ios_enabled'] ) ); ?>><span><?php esc_html_e( 'Enable the native iOS banner', 'mobishop' ); ?></span></label>
				...
				<div class="mobishop-app-promotion-save">
			...
			<aside class="mobishop-app-promotion-preview">
				<header><div><strong><?php esc_html_e( 'Live preview', 'mobishop' ); ?></strong><small data-preview-campaign-label><?php esc_html_e( 'Smart Banner', 'mobishop' ); ?></small></div><div class="mobishop-promotion-device-switch" role="group" aria-label="<?php esc_attr_e( 'Preview device', 'mobishop' ); ?>"><button type="button" data-preview-device="mobile" aria-label="<?php esc_attr_e( 'Mobile preview', 'mobishop' ); ?>" aria-pressed="false"><span class="dashicons dashicons-smartphone" aria-hidden="true"></span></button><button type="button" class="is-active" data-preview-device="desktop" aria-label="<?php esc_attr_e( 'Laptop preview', 'mobishop' ); ?>" aria-pressed="true"><span class="dashicons dashicons-desktop" aria-hidden="true"></span></button></div></header>
				<div class="mobishop-promotion-device is-desktop" data-promotion-preview>
					<div class="mobishop-promotion-screen" data-promotion-screen>
						<div class="mobishop-promotion-browser" data-promotion-browser>
							<div class="mobishop-promotion-browser__bar"><i></i><i></i><i></i><span><?php echo esc_html( $preview_host ); ?></span></div>
							<div class="mobishop-promotion-live-site">
								<iframe
							...
							<div class="mobishop-promotion-preview-output" data-preview-output></div>
						</div>
					</div>
				</div>
				<p class="mobishop-app-promotion-preview__hint"><span class="dashicons dashicons-info-outline"></span><?php esc_html_e( 'Preview updates while you type. The live website also applies device, page and frequency rules.', 'mobishop' ); ?></p>
			</aside>
		</div>
	</form>
</div>
