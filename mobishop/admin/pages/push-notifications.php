...
?>
<div class="wrap mobishop-push-page">
	<header class="mobishop-push-hero">
		...
		<span class="mobishop-push-status <?php echo $push_connected ? 'is-connected' : ''; ?>"><?php echo esc_html( (string) $push_status['label'] ); ?></span>
	</header>

	<?php if ( isset( $_GET['push_sent'] ) ) : ?><div class="mobishop-toast is-visible"><span class="dashicons dashicons-yes-alt"></span><?php esc_html_e( 'Notification saved successfully.', 'mobishop' ); ?></div><?php elseif ( isset( $_GET['push_error'] ) ) : ?><div class="notice notice-error"><p><?php echo esc_html( 'delivery' === sanitize_key( wp_unslash( $_GET['push_error'] ) ) ? __( 'The notification could not be delivered. Prepare Firebase and register a test device, then try again.', 'mobishop' ) : __( 'Check the required fields and scheduled time.', 'mobishop' ) ); ?></p></div><?php endif; ?>
	...
	<div class="mobishop-push-managed">
		...
		<div class="mobishop-push-managed__actions">
			<?php if ( $push_connected ) : ?>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="mobishop_test_push_connection"><?php wp_nonce_field( 'mobishop_test_push_connection', 'mobishop_push_test_nonce' ); ?><button class="button" type="submit"><span class="dashicons dashicons-yes-alt"></span><?php esc_html_e( 'Test connection', 'mobishop' ); ?></button></form>
			<?php elseif ( ! empty( $push_status['license_active'] ) ) : ?>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="mobishop_provision_push"><?php wp_nonce_field( 'mobishop_provision_push', 'mobishop_push_setup_nonce' ); ?><button class="button button-primary" type="submit"><span class="dashicons dashicons-cloud-saved"></span><?php esc_html_e( 'Prepare Firebase', 'mobishop' ); ?></button></form>
			<?php endif; ?>
		</div>
	</div>

	<div class="mobishop-push-readiness" aria-label="<?php esc_attr_e( 'Firebase readiness', 'mobishop' ); ?>">
	...
	<div class="mobishop-push-stats is-four">
	...
	<form class="mobishop-push-builder" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
		<input type="hidden" name="action" value="mobishop_send_push_notification"><?php wp_nonce_field( 'mobishop_send_push_notification', 'mobishop_push_nonce' ); ?>
		<section class="mobishop-push-type-step">
			<div class="mobishop-push-step-title"><b>1</b><div><h2><?php esc_html_e( 'Choose a journey', 'mobishop' ); ?></h2><p><?php esc_html_e( 'Start from a ready event, then control the message, destination and delivery rules.', 'mobishop' ); ?></p></div></div>
			<div class="mobishop-push-types"><?php foreach ( $push_types as $key => $type ) : ?><label><input type="radio" name="push_type" value="<?php echo esc_attr( $key ); ?>" <?php checked( $selected_push_type, $key ); ?> data-push-type><span class="dashicons <?php echo esc_attr( $type[1] ); ?>"></span><strong><?php echo esc_html( $type[0] ); ?></strong><small><?php echo esc_html( $type[2] ); ?></small></label><?php endforeach; ?></div>
		</section>

		<div class="mobishop-push-workspace">
			<section class="mobishop-push-compose">
				<div class="mobishop-push-step-title"><b>2</b><div><h2><?php esc_html_e( 'Message & action', 'mobishop' ); ?></h2><p><?php esc_html_e( 'The recommended destination is selected automatically, but you can change it.', 'mobishop' ); ?></p></div></div>
				<div class="mobishop-push-form-stack">
					<section class="mobishop-push-form-section"><header><span class="dashicons dashicons-format-chat"></span><div><h3><?php esc_html_e( 'Message', 'mobishop' ); ?></h3><p><?php esc_html_e( 'Keep it short, clear and actionable.', 'mobishop' ); ?></p></div></header><div class="mobishop-push-fields">
					...
					<section class="mobishop-push-form-section"><header><span class="dashicons dashicons-admin-links"></span><div><h3><?php esc_html_e( 'Open destination', 'mobishop' ); ?></h3><p><?php esc_html_e( 'Choose where the customer lands after tapping the notification.', 'mobishop' ); ?></p></div></header><div class="mobishop-push-fields">
					...
					<section class="mobishop-push-form-section"><header><span class="dashicons dashicons-controls-repeat"></span><div><h3><?php esc_html_e( 'Audience & delivery', 'mobishop' ); ?></h3><p><?php esc_html_e( 'Send once, schedule, or save a guarded automation.', 'mobishop' ); ?></p></div></header><div class="mobishop-push-fields">
						...
						<div class="mobishop-push-automation is-wide" data-push-automation hidden>
							...
							<label class="mobishop-push-check"><input type="checkbox" name="push_stop_on_purchase" value="1" checked><span><?php esc_html_e( 'Stop immediately after purchase', 'mobishop' ); ?></span></label>
						</div>
					</div></section>
				</div>

				<details class="mobishop-push-advanced"><summary><?php esc_html_e( 'Advanced options', 'mobishop' ); ?></summary><div class="mobishop-push-fields"><label class="is-wide"><span><?php esc_html_e( 'Image URL', 'mobishop' ); ?></span><input type="url" name="push_image_url" placeholder="https://"></label><label><span><?php esc_html_e( 'Priority', 'mobishop' ); ?></span><select name="push_priority"><option value="normal">Normal</option><option value="high">High</option></select></label><label><span><?php esc_html_e( 'Expires after (hours)', 'mobishop' ); ?></span><input type="number" name="push_expiry_hours" min="1" max="168" value="24"></label><label><span><?php esc_html_e( 'App badge number', 'mobishop' ); ?></span><input type="number" name="push_badge" min="0"></label><label class="mobishop-push-check"><input type="checkbox" name="push_sound" value="1" checked><span><?php esc_html_e( 'Play notification sound', 'mobishop' ); ?></span></label></div></details>
				<div class="mobishop-push-submit"><button class="button button-primary" type="submit" data-push-submit><span class="dashicons dashicons-paper-plane"></span><span data-push-submit-label><?php esc_html_e( 'Send notification', 'mobishop' ); ?></span></button></div>
			</section>

			<aside class="mobishop-push-preview"><small><?php esc_html_e( 'Live preview', 'mobishop' ); ?></small><div class="mobishop-push-phone"><div class="mobishop-push-clock">9:41</div><div class="mobishop-push-bubble"><span>W</span><div><b data-push-preview-title><?php esc_html_e( 'Your notification title', 'mobishop' ); ?></b><p data-push-preview-message><?php esc_html_e( 'Your message will appear here.', 'mobishop' ); ?></p></div><small><?php esc_html_e( 'now', 'mobishop' ); ?></small></div></div></aside>
		</div>
	</form>

	<section class="mobishop-push-history"><h2><?php esc_html_e( 'History & automations', 'mobishop' ); ?></h2><?php if ( $history ) : ?><div><?php foreach ( $history as $item ) : ?><article><span class="dashicons dashicons-megaphone"></span><div><strong><?php echo esc_html( (string) ( $item['title'] ?? '' ) ); ?></strong><p><?php echo esc_html( (string) ( $item['message'] ?? '' ) ); ?></p></div><small><?php echo esc_html( ucfirst( str_replace( '_', ' ', (string) ( $item['type'] ?? 'broadcast' ) ) ) ); ?> · <?php echo esc_html( wp_date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), absint( $item['created_at'] ?? time() ) ) ); ?></small><em><?php echo esc_html( (string) ( $item['status'] ?? 'saved' ) ); ?></em></article><?php endforeach; ?></div><?php else : ?><div class="mobishop-data-empty"><span class="dashicons dashicons-megaphone"></span><strong><?php esc_html_e( 'No notifications yet', 'mobishop' ); ?></strong></div><?php endif; ?></section>
</div>
