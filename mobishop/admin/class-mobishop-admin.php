...
require_once MOBISHOP_PATH . 'admin/class-api-monitor.php';

final class MobiShop_Admin {
	...
	private const REPORTING_READY_OPTION = 'mobishop_store_reporting_ready_v1';

	/** @var array<string,string> */
	private const PAGE_BUILDER_SLUGS = array(
		'mobishop-catalog-builder'  => 'catalog',
		'mobishop-product-builder'  => 'product',
		'mobishop-size-chart-builder' => 'size_chart',
		'mobishop-wishlist-builder' => 'wishlist',
		'mobishop-account-builder'  => 'account',
	);

	/** @var array<string,string> Public routes inside the single CMS screen. */
	private const CMS_VIEWS = array(
		'overview'=>'mobishop','setup'=>'mobishop-setup','pages'=>'mobishop-splash-screen','splash'=>'mobishop-splash-screen','home'=>'mobishop-home-builder','category'=>'mobishop-category-builder','catalog'=>'mobishop-catalog-builder','product'=>'mobishop-product-builder','wishlist'=>'mobishop-wishlist-builder','account'=>'mobishop-account-builder','checkout'=>'mobishop-checkout-suggestions','saved-themes'=>'mobishop-saved-themes','store-data'=>'mobishop-store-data','ai-insights'=>'mobishop-ai-insights','push'=>'mobishop-push-notifications','website-promotion'=>'mobishop-website-app-promotion',
	);
	...
	private const EDITOR_PAGES = array(
		'app_header'       => 'mobishop-app-headers',
		'hero_slider'     => 'mobishop-hero-sliders',
		'image_banner'    => 'mobishop-image-banners',
		'product_carousel' => 'mobishop-product-carousels',
		'brand_carousel'  => 'mobishop-brand-carousels',
		'category_grid'   => 'mobishop-category-grids',
		'product_grid'    => 'mobishop-product-grids',
		'section_header'  => 'mobishop-section-headers',
		'promo_strip'     => 'mobishop-promo-strips',
		'coupon_banner'   => 'mobishop-coupon-banners',
		'countdown'       => 'mobishop-countdowns',
		'video_banner'    => 'mobishop-video-banners',
		'text_block'      => 'mobishop-text-blocks',
		'divider'         => 'mobishop-dividers',
		'spacer'          => 'mobishop-spacers',
	);
	...
	public function register(): void {
		...
		add_action(
			'admin_post_mobishop_save_home_builder',
			array(
				$this,
				'save_home_builder',
			)
		...
		add_action(
			'admin_post_mobishop_save_category_builder',
			array( $this, 'save_category_builder' )
		);

		add_action(
			'admin_post_mobishop_save_page_builder',
			array( $this, 'save_page_builder' )
		);

		add_action( 'admin_post_mobishop_save_splash_screen', array( $this, 'save_splash_screen' ) );
		add_action( 'admin_post_mobishop_save_similar_products', array( $this, 'save_similar_products' ) );
		add_action( 'admin_post_mobishop_save_checkout_suggestions', array( $this, 'save_checkout_suggestions' ) );
		add_action( 'admin_post_mobishop_apply_setup_wizard', array( $this, 'apply_setup_wizard' ) );
		add_action( 'admin_post_mobishop_manage_saved_theme', array( $this, 'manage_saved_theme' ) );
			add_action( 'admin_post_mobishop_send_push_notification', array( $this, 'send_push_notification' ) );
			add_action( 'admin_post_mobishop_provision_push', array( $this, 'provision_push' ) );
			add_action( 'admin_post_mobishop_test_push_connection', array( $this, 'test_push_connection' ) );
			add_action( 'admin_post_mobishop_build_ai_action', array( $this, 'build_ai_action' ) );
			add_action( 'admin_post_mobishop_review_ai_result', array( $this, 'review_ai_result' ) );
		add_action( 'admin_post_mobishop_toggle_product_channel', array( $this, 'toggle_product_channel' ) );
		add_action( 'admin_post_mobishop_set_coupon_channel', array( $this, 'set_coupon_channel' ) );
		add_action( 'admin_post_mobishop_start_abandoned_cart_import', array( $this, 'start_abandoned_cart_import' ) );
		add_action( 'mobishop_dispatch_scheduled_push', array( $this, 'dispatch_scheduled_push' ) );
		add_action( 'admin_post_mobishop_activate_license', array( $this, 'activate_license' ) );
		add_action( 'admin_post_mobishop_verify_license', array( $this, 'verify_license' ) );
		add_action( 'wp_ajax_mobishop_apply_product_icon_settings', array( $this, 'apply_product_icon_settings' ) );
		add_action( 'wp_ajax_mobishop_start_ai_analysis', array( $this, 'start_ai_analysis' ) );
		add_action( 'wp_ajax_mobishop_step_ai_analysis', array( $this, 'step_ai_analysis' ) );
		add_action( 'wp_ajax_mobishop_background_ai_analysis', array( $this, 'background_ai_analysis' ) );
		add_action( 'wp_ajax_mobishop_cancel_ai_analysis', array( $this, 'cancel_ai_analysis' ) );
		add_action( 'wp_ajax_mobishop_ai_analysis_status', array( $this, 'ai_analysis_status' ) );
		add_action( 'wp_ajax_mobishop_dismiss_ai_analysis', array( $this, 'dismiss_ai_analysis' ) );
		add_action( 'wp_ajax_mobishop_generate_store_reporting', array( $this, 'generate_store_reporting' ) );
		add_action( 'wp_ajax_mobishop_abandoned_cart_details', array( $this, 'abandoned_cart_details' ) );
		add_action( 'wp_ajax_mobishop_view', array( $this, 'cms_view_fragment' ) );
	 ...
	 * guard remains the authority for every MobiShop mutation.
	 */
	public function enforce_license_gate(): void {
		...
		$license_active = ( new MobiShop_License_Manager() )->is_active();
		$action         = isset( $_REQUEST['action'] ) ? sanitize_key( wp_unslash( $_REQUEST['action'] ) ) : '';
		$allowed        = array(
			'mobishop_activate_license',
			'mobishop_verify_license',
			'mobishop_view',
		);

		if (
			! $license_active
			&& 0 === strpos( $action, 'mobishop_' )
	...
	public function admin_body_class( string $classes ): string {
		...
		if ( $this->is_builder_screen( $page ) ) {
			$classes .= ' mobishop-cms-builder-screen';
		}
		if ( $this->is_public_cms_page( $page ) ) {
			$classes .= ' mobishop-cms-plugin-page';
		}

		if (
			'mobishop' !== $page
			&& $this->is_public_cms_page( $page )
			&& ! ( new MobiShop_License_Manager() )->is_active()
		) {
			$classes .= ' mobishop-cms-license-preview';
		}
	...
	private function is_builder_screen( string $page ): bool {
		$builder_pages = array_merge(
			array(
				'mobishop-home-builder',
				'mobishop-category-builder',
				'mobishop-splash-screen',
			),
	...
	public function apply_product_icon_settings(): void {
		...
		check_ajax_referer( 'mobishop_apply_product_icon_settings', 'nonce' );
		...
		$changed = 0;
		$home_store = new MobiShop_Layout_Store();
		...
		$page_store = new MobiShop_Page_Layout_Store();
		foreach ( array_keys( MobiShop_Page_Layout_Store::pages() ) as $page ) {
		...
		update_option( 'mobishop_global_' . $scope . '_profile', $profile, false );
		...
		foreach ( self::PAGE_BUILDER_SLUGS as $slug => $page ) {
			$labels = MobiShop_Page_Layout_Store::pages();
			$label = $labels[ $page ];
			add_submenu_page(
				null,
		...
		add_submenu_page( null, __( 'Splash Screen', 'mobishop' ), __( 'Splash Screen', 'mobishop' ), self::CAPABILITY, 'mobishop-splash-screen', array( $this, 'splash_screen_page' ) );
		add_submenu_page( null, __( 'Similar Products', 'mobishop' ), __( 'Similar Products', 'mobishop' ), self::CAPABILITY, 'mobishop-similar-products', array( $this, 'similar_products_page' ) );
		add_submenu_page( null, __( 'Checkout Suggestions', 'mobishop' ), __( 'Checkout Suggestions', 'mobishop' ), self::CAPABILITY, 'mobishop-checkout-suggestions', array( $this, 'checkout_suggestions_page' ) );
		add_submenu_page( null, __( 'Setup & Themes', 'mobishop' ), __( 'Setup & Themes', 'mobishop' ), self::CAPABILITY, 'mobishop-setup', array( $this, 'setup_wizard_page' ) );
		add_submenu_page( null, __( 'Saved Themes', 'mobishop' ), __( 'Saved Themes', 'mobishop' ), self::CAPABILITY, 'mobishop-saved-themes', array( $this, 'saved_themes_page' ) );
		add_submenu_page( null, __( 'Store Data', 'mobishop' ), __( 'Store Data', 'mobishop' ), self::CAPABILITY, 'mobishop-store-data', array( $this, 'store_data_page' ) );
		add_submenu_page( null, __( 'AI Offer Studio', 'mobishop' ), __( 'AI Offer Studio', 'mobishop' ), self::CAPABILITY, 'mobishop-ai-insights', array( $this, 'ai_insights_page' ) );
	...
	public function website_app_promotion_page(): void {
		...
		$promotion_settings = MobiShop_Website_App_Promotion::settings();
		$promotion_metrics  = MobiShop_Website_App_Promotion::metrics();
		require MOBISHOP_PATH . 'admin/pages/website-app-promotion.php';
	}
	...
	public function setup_wizard_page(): void {
		...
		$wizard       = new MobiShop_Setup_Wizard();
		$identity     = $wizard->identity();
		$themes       = MobiShop_Setup_Wizard::themes();
		$setup_pages  = MobiShop_Setup_Wizard::setup_pages();
		$push_export_config = MobiShop_Push_Service::client_configuration();
		...
		require MOBISHOP_PATH . 'admin/pages/setup-wizard.php';
	}
	...
	public function saved_themes_page(): void {
		...
		require MOBISHOP_PATH . 'admin/pages/saved-themes.php';
	}
	...
	public function store_data_page(): void {
		...
		if ( 'products' === $store_tab && function_exists( 'wc_get_product' ) ) {
			...
			if ( 'hidden_mobile' === $product_visibility ) {
				$product_query_args['meta_query'] = array( array( 'key' => MobiShop_Product_Channel_Visibility::MOBILE_META, 'value' => 'yes' ) );
			} elseif ( 'hidden_website' === $product_visibility ) {
				$product_query_args['meta_query'] = array( array( 'key' => MobiShop_Product_Channel_Visibility::WEBSITE_META, 'value' => 'yes' ) );
			} elseif ( 'hidden_both' === $product_visibility ) {
				$product_query_args['meta_query'] = array(
					'relation' => 'AND',
					array( 'key' => MobiShop_Product_Channel_Visibility::MOBILE_META, 'value' => 'yes' ),
					array( 'key' => MobiShop_Product_Channel_Visibility::WEBSITE_META, 'value' => 'yes' ),
				);
				...
				$product_query_args['meta_query'] = array(
					...
					array(
						'relation' => 'OR',
						array( 'key' => MobiShop_Product_Channel_Visibility::MOBILE_META, 'compare' => 'NOT EXISTS' ),
						array( 'key' => MobiShop_Product_Channel_Visibility::MOBILE_META, 'value' => 'yes', 'compare' => '!=' ),
					),
					array(
						'relation' => 'OR',
						array( 'key' => MobiShop_Product_Channel_Visibility::WEBSITE_META, 'compare' => 'NOT EXISTS' ),
						array( 'key' => MobiShop_Product_Channel_Visibility::WEBSITE_META, 'value' => 'yes', 'compare' => '!=' ),
					),
				);
			}
		...
		if ( 'discounts' === $store_tab ) {
			...
			if ( 'all' === $coupon_channel ) {
				$meta_query[] = array(
					'relation' => 'OR',
					array( 'key' => MobiShop_Coupon_Channel::META_KEY, 'compare' => 'NOT EXISTS' ),
					array( 'key' => MobiShop_Coupon_Channel::META_KEY, 'value' => 'all' ),
				);
			} elseif ( in_array( $coupon_channel, array( 'website', 'mobile' ), true ) ) {
				$meta_query[] = array( 'key' => MobiShop_Coupon_Channel::META_KEY, 'value' => $coupon_channel );
			}
		...
		if ( 'mobile' === $store_source ) {
			$customer_args['meta_query'] = array(
				'relation' => 'OR',
				array( 'key' => '_mobishop_customer', 'value' => '1' ),
				array( 'key' => '_mobishop_customer_sessions_v1', 'compare' => 'EXISTS' ),
			);
		}
		...
		if ( 'website' === $store_source ) {
			$customers = array_values(
				array_filter(
					$customers,
					static fn( WP_User $customer ): bool => MobiShop_Analytics::customer_sources( $customer->ID )['website']
				)
			);
		}
		...
		$reporting_snapshot = MobiShop_Analytics::empty_commerce_snapshot();
		...
		if ( $store_reporting_ready && in_array( $store_tab, array( 'reports', 'analytics' ), true ) ) {
			$reporting_snapshot = MobiShop_Analytics::reporting_snapshot( $date_from, $date_to, $store_source );
		}
		...
		$analytics = MobiShop_Analytics::empty_summary();
		$analytics_previous = MobiShop_Analytics::empty_summary();
		if ( 'analytics' === $store_tab && $store_reporting_ready ) {
			$previous_reporting = MobiShop_Analytics::reporting_snapshot( $previous_from, $previous_to, $store_source );
			$analytics = MobiShop_Analytics::summary( $date_from, $date_to, $store_source, true, $reporting_snapshot );
			$analytics_previous = MobiShop_Analytics::summary( $previous_from, $previous_to, $store_source, true, $previous_reporting );
		}
		...
		$abandoned_summary = 'abandoned-carts' === $store_tab
			? MobiShop_Analytics::abandoned_summary( $date_from, $date_to, $store_source )
			: array();
		$abandoned_import_state = 'abandoned-carts' === $store_tab
			? MobiShop_Analytics::website_session_import_status()
			: array();
		...
		$abandoned_carts = 'abandoned-carts' === $store_tab
			? MobiShop_Analytics::abandoned_carts(
				$date_from,
				$date_to,
				$store_source,
				$cart_per_page,
			...
			: array();
		$recovery_stats = 'abandoned-carts' === $store_tab ? MobiShop_Recovery_Campaigns::stats() : array();
		$recovery_campaigns = 'abandoned-carts' === $store_tab ? MobiShop_Recovery_Campaigns::recent( 50 ) : array();
		...
		$counts = array(
			...
			'orders'           => function_exists( 'wc_orders_count' ) ? MobiShop_Analytics::total_order_count() : count( $orders ),
			'abandoned-carts'  => MobiShop_Analytics::abandoned_count(),
		);
		require MOBISHOP_PATH . 'admin/pages/store-data.php';
	}
	...
	public function start_abandoned_cart_import(): void {
		...
		check_admin_referer( 'mobishop_start_abandoned_cart_import', 'mobishop_cart_import_nonce' );
		...
		( new MobiShop_Analytics() )->ensure_website_session_import( true, 'full' === $mode );
	...
	public function abandoned_cart_details(): void {
		check_ajax_referer( 'mobishop_abandoned_cart_details', 'nonce' );
		if ( ! current_user_can( self::CAPABILITY ) ) {
		...
		$insight = $cart_id ? MobiShop_Analytics::abandoned_cart_order_insight( $cart_id ) : array();
		if ( empty( $insight ) ) {
	...
	public function toggle_product_channel(): void {
		...
		check_admin_referer( 'mobishop_toggle_product_channel' );
		...
		$meta_key = 'mobile' === $channel
			? MobiShop_Product_Channel_Visibility::MOBILE_META
			: MobiShop_Product_Channel_Visibility::WEBSITE_META;
		if ( $hidden ) {
		...
		$redirect = $redirect ? $redirect : add_query_arg(
			array( 'page' => 'mobishop-store-data', 'store_tab' => 'products' ),
			admin_url( 'admin.php' )
		);
	...
	private function store_data_order_source_args( array $args, string $source ): array {
		if ( 'mobile' === $source ) {
			$args['meta_query'] = array( array( 'key' => '_mobishop_order_source', 'value' => 'mobile' ) );
		} elseif ( 'website' === $source ) {
			$args['meta_query'] = array(
				'relation' => 'OR',
				array( 'key' => '_mobishop_order_source', 'compare' => 'NOT EXISTS' ),
				array( 'key' => '_mobishop_order_source', 'value' => 'website' ),
			);
		}
		return $args;
	}
	...
	public function bundles_page(): void {
		...
		wp_safe_redirect( add_query_arg( array( 'page' => 'mobishop', 'view' => 'ai-insights' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	...
	public function ai_insights_page(): void {
		...
		$ai_generated = MobiShop_Analytics::has_commerce_snapshot( $date_from, $date_to, $ai_source );
		...
		$ai_summary         = MobiShop_Analytics::empty_summary();
		$all_recommendations = array();
		$ai_rotation_segments = array( 'fast' => array(), 'medium' => array(), 'slow' => array(), 'poor' => array() );
		if ( $ai_generated ) {
			$ai_summary         = MobiShop_Analytics::summary( $date_from, $date_to, $ai_source );
			$all_recommendations = MobiShop_AI_Offer_Engine::recommendations( $date_from, $date_to, $ai_source );
			$ai_rotation_segments = MobiShop_AI_Offer_Engine::rotation_segments( $date_from, $date_to, $ai_source );
		}
		...
		$ai_signal_count = count( MobiShop_AI_Offer_Engine::signal_catalog() );
		$ai_action_history = get_option( 'mobishop_ai_action_history_v1', array() );
		$ai_action_history = is_array( $ai_action_history )
			? array_reverse( array_slice( $ai_action_history, -100, null, true ), true )
			: array();
		require MOBISHOP_PATH . 'admin/pages/ai-insights.php';
	}
	...
	public function start_ai_analysis(): void {
		...
		check_ajax_referer( 'mobishop_ai_analysis', 'nonce' );
		...
		$result = MobiShop_AI_Analysis_Job::start( $from, $to, $source, get_current_user_id(), (string) $range['preset'], $full_regenerate );
	...
	public function generate_store_reporting(): void {
		...
		check_ajax_referer( 'mobishop_store_reporting', 'nonce' );
		...
		MobiShop_Analytics::reporting_snapshot( absint( $range['from'] ), absint( $range['to'] ), $source );
	...
	public function step_ai_analysis(): void {
		...
		$result = MobiShop_AI_Analysis_Job::step( $job_id, get_current_user_id() );
		if ( isset( $result['error'] ) ) {
			wp_send_json_error( array( 'message' => $result['error'] ), 400 );
		}
		wp_send_json_success( $result );
	...
	public function background_ai_analysis(): void {
		...
		check_ajax_referer( 'mobishop_ai_analysis', 'nonce' );
		$job_id = sanitize_text_field( (string) wp_unslash( $_POST['job_id'] ?? '' ) );
		$result = MobiShop_AI_Analysis_Job::continue_in_background( $job_id, get_current_user_id() );
		if ( isset( $result['error'] ) ) {
			wp_send_json_error( array( 'message' => $result['error'] ), 400 );
		}
		wp_send_json_success( $result );
	...
	public function cancel_ai_analysis(): void {
		...
		check_ajax_referer( 'mobishop_ai_analysis', 'nonce' );
		$job_id = sanitize_text_field( (string) wp_unslash( $_POST['job_id'] ?? '' ) );
		$result = MobiShop_AI_Analysis_Job::cancel( $job_id, get_current_user_id() );
		if ( isset( $result['error'] ) ) {
			wp_send_json_error( array( 'message' => $result['error'] ), 400 );
		}
		wp_send_json_success( $result );
	...
	public function ai_analysis_status(): void {
		...
		$result  = MobiShop_AI_Analysis_Job::status( $job_id, get_current_user_id(), $advance );
		if ( isset( $result['error'] ) ) {
	...
	public function dismiss_ai_analysis(): void {
		...
		check_ajax_referer( 'mobishop_ai_analysis', 'nonce' );
		$job_id = sanitize_text_field( (string) wp_unslash( $_POST['job_id'] ?? '' ) );
		MobiShop_AI_Analysis_Job::dismiss( $job_id, get_current_user_id() );
	...
	public function build_ai_action(): void {
		...
		check_admin_referer( 'mobishop_build_ai_action', 'mobishop_ai_action_nonce' );
		...
		$recommendation = null;
		foreach ( MobiShop_AI_Offer_Engine::recommendations( $from, $to, $source ) as $candidate ) {
		...
		if ( ! is_array( $recommendation ) ) {
			wp_safe_redirect( add_query_arg( array( 'page' => 'mobishop-ai-insights', 'ai_action_error' => 'not_found' ), admin_url( 'admin.php' ) ) );
			exit;
		}
		...
		if ( $recommended_product_ids ) {
			...
			if ( empty( $available_product_ids ) ) {
				wp_safe_redirect( add_query_arg( array( 'page' => 'mobishop-ai-insights', 'ai_action_error' => 'stock_changed' ), admin_url( 'admin.php' ) ) );
				exit;
			}
		...
		if ( 'coupon' === $action_type && class_exists( 'WC_Coupon' ) ) {
			...
			if ( $value > 0 ) {
				...
				$coupon->set_code( 'MOBISHOP-AI-' . strtoupper( wp_generate_password( 8, false, false ) ) );
				...
				if ( $coupon_id > 0 ) {
					MobiShop_Coupon_Channel::set( $coupon_id, $channel );
					$created_id = (string) $coupon_id;
				}
			}
		} elseif ( 'bundle' === $action_type ) {
			$created_id = MobiShop_Bundle_Recipes::store(
				array(
			...
			$actions = get_option( 'mobishop_ai_action_drafts', array() );
			...
			update_option( 'mobishop_ai_action_drafts', array_slice( $actions, -200, null, true ), false );
		}
		...
		if ( ! empty( $_POST['ai_promote_push'] ) ) {
			$args = array(
				'page'        => 'mobishop-push-notifications',
				'ai_offer_id' => $id,
				'ai_source'   => $source,
				'ai_from'     => $from,
				'ai_to'       => $to,
		...
		$history = get_option( 'mobishop_ai_action_history_v1', array() );
		...
		update_option( 'mobishop_ai_action_history_v1', array_slice( $history, -200, null, true ), false );
	...
	public function review_ai_result(): void {
		...
		check_admin_referer( 'mobishop_review_ai_result', 'mobishop_ai_result_nonce' );
		...
		if ( '' === $decision || ! is_array( $history[ $history_id ] ?? null ) ) {
			wp_safe_redirect( add_query_arg( array( 'page' => 'mobishop-ai-insights', 'ai_result_error' => 'not_found' ), admin_url( 'admin.php' ) ) );
			exit;
		}
		...
		if ( 'stop' === $decision ) {
			if ( 'coupon' === $type && class_exists( 'WC_Coupon' ) && absint( $reference ) > 0 ) {
				...
				MobiShop_Bundle_Recipes::set_status( $reference, 'draft' );
			}
			...
			if ( 'coupon' === $type && class_exists( 'WC_Coupon' ) && absint( $reference ) > 0 ) {
				...
				MobiShop_Bundle_Recipes::set_status( $reference, 'published' );
			} else {
				$actions = get_option( 'mobishop_ai_action_drafts', array() );
				$actions = is_array( $actions ) ? $actions : array();
				if ( is_array( $actions[ $reference ] ?? null ) ) {
					$actions[ $reference ]['status'] = 'publish';
					update_option( 'mobishop_ai_action_drafts', $actions, false );
				}
			}
		...
		update_option( 'mobishop_ai_action_history_v1', $history, false );
		$redirect_args = array(
			'page'            => 'mobishop-ai-insights',
			'ai_result_saved' => '1',
			'ai_generate'     => '1',
	...
	private function publish_ai_action_placement(
		...
		$runtime = get_option( 'mobishop_ai_published_actions_v1', array() );
		...
		update_option( 'mobishop_ai_published_actions_v1', array_slice( $runtime, -200, null, true ), false );
		if ( 'home' !== $placement || ! class_exists( 'MobiShop_Layout_Store' ) ) {
			return $result;
		}

		$layout = ( new MobiShop_Layout_Store() )->get_layout();
		...
		$block = MobiShop_Block_Registry::create( $block_type, count( $layout ) + 1 );
		if ( ! is_array( $block ) ) {
			return $result;
		}
		...
		$layout[] = $block;
		if ( ( new MobiShop_Layout_Store() )->save_layout( $layout ) ) {
	...
	private function set_ai_home_placement_enabled( string $block_id, bool $enabled ): void {
		...
		if ( '' === $block_id || ! class_exists( 'MobiShop_Layout_Store' ) ) {
			return;
		}
		$store  = new MobiShop_Layout_Store();
	...
	public function push_notifications_page(): void {
		...
		$push_status = MobiShop_Push_Service::connection_status();
		$push_connected = ! empty( $push_status['connected'] );
		$push_metrics = MobiShop_Push_Service::aggregate_metrics();
		$subscribed_customers = absint( get_option( 'mobishop_push_subscribed_customers', 0 ) );
		$registered_devices = absint( get_option( 'mobishop_push_registered_devices', 0 ) );
		$automations = get_option( 'mobishop_push_automations', array() );
		...
		if ( '' !== $prefill_id ) {
			...
			if ( $prefill_from > 0 && $prefill_to >= $prefill_from ) {
				foreach ( MobiShop_AI_Offer_Engine::recommendations( $prefill_from, $prefill_to, $prefill_source ) as $candidate ) {
		...
		require MOBISHOP_PATH . 'admin/pages/push-notifications.php';
	}
	...
	public function provision_push(): void {
		...
		check_admin_referer( 'mobishop_provision_push', 'mobishop_push_setup_nonce' );
		$result = MobiShop_Push_Service::provision_project();
		$args = array(
			'page'       => 'mobishop-push-notifications',
	...
	/** Validates FCM through MobiShop without delivering a real message. */
	public function test_push_connection(): void {
		...
		check_admin_referer( 'mobishop_test_push_connection', 'mobishop_push_test_nonce' );
		$result = MobiShop_Push_Service::validate_connection();
		$args = array(
			'page'      => 'mobishop-push-notifications',
	...
	public function set_coupon_channel(): void {
		...
		check_admin_referer( 'mobishop_set_coupon_channel_' . $coupon_id, 'mobishop_coupon_channel_nonce' );
		if ( $coupon_id <= 0 || 'shop_coupon' !== get_post_type( $coupon_id ) ) {
			wp_safe_redirect( add_query_arg( array( 'page' => 'mobishop-store-data', 'store_tab' => 'discounts', 'coupon_error' => 'missing' ), admin_url( 'admin.php' ) ) );
			exit;
		}
		$channel = sanitize_key( (string) wp_unslash( $_POST['coupon_channel'] ?? 'all' ) );
		MobiShop_Coupon_Channel::set( $coupon_id, $channel );
		$redirect = wp_get_referer();
		wp_safe_redirect( $redirect ? add_query_arg( 'coupon_updated', '1', $redirect ) : add_query_arg( array( 'page' => 'mobishop-store-data', 'store_tab' => 'discounts', 'coupon_updated' => '1' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	...
	public function send_push_notification(): void {
		...
		check_admin_referer( 'mobishop_send_push_notification', 'mobishop_push_nonce' );
		...
		if ( '' === $title || '' === $message ) {
			wp_safe_redirect( add_query_arg( array( 'page' => 'mobishop-push-notifications', 'push_error' => 'missing' ), admin_url( 'admin.php' ) ) );
			exit;
		}
		...
		if ( 'scheduled' === $delivery ) {
			...
			if ( ! $timestamp || $timestamp <= time() ) {
				wp_safe_redirect( add_query_arg( array( 'page' => 'mobishop-push-notifications', 'push_error' => 'schedule' ), admin_url( 'admin.php' ) ) );
				exit;
			}
			$payload['scheduled_at'] = $timestamp;
			$payload['status'] = 'scheduled';
			wp_schedule_single_event( $timestamp, 'mobishop_dispatch_scheduled_push', array( $payload ) );
			...
			update_option( 'mobishop_push_automations', $automations, false );
		} else {
		...
		$redirect_args = array( 'page' => 'mobishop-push-notifications' );
	...
	private function create_ai_offer_coupon( array $offer ): string {
		...
		$code     = 'MOBISHOP-AI-' . strtoupper( wp_generate_password( 8, false, false ) );
		$coupon   = new WC_Coupon();
	...
	/** Passes one normalized payload to the managed MobiShop Push service. */
	private function dispatch_push_payload( array $payload ): array {
		return MobiShop_Push_Service::dispatch( $payload );
	}
	...
	public function apply_setup_wizard(): void {
		...
		check_admin_referer( 'mobishop_apply_setup_wizard', 'mobishop_setup_nonce' );
		if ( ! ( new MobiShop_License_Manager() )->is_active() ) {
			wp_safe_redirect(
				add_query_arg(
					array(
						'page'          => 'mobishop',
		...
		$theme     = ( new MobiShop_Setup_Wizard() )->apply( is_array( $submitted ) ? $submitted : array() );
		if ( ! empty( $_POST['build_after_apply'] ) || ! empty( $_POST['export_after_apply'] ) ) {
			$result = ( new MobiShop_App_Exporter() )->start_build();
			$args   = array(
				'page'         => 'mobishop',
				'setup_done'   => '1',
				'theme'        => $theme,
	...
	public function manage_saved_theme(): void {
		...
		check_admin_referer( 'mobishop_manage_saved_theme', 'mobishop_theme_nonce' );
		if ( ! ( new MobiShop_License_Manager() )->is_active() ) {
			wp_safe_redirect( add_query_arg( array( 'page' => 'mobishop', 'license_error' => 'required' ), admin_url( 'admin.php' ) ) );
			exit;
		}
		...
		try {
			if ( 'save' === $operation ) {
				...
				header( 'Content-Disposition: attachment; filename="mobishop-theme-' . sanitize_file_name( $theme_id ) . ( $include_images ? '-with-images' : '-settings' ) . '.json"' );
			...
			$args = array( 'page' => 'mobishop-saved-themes', 'theme_notice' => $operation );
		} catch ( Throwable $error ) {
			$args = array( 'page' => 'mobishop-saved-themes', 'theme_error' => sanitize_key( $error->getMessage() ) ?: 'failed' );
		}
		wp_safe_redirect( add_query_arg( $args, admin_url( 'admin.php' ) ) );
		exit;
	}
	...
	public function splash_screen_page(): void {
		...
		$saved = get_option( 'mobishop_splash_screen', array() );
		$settings = array_merge( $defaults, is_array( $saved ) ? $saved : array() );
		require MOBISHOP_PATH . 'admin/pages/splash-screen.php';
	}
	...
	public function save_splash_screen(): void {
		...
		check_admin_referer( 'mobishop_save_splash_screen', 'mobishop_splash_nonce' );
		...
		update_option( 'mobishop_splash_screen', $clean, false );
		$fallback = add_query_arg( array( 'page' => 'mobishop-splash-screen', 'updated' => '1', 'saved_at' => time() ), admin_url( 'admin.php' ) );
	...
	public function similar_products_page(): void {
		...
		$store = new MobiShop_Page_Layout_Store(); $layout = $store->get_layout( 'product' );
		$definition = null; foreach ( MobiShop_Page_Layout_Store::element_definitions( 'product' ) as $item ) { if ( 'related_products' === $item['id'] ) { $definition = $item; break; } }
		$element = null; foreach ( $layout['elements'] as $item ) { if ( 'related_products' === $item['id'] ) { $element = $item; break; } }
		require MOBISHOP_PATH . 'admin/pages/similar-products.php';
	}

	public function save_similar_products(): void {
		if ( ! current_user_can( self::CAPABILITY ) ) { wp_die( esc_html__( 'You do not have permission to perform this action.', 'mobishop' ) ); }
		check_admin_referer( 'mobishop_save_similar_products', 'mobishop_similar_nonce' );
		$store = new MobiShop_Page_Layout_Store(); $layout = $store->get_layout( 'product' ); $submitted = isset( $_POST['related'] ) && is_array( $_POST['related'] ) ? wp_unslash( $_POST['related'] ) : array();
		...
		$fallback = add_query_arg( array( 'page' => 'mobishop-similar-products', 'updated' => '1', 'saved_at' => time() ), admin_url( 'admin.php' ) );
		wp_safe_redirect( $this->saved_theme_redirect( $fallback ) ); exit;
	}

	public function checkout_suggestions_page(): void {
		if ( ! current_user_can( self::CAPABILITY ) ) { wp_die( esc_html__( 'You do not have permission to access this page.', 'mobishop' ) ); }
		$checkout_store  = new MobiShop_Checkout_Fields_Store();
		$checkout_fields = $checkout_store->get();
		$checkout_design = $checkout_store->design();
		require MOBISHOP_PATH . 'admin/pages/checkout-suggestions.php';
	}
	...
	public function save_checkout_suggestions(): void {
		...
		check_admin_referer( 'mobishop_save_checkout_suggestions', 'mobishop_checkout_suggestions_nonce' );
		$field_store = new MobiShop_Checkout_Fields_Store();
		if ( isset( $_POST['restore_checkout_fields'] ) ) {
			$field_store->reset_from_site();
			wp_safe_redirect( add_query_arg( array( 'page' => 'mobishop-checkout-suggestions', 'fields_restored' => '1' ), admin_url( 'admin.php' ) ) );
			exit;
		}
		...
		$fallback = add_query_arg( array( 'page' => 'mobishop-checkout-suggestions', 'updated' => '1', 'saved_at' => time() ), admin_url( 'admin.php' ) );
	...
	public function page_builder_page(): void {
		...
		$page_labels = MobiShop_Page_Layout_Store::pages();
		$page_label = $page_labels[ $page ];
		$element_definitions = MobiShop_Page_Layout_Store::element_definitions( $page );
		$header_fields = MobiShop_Page_Layout_Store::header_fields();
		$footer_fields = MobiShop_Page_Layout_Store::footer_fields();
		require MOBISHOP_PATH . 'admin/pages/page-builder.php';
	}
	...
	public function save_page_builder(): void {
		...
		check_admin_referer( 'mobishop_save_page_builder', 'mobishop_page_builder_nonce' );
		...
		if ( isset( $_POST['restore_defaults'] ) || ( 'product' === $page && isset( $_POST['restore_product_defaults'] ) ) ) {
			( new MobiShop_Page_Layout_Store() )->reset_layout( $page );
		...
		( new MobiShop_Page_Layout_Store() )->save_layout( $page, is_array( $submitted ) ? $submitted : array() );
		$slug = array_search( $page, self::PAGE_BUILDER_SLUGS, true );
		if ( function_exists( 'nocache_headers' ) ) {
			nocache_headers();
		}
	...
	public function hide_element_library_menus(): void {
		foreach ( self::EDITOR_PAGES as $page_slug ) {
			remove_submenu_page( 'mobishop', $page_slug );
		}
	}
	...
	public function render_cms_shell(): void {
		...
		$tab = static function ( string $label, string $page_slug, string $icon ): array {
			$view = array_search( $page_slug, self::CMS_VIEWS, true ); $args = array( 'page' => 'mobishop' );
		...
		$tabs = array(
			...
			'splash'     => $tab( __( 'Splash', 'mobishop' ), 'mobishop-splash-screen', 'dashicons-format-image' ),
			'home'       => $tab( __( 'Home', 'mobishop' ), 'mobishop-home-builder', 'dashicons-admin-home' ),
			'category'   => $tab( __( 'Categories', 'mobishop' ), 'mobishop-category-builder', 'dashicons-category' ),
			'catalog'    => $tab( __( 'Catalog', 'mobishop' ), 'mobishop-catalog-builder', 'dashicons-grid-view' ),
			'product'    => $tab( __( 'Product', 'mobishop' ), 'mobishop-product-builder', 'dashicons-products' ),
			'wishlist'   => $tab( __( 'Wishlist', 'mobishop' ), 'mobishop-wishlist-builder', 'dashicons-heart' ),
			'account'    => $tab( __( 'Account', 'mobishop' ), 'mobishop-account-builder', 'dashicons-admin-users' ),
			'checkout'   => $tab( __( 'Checkout', 'mobishop' ), 'mobishop-checkout-suggestions', 'dashicons-cart' ),
		);
		$active_map = array(
			'mobishop'                  => 'overview',
			'mobishop-home-builder'         => 'home',
			'mobishop-category-builder'     => 'category',
			'mobishop-catalog-builder'      => 'catalog',
			'mobishop-product-builder'      => 'product',
			'mobishop-wishlist-builder'     => 'wishlist',
			'mobishop-account-builder'      => 'account',
			'mobishop-size-chart-builder'   => 'size_chart',
			'mobishop-splash-screen'        => 'splash',
			'mobishop-similar-products'     => 'similar',
			'mobishop-checkout-suggestions'=> 'checkout',
			'mobishop-setup'                => 'setup',
			'mobishop-saved-themes'         => 'saved_themes',
			'mobishop-store-data'           => 'store_data',
			'mobishop-ai-insights'           => 'ai_insights',
			'mobishop-bundles'              => 'ai_insights',
			'mobishop-push-notifications'   => 'push',
			'mobishop-website-app-promotion' => 'website_promotion',
		);
		$active_tab = $active_map[ $page ] ?? 'overview';
		$store_data_tab = isset( $_GET['store_tab'] ) ? sanitize_key( wp_unslash( $_GET['store_tab'] ) ) : '';
		if ( 'mobishop-store-data' === $page && 'abandoned-carts' === $store_data_tab ) {
			$active_tab = 'abandoned_carts';
		}
		if ( 'mobishop' === $page && ! ( new MobiShop_Setup_Wizard() )->is_complete() ) {
			$active_tab = 'setup';
		}
		...
		$sidebar_items = array(
			'overview' => $tab( __( 'Overview', 'mobishop' ), 'mobishop', 'dashicons-chart-area' ),
			'setup'    => $tab( __( 'Setup Wizard', 'mobishop' ), 'mobishop-setup', 'dashicons-admin-customizer' ),
			'pages'    => $tab( __( 'Customize Your Pages', 'mobishop' ), 'mobishop-splash-screen', 'dashicons-admin-appearance' ),
			'saved_themes' => $tab( __( 'Saved Themes', 'mobishop' ), 'mobishop-saved-themes', 'dashicons-portfolio' ),
			'store_data' => $tab( __( 'Store Data', 'mobishop' ), 'mobishop-store-data', 'dashicons-database' ),
			'ai_insights' => $tab( __( 'AI Offer Studio', 'mobishop' ), 'mobishop-ai-insights', 'dashicons-lightbulb' ),
			...
			'push' => $tab( __( 'Push Notifications', 'mobishop' ), 'mobishop-push-notifications', 'dashicons-megaphone' ),
			'website_promotion' => $tab( __( 'Promote App on Website', 'mobishop' ), 'mobishop-website-app-promotion', 'dashicons-smartphone' ),
		);
		...
		$license_status = ( new MobiShop_License_Manager() )->status();
		$app_build_state = MobiShop_App_Exporter::state();
		$abandoned_import_state = MobiShop_Analytics::website_session_import_status();
		require MOBISHOP_PATH . 'admin/pages/cms-shell.php';
	}
	...
	public function cms_view_fragment(): void {
		...
		check_ajax_referer( 'mobishop_view', 'nonce' );
		...
		wp_send_json_success(
			array(
				...
				'bodyClasses'   => array_values( array_filter( preg_split( '/\s+/', trim( $this->admin_body_class( 'mobishop' ) ) ) ?: array() ) ),
				'version'       => MOBISHOP_VERSION,
	...
	private function render_effective_cms_view(): void {
		...
		if ( 'mobishop-home-builder' === $page ) {
			$this->home_builder_page();
			return;
		}
		if ( 'mobishop-category-builder' === $page ) {
		...
		$callbacks = array(
			'mobishop-splash-screen'         => 'splash_screen_page',
			'mobishop-checkout-suggestions' => 'checkout_suggestions_page',
			'mobishop-setup'                => 'setup_wizard_page',
			'mobishop-saved-themes'         => 'saved_themes_page',
			'mobishop-store-data'           => 'store_data_page',
			'mobishop-ai-insights'          => 'ai_insights_page',
			'mobishop-bundles'              => 'bundles_page',
			'mobishop-push-notifications'   => 'push_notifications_page',
			'mobishop-website-app-promotion'=> 'website_app_promotion_page',
		);
		...
		return in_array(
			...
			array_merge(
				array(
					...
					'mobishop-category-builder',
					'mobishop-splash-screen',
					'mobishop-similar-products',
					'mobishop-checkout-suggestions',
					'mobishop-setup',
					'mobishop-saved-themes',
					'mobishop-store-data',
					'mobishop-ai-insights',
					'mobishop-bundles',
					'mobishop-push-notifications',
					'mobishop-website-app-promotion',
				),
				array_keys( self::PAGE_BUILDER_SLUGS )
			),
			true
	...
	private function effective_cms_page(): string {
		...
		if ( 'mobishop' !== $page ) { return $page; }
		$view = isset( $_GET['view'] ) ? sanitize_key( wp_unslash( $_GET['view'] ) ) : 'overview';
		return self::CMS_VIEWS[ $view ] ?? 'mobishop';
	}
	...
	public function save_category_builder(): void {
		...
		check_admin_referer( 'mobishop_save_category_builder', 'mobishop_category_builder_nonce' );
		if ( isset( $_POST['restore_defaults'] ) ) {
			( new MobiShop_Category_Page_Store() )->save_settings(
				array(
					'enabled'    => true,
					'general'    => MobiShop_Category_Page_Store::general_defaults(),
					'categories' => array(),
				)
			);
			( new MobiShop_Page_Layout_Store() )->reset_layout( 'category' );
			wp_safe_redirect( add_query_arg( array( 'page' => 'mobishop-category-builder', 'restored' => '1', 'restored_at' => time() ), admin_url( 'admin.php' ) ) );
			exit;
		}
		...
		( new MobiShop_Page_Layout_Store() )->save_layout( 'category', is_array( $layout ) ? $layout : array() );
		$fallback = add_query_arg( array( 'page' => 'mobishop-category-builder', 'updated' => '1' ), admin_url( 'admin.php' ) );
		wp_safe_redirect( $this->requested_builder_redirect( $fallback ) );
		exit;
	}
    	...
    	public function dashboard_page(): void {
			...
			if ( isset( self::CMS_VIEWS[ $view ] ) && 'overview' !== $view ) {
				...
				if ( 'mobishop-home-builder' === $legacy_page ) { $this->home_builder_page(); return; }
				if ( 'mobishop-category-builder' === $legacy_page ) { $this->category_builder_page(); return; }
				if ( isset( self::PAGE_BUILDER_SLUGS[ $legacy_page ] ) ) { $this->page_builder_page(); return; }
				$callbacks = array('mobishop-splash-screen'=>'splash_screen_page','mobishop-checkout-suggestions'=>'checkout_suggestions_page','mobishop-setup'=>'setup_wizard_page','mobishop-saved-themes'=>'saved_themes_page','mobishop-store-data'=>'store_data_page','mobishop-ai-insights'=>'ai_insights_page','mobishop-bundles'=>'bundles_page','mobishop-push-notifications'=>'push_notifications_page','mobishop-website-app-promotion'=>'website_app_promotion_page');
				if ( isset( $callbacks[ $legacy_page ] ) ) { $this->{$callbacks[ $legacy_page ]}(); return; }
			}

			$monitor = new MobiShop_API_Monitor();

    		$api = $monitor->get_status();
			$license_manager = new MobiShop_License_Manager();
			$license         = $license_manager->status();
			$setup_complete  = ( new MobiShop_Setup_Wizard() )->is_complete();
			$website_connected = ! empty( $license['active'] )
				|| '1' === (string) get_option( 'mobishop_website_connected', '0' );

			if (
				isset( $_GET['mobishop_connected'], $_GET['mobishop_connect_nonce'] )
				&& '1' === sanitize_key( wp_unslash( $_GET['mobishop_connected'] ) )
				&& wp_verify_nonce(
					sanitize_text_field( wp_unslash( $_GET['mobishop_connect_nonce'] ) ),
					'mobishop_connect_return'
				)
			) {
				update_option( 'mobishop_website_connected', '1', false );
			...
			$connect_return_url = add_query_arg(
				array(
					'page'                     => 'mobishop',
					'mobishop_connected'      => '1',
					'mobishop_connect_nonce'  => wp_create_nonce( 'mobishop_connect_return' ),
				),
				admin_url( 'admin.php' )
			) . '#mobishop-license-key';
			$connect_url = apply_filters(
				'mobishop_customer_connect_url',
				add_query_arg(
					array(
						'platform'         => 'wordpress',
						'plugin_installed' => '1',
			...
			$app_export_state   = MobiShop_App_Exporter::state();
			$app_export_current = MobiShop_App_Exporter::is_current();
			$push_export_config = MobiShop_Push_Service::client_configuration();
	...
	public function activate_license(): void {
		...
		$result = ( new MobiShop_License_Manager() )->activate( $key );
	...
	public function verify_license(): void {
		$this->assert_license_action();
		$result = ( new MobiShop_License_Manager() )->verify( true );
	...
	private function assert_license_action(): void {
		...
		check_admin_referer( 'mobishop_license_action', 'mobishop_license_nonce' );
	}
	...
	private function redirect_license_result( $result, string $success ): void {
		...
		exit;
        	...
        	public function save_home_builder(): void {
				...
				if ( isset( $_POST['restore_defaults'] ) ) {
					...
					( new MobiShop_Page_Layout_Store() )->reset_layout( 'home' );
					wp_safe_redirect( add_query_arg( array( 'page' => 'mobishop-home-builder', 'restored' => '1', 'restored_at' => time() ), admin_url( 'admin.php' ) ) );
					exit;
				}
				...
				$submitted_blocks = MobiShop_Layout_Store::decode_submission(
					$payload,
					$encoding
				);
		...
		$store->save_layout(
			...
			( new MobiShop_Page_Layout_Store() )->save_layout( 'home', is_array( $chrome ) ? $chrome : array() );
			...
			private function requested_builder_redirect( string $fallback ): string {
				$requested = isset( $_POST['mobishop_redirect_to'] )
					? esc_url_raw( wp_unslash( $_POST['mobishop_redirect_to'] ) )
					: '';
			...
			private function saved_theme_redirect( string $fallback ): string {
				$name = isset( $_POST['mobishop_save_theme_name'] )
					? sanitize_text_field( wp_unslash( (string) $_POST['mobishop_save_theme_name'] ) )
					: '';
				if ( '' === $name ) {
					return $fallback;
				}
				( new MobiShop_Setup_Wizard() )->save_current_theme( $name );
				return add_query_arg(
					array(
						'page'         => 'mobishop-saved-themes',
						'theme_notice' => 'save',
					),
					admin_url( 'admin.php' )
				);
		...
		public function enqueue_assets(
					...
					if ( current_user_can( self::CAPABILITY ) ) {
						wp_enqueue_script(
							'mobishop-entry',
							MOBISHOP_URL . 'admin/assets/cms-entry.js',
							array(),
							MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/cms-entry.js' ),
							true
						);
						wp_localize_script(
							'mobishop-entry',
							...
							array(
								...
								'nonce'   => wp_create_nonce( 'mobishop_view' ),
								'version' => MOBISHOP_VERSION,
							)
						);
					}
					...
					wp_enqueue_style(
						...
						array(),
						MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/admin-theme.css' )
					);
					wp_enqueue_style( 'mobishop-shell', MOBISHOP_URL . 'admin/assets/cms-shell.css', array( 'mobishop-admin-theme' ), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/cms-shell.css' ) );
					wp_enqueue_script( 'mobishop-shell', MOBISHOP_URL . 'admin/assets/cms-shell.js', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/cms-shell.js' ), true );
					wp_localize_script(
						'mobishop-shell',
						'mobishopCMSBackground',
						array(
							'ajaxUrl'     => admin_url( 'admin-ajax.php' ),
							'aiNonce'     => wp_create_nonce( 'mobishop_ai_analysis' ),
							'storeNonce'  => wp_create_nonce( 'mobishop_store_reporting' ),
							'cartNonce'   => wp_create_nonce( 'mobishop_abandoned_cart_details' ),
							'activeAiJob' => MobiShop_AI_Analysis_Job::active_job_id( get_current_user_id() ),
							'aiUrl'       => add_query_arg( array( 'page' => 'mobishop-ai-insights' ), admin_url( 'admin.php' ) ),
						)
					);
					wp_localize_script(
						'mobishop-shell',
						'mobishopCMSNavigation',
						array(
							'ajaxUrl' => admin_url( 'admin-ajax.php' ),
							'nonce'   => wp_create_nonce( 'mobishop_view' ),
							'version' => MOBISHOP_VERSION,
					...
					if ( 'mobishop-ai-insights' === $page ) {
						wp_enqueue_script(
							'mobishop-ai-offer-workspace',
							MOBISHOP_URL . 'admin/assets/ai-offer-workspace.js',
							array( 'mobishop-shell' ),
							MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/ai-offer-workspace.js' ),
							true
						);
					}
					if ( $this->is_public_cms_page( $page ) ) {
						wp_enqueue_script(
							'mobishop-app-builder',
							MOBISHOP_URL . 'admin/assets/app-builder.js',
							array(),
							MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/app-builder.js' ),
							true
						);
						wp_localize_script(
							'mobishop-app-builder',
							...
							array(
								'ajaxUrl'     => admin_url( 'admin-ajax.php' ),
								'nonce'       => wp_create_nonce( 'mobishop_app_build_status' ),
								'cancelNonce' => wp_create_nonce( 'mobishop_app_build_cancel' ),
								'downloadUrl' => wp_nonce_url(
									add_query_arg(
										array( 'action' => 'mobishop_download_apk' ),
										admin_url( 'admin-post.php' )
									),
									'mobishop_download_apk',
									'mobishop_download_nonce'
								),
								'labels'      => array(
									'queued'        => __( 'Android and iOS build queued…', 'mobishop' ),
									'building'      => __( 'Building Android and iOS…', 'mobishop' ),
									'ready'         => __( 'Android and iOS build files are ready.', 'mobishop' ),
									'failed'        => __( 'The Android and iOS build failed.', 'mobishop' ),
									'timeout'       => __( 'The Android and iOS build request took too long. Please try again.', 'mobishop' ),
									'starting'      => __( 'Starting Android and iOS build…', 'mobishop' ),
									'buildDownload' => __( 'Build Your App', 'mobishop' ),
									'download'      => __( 'Download Android and iOS', 'mobishop' ),
					...
					if ( 'mobishop-website-app-promotion' === $page ) {
						wp_enqueue_media();
						wp_enqueue_style(
							'mobishop-website-app-promotion-preview',
							MOBISHOP_URL . 'public/assets/website-app-promotion.css',
							array( 'mobishop-shell' ),
							MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'public/assets/website-app-promotion.css' )
						);
						wp_enqueue_style(
							'mobishop-website-app-promotion-admin',
							MOBISHOP_URL . 'admin/assets/website-app-promotion.css',
							array( 'mobishop-website-app-promotion-preview' ),
							MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/website-app-promotion.css' )
						);
						wp_enqueue_script(
							'mobishop-qrcode',
							MOBISHOP_URL . 'public/assets/vendor/qrcode.min.js',
							array(),
							'1.0.0',
							true
						);
						wp_enqueue_script(
							'mobishop-website-app-promotion-admin',
							MOBISHOP_URL . 'admin/assets/website-app-promotion.js',
							array( 'mobishop-qrcode' ),
							MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/website-app-promotion.js' ),
							true
						);
					}
					if (
						...
						wp_enqueue_style( 'mobishop-license-preview', MOBISHOP_URL . 'admin/assets/license-preview.css', array( 'mobishop-shell' ), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/license-preview.css' ) );
						wp_enqueue_script( 'mobishop-license-preview', MOBISHOP_URL . 'admin/assets/license-preview.js', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/license-preview.js' ), true );
						wp_localize_script(
							'mobishop-license-preview',
							'mobishopLicensePreview',
							array(
								...
								'actionUrl'   => admin_url( 'admin.php?page=mobishop#mobishop-license-key' ),
							)
						);
					}
					if ( in_array( $page, array( 'mobishop-setup', 'mobishop-saved-themes' ), true ) || ( 'mobishop' === $page && ! ( new MobiShop_Setup_Wizard() )->is_complete() ) ) {
						wp_enqueue_media();
						wp_enqueue_style( 'mobishop-setup-wizard', MOBISHOP_URL . 'admin/assets/setup-wizard.css', array( 'mobishop-shell' ), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/setup-wizard.css' ) );
						if ( 'mobishop-saved-themes' === $page ) {
							wp_enqueue_script( 'mobishop-saved-themes', MOBISHOP_URL . 'admin/assets/saved-themes.js', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/saved-themes.js' ), true );
							...
							wp_localize_script(
								'mobishop-saved-themes',
								'mobishopSavedThemePreview',
								array(
									'flutterUrl'              => MOBISHOP_URL . 'admin/flutter-preview/index.html',
									'layoutPreviewBase'       => rest_url( 'mobishop/v1/page-layout/' ),
									'homePreviewEndpoint'     => rest_url( 'mobishop/v1/home-layout/preview' ),
									'categoryPreviewEndpoint' => rest_url( 'mobishop/v1/category-page/preview' ),
							...
							wp_enqueue_script( 'mobishop-setup-wizard', MOBISHOP_URL . 'admin/assets/setup-wizard.js', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/setup-wizard.js' ), true );
							$setup_preview_wizard = new MobiShop_Setup_Wizard();
							$setup_theme_snapshots = array();
							foreach ( array_keys( MobiShop_Setup_Wizard::themes() ) as $setup_theme_key ) {
								$setup_theme_snapshots[ $setup_theme_key ] = $setup_preview_wizard->preview_snapshot( (string) $setup_theme_key );
							}
							wp_localize_script(
								'mobishop-setup-wizard',
								'mobishopSetupThemePreview',
								array(
									'flutterUrl' => MOBISHOP_URL . 'admin/flutter-preview/index.html',
									'layoutPreviewBase' => rest_url( 'mobishop/v1/page-layout/' ),
									'homePreviewEndpoint' => rest_url( 'mobishop/v1/home-layout/preview' ),
					...
					if (
							'mobishop-home-builder' !== $page
							&& 'mobishop-category-builder' !== $page
							&& ! isset( self::PAGE_BUILDER_SLUGS[ $page ] )
							&& 'mobishop-splash-screen' !== $page
							&& 'mobishop-similar-products' !== $page
							&& 'mobishop-checkout-suggestions' !== $page
							&& 'mobishop_page_mobishop-home-builder'
								!== $hook_suffix
					) {
            			return;
            		}
					...
					wp_enqueue_media();
					wp_enqueue_style( 'mobishop-fixed-chrome', MOBISHOP_URL . 'admin/assets/page-builder.css', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/page-builder.css' ) );
					wp_enqueue_style( 'mobishop-chrome-layout', MOBISHOP_URL . 'admin/assets/chrome-layout.css', array( 'mobishop-fixed-chrome' ), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/chrome-layout.css' ) );
					if ( 'mobishop-checkout-suggestions' === $page ) {
						wp_enqueue_script( 'mobishop-checkout-fields', MOBISHOP_URL . 'admin/assets/checkout-fields-builder.js', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/checkout-fields-builder.js' ), true );
					}
					wp_enqueue_script( 'mobishop-settings-sections', MOBISHOP_URL . 'admin/assets/settings-sections.js', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/settings-sections.js' ), true );
					wp_localize_script(
						'mobishop-settings-sections',
						'mobishopProductApplyAll',
						array(
							'ajaxUrl' => admin_url( 'admin-ajax.php' ),
							'nonce'   => wp_create_nonce( 'mobishop_apply_product_icon_settings' ),
							'labels'  => array(
					...
					wp_enqueue_script( 'mobishop-chrome-layout', MOBISHOP_URL . 'admin/assets/chrome-layout.js', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/chrome-layout.js' ), true );
					$category_footer = ( new MobiShop_Page_Layout_Store() )->get_layout( 'category' )['footer']['settings'];
					wp_localize_script(
						'mobishop-chrome-layout',
						'mobishopChromePreviewConfig',
						array(
							'footerSize' => array(
					...
					$is_layout_builder = 'mobishop-home-builder' === $page
						|| 'mobishop-category-builder' === $page
						|| isset( self::PAGE_BUILDER_SLUGS[ $page ] )
						|| 'mobishop_page_mobishop-home-builder' === $hook_suffix;
					if ( $is_layout_builder ) {
						wp_enqueue_script( 'mobishop-unsaved-changes', MOBISHOP_URL . 'admin/assets/unsaved-changes.js', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/unsaved-changes.js' ), true );
						wp_localize_script(
							'mobishop-unsaved-changes',
							'mobishopUnsavedChanges',
							array(
								'labels' => array(
					...
					if ( 'mobishop-splash-screen' === $page ) {
						wp_enqueue_script( 'mobishop-splash-screen', MOBISHOP_URL . 'admin/assets/splash-screen.js', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/splash-screen.js' ), true ); return;
					}
					if ( 'mobishop-checkout-suggestions' === $page ) {
						wp_enqueue_script( 'mobishop-commerce-preview', MOBISHOP_URL . 'admin/assets/commerce-preview.js', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/commerce-preview.js' ), true );
						return;
					}
					if ( 'mobishop-similar-products' === $page ) {
						...
						wp_enqueue_script( 'mobishop-commerce-preview', MOBISHOP_URL . 'admin/assets/commerce-preview.js', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/commerce-preview.js' ), true );
						wp_localize_script( 'mobishop-commerce-preview', 'mobishopCommercePreview', array( 'products' => $preview_products ) );
						return;
					}

					if ( 'mobishop-category-builder' === $page ) {
						wp_enqueue_style( 'mobishop-category-builder', MOBISHOP_URL . 'admin/assets/category-builder.css', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/category-builder.css' ) );
						wp_enqueue_script( 'mobishop-category-builder', MOBISHOP_URL . 'admin/assets/category-builder.js', array( 'jquery', 'jquery-ui-sortable' ), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/category-builder.js' ), true );
						if ( file_exists( MOBISHOP_PATH . 'admin/flutter-preview/index.html' ) ) {
							wp_enqueue_script( 'mobishop-flutter-category-preview-bridge', MOBISHOP_URL . 'admin/assets/flutter-category-preview-bridge.js', array( 'mobishop-category-builder' ), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/flutter-category-preview-bridge.js' ), true );
							wp_localize_script( 'mobishop-flutter-category-preview-bridge', 'mobishopFlutterPreview', array(
								'layoutPreviewEndpoint' => esc_url_raw( rest_url( 'mobishop/v1/page-layout/category/preview' ) ),
								'categoryPreviewEndpoint' => esc_url_raw( rest_url( 'mobishop/v1/category-page/preview' ) ),
								'restNonce' => wp_create_nonce( 'wp_rest' ),
							) );
						}
						return;
					...
					if ( isset( self::PAGE_BUILDER_SLUGS[ $page ] ) ) {
						...
						wp_enqueue_style( 'mobishop-page-builder', MOBISHOP_URL . 'admin/assets/page-builder.css', array(), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/page-builder.css' ) );
						wp_enqueue_script( 'mobishop-page-builder', MOBISHOP_URL . 'admin/assets/page-builder.js', array( 'jquery', 'jquery-ui-sortable' ), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/page-builder.js' ), true );
						if ( file_exists( MOBISHOP_PATH . 'admin/flutter-preview/index.html' ) ) {
							wp_enqueue_script( 'mobishop-flutter-preview-bridge', MOBISHOP_URL . 'admin/assets/flutter-preview-bridge.js', array( 'mobishop-page-builder' ), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/flutter-preview-bridge.js' ), true );
							wp_localize_script( 'mobishop-flutter-preview-bridge', 'mobishopFlutterPreview', array(
								'layoutPreviewEndpoint' => esc_url_raw( rest_url( 'mobishop/v1/page-layout/' . self::PAGE_BUILDER_SLUGS[ $page ] . '/preview' ) ),
								'restNonce' => wp_create_nonce( 'wp_rest' ),
							) );
						}
						wp_localize_script(
							'mobishop-page-builder',
							'mobishopPageBuilder',
							array(
					...
					wp_enqueue_style(
						...
						array(),
						MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/home-builder.css' )
					);
					...
					wp_enqueue_script(
						...
						array(),
						MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/home-builder.js' ),
						true
					);
					if ( file_exists( MOBISHOP_PATH . 'admin/flutter-preview/index.html' ) ) {
						wp_enqueue_script( 'mobishop-flutter-home-preview-bridge', MOBISHOP_URL . 'admin/assets/flutter-home-preview-bridge.js', array( 'mobishop-home-builder' ), MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'admin/assets/flutter-home-preview-bridge.js' ), true );
					}
            		...
            		wp_localize_script(
						...
						'previewEndpoint' => esc_url_raw(
							add_query_arg(
								'locale',
								$preview_locale,
								rest_url( 'mobishop/v1/home-layout' )
							)
						),
					'livePreviewEndpoint' => esc_url_raw( rest_url( 'mobishop/v1/home-layout/preview' ) ),
					'layoutPreviewEndpoint' => esc_url_raw( rest_url( 'mobishop/v1/page-layout/home/preview' ) ),
						'restNonce'           => wp_create_nonce( 'wp_rest' ),
					)
            		);
            	}
