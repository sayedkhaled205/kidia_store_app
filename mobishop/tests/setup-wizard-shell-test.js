...
const service = read("includes", "class-mobishop-setup-wizard.php");
const exporter = read("includes", "class-mobishop-app-exporter.php");
const licenseManager = read("includes", "class-mobishop-license-manager.php");
const pushService = read("includes", "class-mobishop-push-service.php");
const bootstrap = read("includes", "class-mobishop.php");
...
const shellStyleRules = shellCss.replace(/\/\*[\s\S]*?\*\//g, "");
...
const pageStore = read("includes", "class-mobishop-page-layout-store.php");
...
assert.match(service, /MobiShop_Layout_Store/, "Themes must update the Home builder.");
assert.match(service, /MobiShop_Page_Layout_Store/, "Themes must update application page builders.");
assert.match(service, /MobiShop_Category_Page_Store/, "Themes must update the Category builder.");
assert.match(service, /mobishop_splash_screen/, "Themes must configure the Splash screen.");
...
assert.match(service, /MobiShop_Checkout_Fields_Store::DESIGN_OPTION/, "Themes must preserve the selected checkout design.");
assert.doesNotMatch(service, /mobishop_checkout_suggestions/, "Themes must not restore removed Checkout Suggested Products.");
...
assert.match(familyTheme, /'blocks'\s*=>\s*array\(\s*'category_grid',\s*'image_banner',\s*'quick_links',\s*'banner_grid',\s*'product_carousel',\s*'product_grid'\s*\)/, "Family Pop Home must install the six PatPat reference sections in their visual order.");
assert.match(service, /'family_pop'\s*===\s*\$profile[\s\S]*'Customer Service'[\s\S]*\$settings\['layout'\]\s*=\s*'service'[\s\S]*'family_service_chat'[\s\S]*'family_service_call'[\s\S]*'family_service_faq'/, "Family Pop must install the three-action customer service section.");
assert.match(service, /isset\(\s*\$settings\['signature_feature'\]\s*\)[\s\S]*!\s*\(\s*'family_pop'\s*===\s*\$profile\s*&&\s*'quick_links'\s*===\s*\$type\s*\)/, "Family Pop customer service must not be overwritten by the legacy quick-link signature.");
assert.match(service, /'family_pop'\s*=>\s*array\([\s\S]*'Baby Shoes'[\s\S]*'Matching Family'[\s\S]*'Accessories'/, "Family Pop must seed all nine category-grid cells shown in the PatPat reference.");
assert.match(service, /'family_pop'\s*===\s*\$profile[\s\S]*\$settings\['starter_items'\][\s\S]*theme_demo_labels/, "Family Pop category grid must fill missing live categories from its nine-cell starter set.");
assert.match(fs.readFileSync(path.join(root, 'includes/blocks/class-mobishop-quick-links-block.php'), 'utf8'), /array\(\s*'carousel',\s*'grid',\s*'service'\s*\)/, "Quick Links must preserve the Family Pop service layout through PHP sanitization.");
assert.match(fs.readFileSync(path.join(root, 'includes/blocks/class-mobishop-banner-grid-block.php'), 'utf8'), /array\(\s*'equal',\s*'featured',\s*'mosaic',\s*'carousel'\s*\)/, "Banner Grid must preserve the horizontal Explore Trends layout through PHP sanitization.");
assert.match(service, /'family_pop'\s*===\s*\$profile[\s\S]*'Explore Trends'[\s\S]*\$settings\['layout'\]\s*=\s*'carousel'/, "Family Pop trend campaigns must use the horizontal PatPat card layout.");
assert.match(service, /'product_carousel'\s*===\s*\$type\s*\?\s*__\(\s*'Flash Sale'[\s\S]*__\(\s*'Just for you'/, "Family Pop product sections must carry the PatPat Flash Sale and Just for you labels.");
...
assert.match(familyTheme, /'product_ratio'\s*=>\s*1\.22[\s\S]*'product_radius'\s*=>\s*12[\s\S]*'product_style'\s*=>\s*'outlined'/, "Family Pop Home product cards must use the compact outlined campaign proportions.");
assert.match(familyTheme, /'category_design'[\s\S]*'category_layout'\s*=>\s*'default'[\s\S]*'card_gap'\s*=>\s*12[\s\S]*'card_height'\s*=>\s*84[\s\S]*'card_style'\s*=>\s*'outlined'[\s\S]*'card_radius'\s*=>\s*16[\s\S]*'image_size'\s*=>\s*68[\s\S]*'image_shape'\s*=>\s*'rounded'/, "Family Pop categories must explicitly use the compact rounded PatPat list layout instead of inheriting the circular theme grid.");
assert.match(familyTheme, /'image_position'\s*=>\s*'right'/, "Family Pop category artwork must remain on the right side of the list row.");
assert.match(familyProfile, /'font_family'\s*=>\s*'poppins'/, "Family Pop chrome must carry its own deterministic application font.");
assert.match(familyProfile, /'header_heights'\s*=>\s*array\(\s*'home'\s*=>\s*120[\s\S]*'catalog'\s*=>\s*64[\s\S]*'search_height'\s*=>\s*44[\s\S]*'search_icon_size'\s*=>\s*20/, "Family Pop must use the exact two-row PatPat home header and compact inner-page header measurements.");
assert.match(service, /Setup themes use flat headers[\s\S]*'shadow'\]\s*=\s*'none'[\s\S]*'compact_shadow'\]\s*=\s*'none'[\s\S]*'border_width'\]\s*=\s*0[\s\S]*'compact_border_width'\]\s*=\s*0[\s\S]*'corner_radius'\]\s*=\s*0/, "Every setup-theme header must blend into the following section without a border, shadow or rounded seam.");
assert.match(service, /'family_pop'\s*===\s*\$profile[\s\S]*'overlay_strength'\]\s*=\s*0[\s\S]*'text_color'\]\s*=\s*\$primary[\s\S]*'button_color'\]\s*=\s*\$primary[\s\S]*'button_text_color'\]\s*=\s*'#FFFFFF'/, "Family Pop banner-grid overlays must use the selected brand color with white button text.");
assert.match(service, /'family_pop'\s*===\s*\$profile[\s\S]*'content_bottom_padding'\]\s*=\s*0/, "Family Pop product cards must not reserve empty space below the price.");
assert.match(service, /\$is_family_pop[\s\S]*'category'\s*=>\s*array[\s\S]*28,\s*array\(\s*'search',\s*'cart'[\s\S]*72,\s*array\(\s*'title'[\s\S]*'catalog'\s*=>\s*array[\s\S]*'back'[\s\S]*'search',\s*'cart'/, "Family Pop must install the PatPat search, bag and title header composition.");
assert.match(familyProfile, /'icon_size'\s*=>\s*24[\s\S]*'label_size'\s*=>\s*11[\s\S]*'icon_label_gap'\s*=>\s*3/, "Family Pop navigation must carry the compact PatPat icon and label measurements.");
...
assert.match(wizardCss, /\.mobishop-theme-modal__device iframe\{[^}]*transform:scale\(var\(--mobishop-theme-preview-scale\)\);[^}]*transform-origin:top left/, "Theme preview must scale deterministically inside the shared phone frame.");
assert.doesNotMatch(wizardCss, /\.mobishop-theme-modal__device iframe\{[^}]*zoom:/, "Theme preview must not use browser-dependent iframe zoom that distorts the phone frame.");
...
assert.match(bootstrap, /class-mobishop-app-exporter\.php[\s\S]*MobiShop_App_Exporter\(\)\)->register/, "The app exporter must load and register with the plugin.");
assert.match(exporter, /mobishop-app-build-package[\s\S]*app-config\.json[\s\S]*push-config\.json[\s\S]*dart-defines\.json/, "Export App must download a portable build package.");
assert.match(exporter, /admin_post_mobishop_build_app[\s\S]*admin_post_mobishop_download_apk[\s\S]*wp_ajax_mobishop_app_build_start[\s\S]*wp_ajax_mobishop_app_build_status/, "APK builds must expose fallback, asynchronous start, status and download actions.");
assert.match(exporter, /start_build\(\)[\s\S]*'platform'\s*=>\s*'both'[\s\S]*'artifact'\s*=>\s*'zip'/, "The unified button must request Android and iOS in one build bundle.");
...
assert.match(licenseManager, /BUILD_API_BASE_URL[\s\S]*build_service_request[\s\S]*Authorization[\s\S]*X-MobiShop-Installation/, "The build service must reuse installation-bound license authorization without exposing it to the browser.");
assert.match(licenseManager, /FIREBASE_API_BASE_URL[\s\S]*firebase_service_request[\s\S]*Authorization[\s\S]*X-MobiShop-Installation/, "Managed Firebase must reuse installation-bound license authorization.");
assert.match(exporter, /secondaryColor[\s\S]*enabledPages/, "Exported app identity must include colors and selected pages.");
assert.match(exporter, /MobiShop_Push_Service::client_configuration/, "Every exported application must inherit the plugin's public Push bootstrap.");
...
assert.doesNotMatch(admin, /remove_submenu_page\(\s*'mobishop',\s*'mobishop-home-builder'/, "Public builders must not be unregistered while hiding sidebar links.");
assert.match(admin, /MobiShop_Setup_Wizard\(\) \)->is_complete/, "First visit must resolve setup state.");
assert.match(wizardTemplate, /mobishop-theme-gallery/, "Wizard must render a theme gallery.");
...
assert.match(wizardCss, /\.mobishop-setup-color-control\{display:flex!important;[^}]*align-items:stretch;[^}]*width:100%}/, "Each color picker and HEX code must stay in one compact row.");
assert.match(wizardCss, /\.mobishop-setup-color-control input\[type=text\]\{[^}]*width:auto!important;[^}]*flex:1 1 auto;[^}]*direction:ltr;/, "HEX fields must share the color row without wrapping and keep the hash on the left.");
assert.match(wizardTemplate, /if \( \$is_required \)[\s\S]*type="hidden"[\s\S]*data-required-page="1"[\s\S]*else[\s\S]*type="checkbox"[\s\S]*if \( ! \$is_required \)[\s\S]*mobishop-page-choice__switch/, "Required pages must stay enabled without rendering a checkbox or On/Off switch.");
...
assert.match(admin, /'page'\s*=>\s*'mobishop-splash-screen'[\s\S]*'setup_done'\s*=>\s*'1'/, "Finish must open Customize Your Pages after applying the setup.");
...
assert.match(dashboardTemplate, /mobishop-customer-journey__build-step[\s\S]*data-mobishop-app-build[\s\S]*mobishop-app-build__card-form[\s\S]*data-build-form-action[\s\S]*mobishop-app-build__card mobishop-app-build__card-button[\s\S]*data-build-action/, "The complete fourth launch card must be the single stateful build/download button.");
assert.match(shellTemplate, /data-build-persistent[\s\S]*data-mobishop-background-job="app-build"[\s\S]*data-build-modal[\s\S]*role="dialog"[\s\S]*data-build-message[\s\S]*data-build-progress[\s\S]*data-build-cancel/, "The build progress card must live in the permanent CMS shell and keep its action across page changes.");
assert.match(admin, /\$abandoned_import_state\s*=\s*MobiShop_Analytics::website_session_import_status\(\)/, "Every full CMS load must restore the saved Abandoned Cart job state.");
assert.match(shellTemplate, /data-mobishop-background-job-stack[\s\S]*data-mobishop-background-job="app-build"[\s\S]*data-mobishop-background-job="abandoned-carts"/, "The permanent shell must render Build and Abandoned Cart progress inside one shared stack.");
...
assert.match(admin, /if \( \$this->is_public_cms_page\( \$page \) \)[\s\S]*mobishop-app-builder/, "Every CMS page must load the persistent build tracker.");
...
assert.match(dashboardTemplate, /\.mobishop-app-build__card-button\s*\{[^}]*width:\s*100%;[^}]*height:\s*100%;[^}]*min-height:\s*120px;[^}]*background:\s*#2f806e;/, "The complete build card button must use the MobiShop color and fill the journey card without a blank strip.");
assert.doesNotMatch(dashboardTemplate, /mobishop-app-build__card-button[\s\S]{0,500}mobishop-customer-journey__number[^}]*>4</, "The complete build card button must not show a fourth-step number.");
assert.match(dashboardTemplate, /\.mobishop-app-build__card-label\s*\{[^}]*font-size:\s*clamp\([^}]*font-weight:\s*700;[^}]*text-wrap:\s*balance;/, "The build action label must use balanced, polished typography.");
...
assert.match(shellTemplate, /mobishop-cms-sidebar/, "Shell must expose the primary left navigation.");
assert.match(shellTemplate, /data-mobishop-cms-shell[\s\S]*\$show_page_tabs \? '' : ' hidden'/, "The one permanent page frame must hide outside Customize without being removed.");
assert.match(shellTemplate, /mobishop-cms-tabs/, "Customize Your Pages must preserve the existing top page tabs.");
assert.doesNotMatch(shellTemplate, /mobishop-cms-more/, "The obsolete More menu must not appear in the page header.");
assert.doesNotMatch(shellTemplate, /window\.prompt/, "Save Theme must not use the browser prompt.");
assert.match(shellTemplate, /data-mobishop-save-theme[\s\S]*data-mobishop-theme-modal[\s\S]*data-mobishop-theme-name[\s\S]*Save Theme/, "Every page header must open the centered themed Save Theme dialog.");
assert.doesNotMatch(shellTemplate.match(/<form class="mobishop-cms-save-theme"[\s\S]*?<\/form>/)?.[0] || "", /dashicons/, "The compact Save Theme button must not render a trailing icon.");
assert.match(shellCss, /\.mobishop-cms-save-theme \.button\s*\{[^}]*width:max-content;[^}]*min-width:0;/, "The Save Theme button must fit its label instead of keeping an oversized width.");
assert.match(shellCss, /\.mobishop-cms-save-theme \.button\s*\{[^}]*border-color:#236b59!important;[^}]*background:#2f806e!important;[^}]*color:#fff!important;/, "Save Theme must keep the MobiShop color instead of inheriting WordPress blue.");
assert.match(shellScript, /data-mobishop-theme-modal[\s\S]*mobishop_save_theme_name[\s\S]*requestSubmit/, "The themed Save Theme dialog must preserve unsaved builder fields before creating the named theme.");
assert.match(shellScript, /installPersistentCmsNavigation[\s\S]*mobishop_view[\s\S]*response\.json\(\)[\s\S]*persistentShellNode\(node, sidebar, shell\)[\s\S]*history\.pushState/, "CMS navigation must request only a view fragment and retain the shell plus every background-job card.");
...
assert.match(shellScript, /window\.mobishopCmsNavigatorInstalled/, "Persistent CMS navigation must be installed once even when page assets are refreshed.");
assert.match(shellCss, /#wpbody-content\.is-mobishop-page-loading \.mobishop-cms-sidebar\{[^}]*opacity:1;[^}]*pointer-events:auto;/, "The persistent sidebar must never show the adjacent page loading state.");
...
assert.match(savedThemesTemplate, /mobishop-theme-file[\s\S]*button-primary/, "Theme import must use the MobiShop file control and theme-colored action.");
...
assert.match(savedThemesTemplate, /data-saved-theme-snapshot[\s\S]*MobiShop_Setup_Wizard::setup_pages\(\)[\s\S]*data-saved-theme-page[\s\S]*data-saved-theme-dialog-frame/, "Saved Theme Preview must expose every application page in the real Flutter surface.");
assert.match(savedThemesTemplate, /data-saved-theme-page="splash"[\s\S]*MobiShop_Setup_Wizard::setup_pages/, "Saved Theme Preview must visibly expose its stored Splash before the application pages.");
assert.match(savedThemesScript, /page === 'splash'[\s\S]*splash:\s*snapshot\.splash[\s\S]*homePreviewEndpoint[\s\S]*categoryPreviewEndpoint[\s\S]*mobishop-flutter-preview-ready/, "Saved Theme Preview must send the stored Splash and page layouts to the real Flutter preview.");
assert.match(savedThemesTemplate, /mobishop-saved-theme-dialog__device[\s\S]*mobishop-saved-theme-dialog__screen[\s\S]*data-saved-theme-dialog-frame/, "Saved Theme Preview must reuse the clean Page Builder phone and screen structure.");
assert.match(wizardCss, /\.mobishop-saved-theme-dialog__device:before\{display:none;content:none\}[\s\S]*\.mobishop-saved-theme-dialog__screen\{[^}]*height:680px[\s\S]*zoom:\.85/, "Saved Theme Preview must remove the earpiece and show the full 800px Flutter surface at the Page Builder scale.");
...
assert.match(admin, /mobishopSavedThemePreview[\s\S]*flutterUrl[\s\S]*layoutPreviewBase[\s\S]*homePreviewEndpoint[\s\S]*categoryPreviewEndpoint/, "Saved Theme Preview must receive authenticated live-preview endpoints.");
assert.match(admin, /preview_snapshot[\s\S]*mobishopSetupThemePreview[\s\S]*flutterUrl[\s\S]*layoutPreviewBase[\s\S]*homePreviewEndpoint[\s\S]*categoryPreviewEndpoint/, "Setup themes must receive their exact generated layouts and authenticated Flutter preview endpoints.");
assert.match(admin, /mobishop-saved-themes[\s\S]*admin\/assets\/saved-themes\.js/, "The Saved Themes page must load its preview interactions.");
...
assert.match(admin, /saved_theme_redirect[\s\S]*save_current_theme[\s\S]*mobishop-saved-themes/, "Named page-header saves must persist the latest builder state and open Saved Themes.");
...
assert.match(admin, /function send_push_notification[\s\S]*mobishop_send_push_notification[\s\S]*mobishop_push_history/, "Push Notifications must validate, dispatch and record notifications.");
...
for (const layout of ["mobishop-data-table", "mobishop-order-list", "mobishop-customer-list", "mobishop-category-list"]) {
...
assert.match(admin, /function ai_insights_page[\s\S]*MobiShop_AI_Offer_Engine::recommendations/, "AI Offer Studio must have its own evidence-backed page.");
...
assert.match(wizardCss, /mobishop-theme-phone/, "Theme previews must have a detailed mobile mockup.");
assert.match(wizardCss, /mobishop-theme-modal__workspace[\s\S]*mobishop-theme-modal__device[\s\S]*iframe/, "Full theme preview must host the real Flutter application surface.");
assert.match(wizardTemplate, /mobishop-theme-modal__controls[\s\S]*mobishop-theme-modal__pages[\s\S]*mobishop-theme-modal__select/, "Theme selection must sit directly below the page buttons.");
assert.match(wizardCss, /\.mobishop-theme-modal__select\{[^}]*margin-top:0!important;[^}]*border-color:#236b59!important;[^}]*background:#2f806e!important/, "Theme selection must stay directly below the page buttons and retain the plugin action color independently of the chosen theme.");
assert.match(wizardCss, /mobishop-theme-modal__dialog\{[^}]*height:min\(850px,calc\(100vh - 40px\)\)[^}]*overflow:hidden/, "Theme preview must fit the popup viewport without an outer scrollbar.");
assert.match(wizardCss, /mobishop-theme-modal__device\{[^}]*--mobishop-theme-preview-scale:\.72[^}]*width:calc\(360px \* var\(--mobishop-theme-preview-scale\) \+ 10px\)[^}]*height:calc\(800px \* var\(--mobishop-theme-preview-scale\) \+ 10px\)[^}]*padding:0[^}]*overflow:hidden[^}]*border:5px solid #2f806e/, "The complete phone must keep one continuous green border outside the clipped curved screen.");
assert.match(wizardCss, /mobishop-theme-modal__device iframe\{[^}]*top:0[^}]*left:0[^}]*border-radius:21px/, "The scaled Flutter screen must sit inside the frame without covering its curved border.");
assert.match(service, /\$is_family_pop[\s\S]*\['cart_offset_y'\]\s*=\s*-2/, "Family Pop must optically align the action beside its search bar.");
assert.match(wizardCss, /mobishop-setup-apply\{[^}]*display:inline-flex!important[^}]*flex-direction:row!important[^}]*flex-wrap:nowrap!important[^}]*align-items:center!important[^}]*white-space:nowrap\}[\s\S]*mobishop-setup-apply \.dashicons\{[^}]*flex:0 0 20px[^}]*line-height:20px[^}]*transform:none!important/, "Finish label and check icon must remain centered beside each other on one row.");
assert.match(wizardCss, /mobishop-setup-color-control\{display:flex!important;[^}]*align-items:stretch;[^}]*width:100%}/, "Color picker and HEX code must stay compact and side by side.");
assert.match(wizardCss, /\.mobishop-setup-actions \.button\[hidden\]\{display:none!important\}/, "Apply Theme must remain hidden until the final setup step.");
assert.match(wizardCss, /--mobishop-setup-theme-color:#2f806e/, "Setup actions must expose a theme-driven color.");
const wizardScript = read("admin", "assets", "setup-wizard.js");
assert.match(wizardScript, /setProperty\('--mobishop-setup-theme-color', '#2f806e'\)/, "Setup actions must keep the MobiShop brand color.");
...
assert.match(wizardScript, /homePreviewEndpoint[\s\S]*categoryPreviewEndpoint[\s\S]*mobishop-preview-layout[\s\S]*mobishop-flutter-preview-ready/, "Built-in theme preview must render real Home, Category and page layouts in Flutter.");
...
assert.match(wizardScript, /mobishop-theme-modal__select/, "Full preview must allow selecting the previewed complete theme.");
assert.match(wizardScript, /document\.querySelector\('\.mobishop-setup-hero'\)/, "Wizard navigation must keep the Setup & Themes hero visible.");
...
assert.match(wizardScript, /function scrollWorkspaceTo[\s\S]*document\.querySelector\('#wpbody'\)[\s\S]*workspace\.scrollTo/, "Wizard steps must scroll only the MobiShop workspace.");
...
assert.match(shellCss, /box-shadow:inset 0 0 0 2px #2f806e/, "Header focus must use an inset MobiShop-colored ring.");
assert.match(shellCss, /\.mobishop-cms-setup-link\{[^}]*background:#236b59;[^}]*color:#fff\}/, "Quick Setup & Themes must use the dark MobiShop button color.");
assert.match(shellCss, /\.mobishop-cms-tabs>a:focus[^}]*color:#216e5e/, "Focused CMS tabs must keep readable dark-green text instead of turning white.");
assert.match(shellCss, /\.mobishop-cms-setup-link:focus[^}]*box-shadow:none!important/, "Quick Setup must not draw a square focus box after it is clicked.");
...
assert.doesNotMatch(shellStyleRules, /(?:^|\})\s*html(?:\.[^{]*)?\s*\{[^}]*overflow\s*:\s*hidden/i, "MobiShop must leave the native WordPress document scrollbar enabled.");
assert.match(shellCss, /body\.mobishop-cms-plugin-page #wpbody\{[^}]*height:calc\(100vh - 46px\)!important;[^}]*overflow-y:scroll!important;[^}]*scrollbar-gutter:stable;/, "The plugin workspace must keep an independent inner scrollbar on every CMS screen.");
assert.match(shellCss, /@media\(min-width:783px\)\{[\s\S]*body\.mobishop-cms-plugin-page #wpbody\{[^}]*height:calc\(100vh - 32px\)!important;[^}]*direction:ltr;/, "The desktop plugin scrollbar must fit below the WordPress toolbar.");
...
assert.match(admin, /admin_body_class[\s\S]*mobishop-cms-builder-screen/, "Builder pages must be marked for the fixed workspace before rendering.");
assert.doesNotMatch(shellStyleRules, /body\.mobishop-cms-builder-screen\s*\{[^}]*overflow\s*:\s*hidden/i, "Builder pages must leave WordPress document scrolling untouched.");
assert.doesNotMatch(shellStyleRules, /body\.mobishop-cms-builder-screen #wpwrap\s*\{[^}]*overflow\s*:\s*hidden/i, "The Builder must not clip the outer WordPress wrapper.");
assert.match(shellCss, /body\.mobishop-cms-builder-screen #wpcontent\{[^}]*overflow:hidden!important;/, "Builder content viewports must remain fixed while the internal card rail owns scrolling.");
assert.match(shellCss, /body\.mobishop-cms-builder-screen #wpbody\{[^}]*overflow-y:scroll!important;[^}]*scrollbar-gutter:stable;/, "Builder documents must retain the shared plugin scrollbar rail without moving the workspace.");
assert.match(shellCss, /body\.mobishop-cms-builder-screen #wpbody-content\{[^}]*height:calc\(100% - 24px\)!important;[^}]*margin-bottom:6px;[^}]*overflow:visible!important;/, "The fixed Builder frame must contain the complete phone without clipping the shared sidebar gutter.");
...
const wizardDom = new JSDOM(`<!doctype html><body>
  <div class="mobishop-setup-progress">${Array.from({ length: 5 }, () => "<span></span>").join("")}</div>
  <form class="mobishop-setup-form">
    <section class="mobishop-setup-step" data-step="1"><span data-step-number></span><input required value="Store"></section>
    <section class="mobishop-setup-step" data-step="2"><span data-step-number></span><input type="hidden" data-page-toggle="home" data-required-page="1"><input type="checkbox" data-page-toggle="category"><input type="checkbox" data-page-toggle="catalog" checked><input type="hidden" data-page-toggle="product" data-required-page="1"><input type="checkbox" data-page-toggle="wishlist" checked><input type="checkbox" data-page-toggle="account" checked></section>
    <section class="mobishop-setup-step" data-step="3"><span data-step-number></span><label class="mobishop-theme-card" data-theme-key="fashion" data-theme-name="Fashion" style="--theme-primary:#222222;--theme-soft:#eeeeee;--theme-ink:#111111;--theme-surface:#ffffff"><input type="radio" name="setup[theme]" required checked><button type="button" class="mobishop-theme-preview-button">Preview</button></label></section>
    <section class="mobishop-setup-step" data-step="4"><span data-step-number></span><h3 data-review-name></h3><strong data-review-theme></strong><span data-review-page="category"></span></section>
    <section class="mobishop-setup-step" data-step="5"><span data-step-number></span></section>
	...
	<div class="mobishop-setup-logo-preview"></div>
    ...
    <button type="button" class="mobishop-setup-back"></button>
    <button type="button" class="mobishop-setup-next"></button>
    <button type="submit" class="mobishop-setup-apply"></button>
  </form>
  <div class="mobishop-theme-modal" hidden>
    <button type="button" class="mobishop-theme-modal__close" data-theme-modal-close></button>
    ...
    <button type="button" class="mobishop-theme-modal__select"></button>
  </div>
</body>`, { runScripts: "outside-only", url: "https://example.test/wp-admin/admin.php" });
wizardDom.window.scrollTo = () => {};
wizardDom.window.mobishopSetupThemePreview = {
  flutterUrl: "https://example.test/wp-content/plugins/mobishop/admin/flutter-preview/index.html",
  layoutPreviewBase: "https://example.test/wp-json/mobishop/v1/page-layout/",
  homePreviewEndpoint: "https://example.test/wp-json/mobishop/v1/home-layout/preview",
  categoryPreviewEndpoint: "https://example.test/wp-json/mobishop/v1/category-page/preview",
...
const next = wizardDom.window.document.querySelector(".mobishop-setup-next");
next.click();
...
assert.equal(wizardDom.window.document.querySelector(".mobishop-setup-apply").hidden, false, "Build APK must appear only on the final setup step.");
...
wizardDom.window.document.querySelector(".mobishop-theme-preview-button").click();
assert.equal(wizardDom.window.document.querySelector(".mobishop-theme-modal").hidden, false, "Theme preview must open without applying the theme.");
...
const shellDom = new JSDOM(`<!doctype html><body><div class="mobishop-cms-shell"></div></body>`, { runScripts: "outside-only" });
shellDom.window.scrollTo = () => {};
shellDom.window.eval(read("admin", "assets", "cms-shell.js"));
assert.equal(shellDom.window.document.querySelector(".mobishop-cms-more"), null, "More menu must remain removed.");
...
const savedThemeDom = new JSDOM(`<!doctype html><body>
  <article data-saved-theme-card>
    ...
    <div class="mobishop-saved-theme-phone" data-saved-theme-phone><img src="theme-banner.jpg" alt=""></div>
...
savedThemeDom.window.mobishopSavedThemePreview = {
  flutterUrl: "https://example.test/wp-content/plugins/mobishop/admin/flutter-preview/index.html",
  layoutPreviewBase: "https://example.test/wp-json/mobishop/v1/page-layout/",
  homePreviewEndpoint: "https://example.test/wp-json/mobishop/v1/home-layout/preview",
  categoryPreviewEndpoint: "https://example.test/wp-json/mobishop/v1/category-page/preview",
