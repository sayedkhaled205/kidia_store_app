...
const plugin = read("mobishop.php");

const headerVersion = plugin.match(/^ \* Version:\s+([0-9]+\.[0-9]+\.[0-9]+)\s*$/m);
const runtimeVersion = plugin.match(/MOBISHOP_VERSION',\s*'([0-9]+\.[0-9]+\.[0-9]+)'/);
...
assert.match(
  plugin,
  /MOBISHOP_LICENSE_PUBLIC_KEY[\s\S]*pno\+qR490JO\/niHqlK82hXz0SwloDlwShxnmimmLQz0=/,
...
assert.match(manager, /X-MobiShop-Installation/, "Verification must bind to the stored installation.");
...
assert.match(
  shellCss,
  /\.mobishop-cms-sidebar\{[\s\S]*position:absolute;[\s\S]*inset-block-start:0;[\s\S]*inset-block-end:auto;[\s\S]*inset-inline-start:-16px;[\s\S]*width:236px;[\s\S]*height:calc\(100vh - 102px\);/,
...
assert.doesNotMatch(
  shellCss,
  /body\[class\*="mobishop"\] \.mobishop-cms-sidebar\{[\s\S]*position:fixed!important;/,
  "A global fixed viewport override must not pull the sidebar outside the workspace card."
);
assert.match(admin, /admin_post_mobishop_activate_license/, "The activation handler must be registered.");
assert.doesNotMatch(admin, /admin_post_mobishop_deactivate_license/, "Customers must not be able to deactivate a site-bound license.");
assert.match(admin, /enforce_license_gate/, "All configuration writes must be locked until license activation.");
assert.doesNotMatch(
  admin,
  /license_error['"]?\s*=>\s*__\(\s*'Activate your website license before using MobiShop/,
...
assert.doesNotMatch(
  dashboard,
  /Connect your store to MobiShop|Purchase and connect website/,
  "The license card must not repeat the external purchase and connection prompt."
);
assert.match(
  dashboard,
  /if \( \$license_active \)[\s\S]*else[\s\S]*class="mobishop-license-form"/,
  "The activation form must only render while the license is inactive."
);
assert.doesNotMatch(
  dashboard,
  /class="mobishop-setup-overview/,
...
assert.doesNotMatch(dashboard, /mobishop_deactivate_license/, "The license UI must not expose deactivation.");
...
assert.match(
  dashboard,
  /MobiShop is already installed\. Connect this website, activate its serial, then choose the wizard or manual setup\./,
