...
$GLOBALS['mobishop_splash_routes'] = array();
...
function register_rest_route( string $namespace, string $route, array $definition ): void { $GLOBALS['mobishop_splash_routes'][ $namespace . $route ] = $definition; }
...
function get_bloginfo( string $field ): string { unset( $field ); return 'MobiShop'; }
...
$endpoint = new MobiShop_Splash_Screen_Endpoint();
$endpoint->register_routes();
if ( ! isset( $GLOBALS['mobishop_splash_routes']['mobishop/v1/splash-screen'] ) ) { throw new RuntimeException( 'Splash route must register.' ); }
$response = $endpoint->get_settings();
if ( 2500 !== $response->data['duration_ms'] || 'MobiShop' !== $response->data['store_name'] ) { throw new RuntimeException( 'Splash defaults and saved settings must merge.' ); }
