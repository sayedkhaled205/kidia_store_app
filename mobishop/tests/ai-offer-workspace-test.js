...
const admin = readPlugin("admin", "class-mobishop-admin.php");
const engine = readPlugin(
  "includes",
  "class-mobishop-ai-offer-engine.php",
);
const bundleRecipes = readPlugin(
  "includes",
  "class-mobishop-bundle-recipes.php",
);
...
assert.doesNotMatch(
  aiInsights,
  /Optional manual bundle builder|mobishop_save_bundle_recipe/,
...
assert.match(
  bundleRecipes,
  /'page'\s*=>\s*'mobishop'[\s\S]*'view'\s*=>\s*'ai-insights'/,
...
assert.match(
  shellCss,
  /mobishop-ai-playbook-groups section>div\{[^}]*display:grid[^}]*repeat\(2,minmax\(0,1fr\)\)/,
  "Playbook buttons must use a stable grid instead of random wrapping.",
);
assert.match(
  shellCss,
  /mobishop-ai-decision-card>footer \.button\{[^}]*border-color:#2f806e!important[^}]*color:#236b59!important/,
  "Review product buttons must use the MobiShop brand palette.",
);

const pageMarkup = (id) => `
  <div class="mobishop-ai-page" id="${id}">
    <nav>
...
dom.window.document.dispatchEvent(
  new dom.window.CustomEvent("mobishop:cms-page-ready"),
);
