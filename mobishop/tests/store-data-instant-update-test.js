...
assert.match(storeDataPage, /data-mobishop-instant-filter="date"/);
assert.match(storeDataPage, /data-mobishop-reporting-filter="1"[\s\S]*data-mobishop-generate-store-reporting/);
assert.match(storeDataPage, /data-mobishop-instant-filter="products"/);
assert.match(storeDataPage, /data-mobishop-instant-filter="coupons"/);
assert.match(storeDataPage, /data-mobishop-instant-action="coupon-channel"/);
assert.match(shellCss, /mobishop-cms-js \[data-mobishop-instant-submit-fallback\]\{display:none!important\}/);
assert.match(
  shellScript,
  /isStoreDataView[\s\S]*canUseCache = !isStoreDataView[\s\S]*window\.mobishopCmsNavigate/,
  "Store Data navigation must always bypass stale view-cache entries.",
);
assert.match(
  shellScript,
  /mobishop:cms-page-ready[\s\S]*initLiveStoreData\(document\)/,
...
(async () => {
  const dom = new JSDOM(
    `<!doctype html><html><body>
      <form method="get" data-mobishop-instant-filter="products">
        ...
        <button type="submit" data-mobishop-instant-submit-fallback>Search</button>
      </form>
      <form method="get" data-mobishop-instant-filter="date" data-mobishop-reporting-filter="1" data-mobishop-reporting-ready="0">
        <input type="hidden" name="page" value="mobishop-store-data">
        ...
        <button type="button" data-mobishop-generate-store-reporting>Generate</button>
      </form>
      <form method="post" action="https://store.test/wp-admin/admin-post.php" data-mobishop-instant-action="coupon-channel">
        <input type="hidden" name="action" value="mobishop_set_coupon_channel">
        <input type="hidden" name="coupon_id" value="31">
        <select name="coupon_channel" data-mobishop-saved-value="all">
          <option value="all" selected>All</option>
          <option value="mobile">Mobile</option>
        </select>
        <button type="submit" data-mobishop-instant-submit-fallback>Save channel</button>
      </form>
    </body></html>`,
    {
      runScripts: "outside-only",
      url: "https://store.test/wp-admin/admin.php?page=mobishop&view=store-data&store_tab=products",
    },
  );

  const { window } = dom;
  window.mobishopCMSBackground = { ajaxUrl: "https://store.test/wp-admin/admin-ajax.php", storeNonce: "store-nonce" };
  window.scrollTo = () => {};
  const posts = [];
  window.fetch = async (url, options = {}) => {
    posts.push({ url: String(url), options });
    if (options.body && options.body.get("action") === "mobishop_generate_store_reporting") {
      return {
        ok: true,
        json: async () => ({
          success: true,
          data: { url: "https://store.test/wp-admin/admin.php?page=mobishop-store-data&store_tab=analytics&date_preset=last_7_days&report_ready=1" },
        }),
      };
    }
    return { ok: true };
  ...
  const navigations = [];
  window.mobishopCmsNavigate = async (url, options) => {
    navigations.push({ url: String(url), options });
  };

  assert.equal(
    window.document.documentElement.classList.contains("mobishop-cms-js"),
    true,
    "JavaScript-enhanced Store Data must hide manual fallback buttons.",
  );

  const productForm = window.document.querySelector('[data-mobishop-instant-filter="products"]');
  ...
  const generate = window.document.querySelector("[data-mobishop-generate-store-reporting]");
  generate.click();
  await new Promise((resolve) => window.setTimeout(resolve, 30));
  assert.equal(posts.length, 1, "Generate must explicitly prepare Store Data.");
  assert.equal(posts[0].options.body.get("action"), "mobishop_generate_store_reporting");
