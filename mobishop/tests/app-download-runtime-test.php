...
$GLOBALS['mobishop_redirect'] = array();
...
function wp_redirect( string $url, int $status, string $application ): bool {
	$GLOBALS['mobishop_redirect'] = compact( 'url', 'status', 'application' );
	return true;
}

require_once dirname( __DIR__ ) . '/includes/class-mobishop-app-exporter.php';

$method = new ReflectionMethod( MobiShop_App_Exporter::class, 'redirect_to_artifact' );
$method->setAccessible( true );
$result = $method->invoke(
	new MobiShop_App_Exporter(),
...
if ( 'https://api.codemagic.io/artifacts/signed-build-files.zip' !== $GLOBALS['mobishop_redirect']['url'] ) {
	throw new RuntimeException( 'The browser was not redirected to the Codemagic artifact.' );
}
if ( 302 !== $GLOBALS['mobishop_redirect']['status'] ) {
	throw new RuntimeException( 'The artifact redirect must be temporary.' );
}
if ( false !== $method->invoke( new MobiShop_App_Exporter(), 'http://unsafe.example.test/file.zip' ) ) {
