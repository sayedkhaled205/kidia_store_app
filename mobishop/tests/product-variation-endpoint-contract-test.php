 ...
 *
 * Run with: php mobishop/tests/product-variation-endpoint-contract-test.php
 */
...
$GLOBALS['mobishop_variation_routes'] = array();
...
function register_rest_route( string $namespace, string $route, array $definition ): void {
	$GLOBALS['mobishop_variation_routes'][ $namespace . $route ] = $definition;
}
...
function mobishop_variation_assert( bool $condition, string $message ): void {
	if ( ! $condition ) {
...
$endpoint = new MobiShop_Product_Variation_Endpoint();
$endpoint->register_routes();
mobishop_variation_assert( 1 === count( $GLOBALS['mobishop_variation_routes'] ), 'The public variation route must be registered.' );

$variations = $endpoint->get_variations( new WP_REST_Request( array( 'id' => 58 ) ) );
mobishop_variation_assert( 2 === count( $variations ), 'Every valid product variation must be returned.' );
$taxonomy = 'pa_%d8%a7%d9%84%d9%85%d9%82%d8%a7%d8%b3';
mobishop_variation_assert( $taxonomy === $variations[0]['attributes'][0]['taxonomy'], 'Percent-encoded Arabic taxonomies must not be stripped.' );
mobishop_variation_assert( $taxonomy === $variations[1]['attributes'][0]['taxonomy'], 'A uniquely resolvable pa_ key must use the parent taxonomy.' );
mobishop_variation_assert( '27' === $variations[1]['attributes'][0]['value'], 'The repaired attribute value must be preserved.' );
