...
const service = read(
  "includes",
  "class-mobishop-website-app-promotion.php",
);
...
assert.match(
  bootstrap,
  /class-mobishop-website-app-promotion\.php[\s\S]*MobiShop_Website_App_Promotion\(\)\)->register/,
...
for (const hook of [
  ...
  "wp_footer",
  "add_shortcode",
  "admin_post_mobishop_save_website_app_promotion",
  "wp_ajax_mobishop_app_promotion_event",
  "wp_ajax_nopriv_mobishop_app_promotion_event",
]) {
...
assert.match(
  admin,
  /mobishop-website-app-promotion[\s\S]*website_promotion[\s\S]*Promote App on Website/,
...
assert.match(
  shellCss,
  /\.mobishop-cms-sidebar__nav a:focus,[\s\S]*box-shadow:inset 0 0 0 2px #2f806e;/,
  "Sidebar keyboard focus must use the MobiShop brand color instead of WordPress blue.",
);
assert.match(
  admin,
...
assert.match(
  template,
  /data-preview-device="mobile"[\s\S]*class="is-active" data-preview-device="desktop"[\s\S]*mobishop-promotion-device is-desktop/,
  "The campaign builder must open on the laptop preview while keeping mobile available.",
);
assert.match(
  template,
  /MobiShop_Website_App_Promotion::preview_url\(\)[\s\S]*data-promotion-site-frame/,
  "The campaign preview must load the current customer's real website.",
);
assert.doesNotMatch(
  template,
  /mobishop-promotion-store__(?:hero|cards|lines)/,
...
assert.match(
  publicScript,
  /if \(!root\)[\s\S]*document\.createElement\("div"\)[\s\S]*mobishopAppPromoRoot[\s\S]*append\(root\)/,
  "The frontend must create its own promotion root when a theme omits the footer slot.",
);
assert.match(
  service,
  /data-mobishop-app-promo-config[\s\S]*wp_json_encode\([\s\S]*frontend_config/,
...
assert.match(
  publicScript,
  /readConfig[\s\S]*data-mobishop-app-promo-config[\s\S]*bootAttempts[\s\S]*setTimeout\(boot, 250\)/,
...
assert.match(
  adminCss,
  /\.mobishop-app-promotion-workspace[\s\S]*grid-template-columns:minmax\(0,1fr\) 370px/,
  "The editor must keep a dedicated sticky preview rail.",
);
assert.match(
  adminCss,
  /is-desktop-preview \.mobishop-app-promotion-workspace\{grid-template-columns:minmax\(0,1fr\)\}[\s\S]*is-desktop-preview \.mobishop-app-promotion-preview\{grid-row:1;position:relative;top:auto;width:100%;max-width:none\}/,
  "Desktop preview mode must place the laptop preview at full page width above the editor.",
);
assert.match(
  adminCss,
  /\.mobishop-promotion-device-switch\{[\s\S]*gap:8px[\s\S]*\.mobishop-promotion-device-switch button\{[\s\S]*width:36px[\s\S]*min-width:36px/,
  "Mobile and laptop controls must have separate non-overlapping click targets.",
);
assert.match(
  adminCss,
  /\.mobishop-promotion-device\.is-desktop \.mobishop-promotion-browser\{width:1366px;height:768px/,
  "The desktop website preview must use a real 1366 by 768 laptop viewport.",
);
assert.match(
  adminCss,
  /\.mobishop-promotion-live-site iframe\{[\s\S]*width:100%;height:100%;border:0/,
...
assert.match(
  publicCss,
  /\.mobishop-app-promo\{[\s\S]*--mobishop-app-promo-soft:color-mix/,
  "Campaigns moved outside the root must keep an opaque themed surface.",
);
assert.match(
  service,
  /data-mobishop-app-promo-placement="after_header"[\s\S]*data-mobishop-app-promo-placement="before_footer"/,
...
const adminMarkup = `
    <div data-promotion-admin>
      ...
      <div class="mobishop-app-promotion-state">Campaigns paused</div>
...
adminDom.window.document.dispatchEvent(
  new adminDom.window.CustomEvent("mobishop:cms-page-ready"),
);
assert.equal(
...
assert.ok(
  adminDom.window.document.querySelector(
    "[data-preview-output] .mobishop-app-promo--smart_banner",
  ),
  "The editor preview must render the same campaign component used by the live website.",
);
assert.equal(
  adminDom.window.document.querySelector(".mobishop-preview-promo"),
  null,
...
const dom = new JSDOM(
  `<!doctype html><body>
    ...
    <div data-mobishop-app-promo-slot="inline" data-mobishop-app-promo-placement="after_header"></div>
    <div data-mobishop-app-promo-slot="inline" data-mobishop-app-promo-placement="before_footer"></div>
    <footer id="footer">Store footer</footer>
    <div data-mobishop-app-promo-root></div>
  </body>`,
  {
...
dom.window.MobiShopAppPromotion = {
...
const qrDom = new JSDOM(
  `<!doctype html><body><div data-mobishop-app-promo-root></div></body>`,
  {
    runScripts: "outside-only",
...
qrDom.window.MobiShopAppPromotion = {
  ajaxUrl: "https://store.example/wp-admin/admin-ajax.php",
  nonce: "nonce",
  loggedIn: false,
  page: { type: "home", path: "/" },
  settings: {
    ...
    smart_url: "",
    deep_link: "",
    qr_url: "https://download.example/mobishop",
    audience_devices: "all",
    audience_users: "all",
    page_target: "all",
    custom_paths: "",
...
lateRootDom.window.MobiShopAppPromotion = JSON.parse(
  JSON.stringify(qrDom.window.MobiShopAppPromotion),
);
lateRootDom.window.MobiShopAppPromotion.settings.page_target = "home";

lateRootDom.window.eval(publicScript);
const lateRoot = lateRootDom.window.document.createElement("div");
lateRoot.dataset.mobishopAppPromoRoot = "";
...
missingRootDom.window.MobiShopAppPromotion = JSON.parse(
  JSON.stringify(qrDom.window.MobiShopAppPromotion),
);
missingRootDom.window.MobiShopAppPromotion.settings.page_target = "home";
...
const qrOnlyConfig = JSON.parse(JSON.stringify(qrDom.window.MobiShopAppPromotion));
...
embeddedConfig.dataset.mobishopAppPromoConfig = "";
embeddedConfig.textContent = JSON.stringify(qrOnlyConfig);
delayedConfigDom.window.document.body.append(embeddedConfig);
delayedConfigDom.window.document.dispatchEvent(
  new delayedConfigDom.window.Event("mobishop:app-promotion-config-ready"),
);

const messageOnlyDom = new JSDOM(
  "<!doctype html><body><main>Campaign without a published store URL</main><div data-mobishop-app-promo-root></div></body>",
  {
    runScripts: "outside-only",
    url: "https://store.example/",
  },
...
messageOnlyDom.window.MobiShopAppPromotion = {
  ajaxUrl: "https://store.example/wp-admin/admin-ajax.php",
  nonce: "nonce",
  loggedIn: false,
  page: { type: "home", path: "/" },
...
const privateQrTestDom = new JSDOM(
  "<!doctype html><body><main>Private campaign test</main><div data-mobishop-app-promo-root></div></body>",
  {
    runScripts: "outside-only",
    url: "https://store.example/?mobishop_app_promotion_test=desktop_qr",
  },
);
...
privateQrTestDom.window.MobiShopAppPromotion = JSON.parse(
  JSON.stringify(messageOnlyDom.window.MobiShopAppPromotion),
);
privateQrTestDom.window.MobiShopAppPromotion.testCampaign = "desktop_qr";
privateQrTestDom.window.MobiShopAppPromotion.labels = {
...
const privateFloatingTestDom = new JSDOM(
  "<!doctype html><body><main>Private floating test</main><div data-mobishop-app-promo-root></div></body>",
  {
    runScripts: "outside-only",
    url: "https://store.example/?mobishop_app_promotion_test=floating_button",
  },
);
privateFloatingTestDom.window.matchMedia = () => ({ matches: false });
privateFloatingTestDom.window.navigator.sendBeacon = () => true;
privateFloatingTestDom.window.MobiShopAppPromotion = JSON.parse(
  JSON.stringify(privateQrTestDom.window.MobiShopAppPromotion),
);
privateFloatingTestDom.window.MobiShopAppPromotion.testCampaign =
  "floating_button";
privateFloatingTestDom.window.eval(publicScript);

const mobileConflictDom = new JSDOM(
  "<!doctype html><body><main>Mobile campaign orchestration</main><div data-mobishop-app-promo-root></div></body>",
  {
    runScripts: "outside-only",
    url: "https://store.example/",
  },
...
mobileConflictDom.window.MobiShopAppPromotion = JSON.parse(
  JSON.stringify(messageOnlyDom.window.MobiShopAppPromotion),
);
mobileConflictDom.window.MobiShopAppPromotion.settings.smart_banner.enabled = false;
mobileConflictDom.window.MobiShopAppPromotion.settings.bottom_sheet = {
  enabled: true,
  trigger: "immediate",
  delay: 0,
  style: "compact",
};
mobileConflictDom.window.MobiShopAppPromotion.settings.popup = {
  enabled: true,
  trigger: "delay",
  delay: 0,
  style: "split",
...
setTimeout(() => {
  const smartBanner = dom.window.document.querySelector(
    ".mobishop-app-promo--smart_banner",
  );
  assert.ok(smartBanner, "The enabled Smart Banner must render.");
  assert.equal(
    smartBanner.querySelector(".mobishop-app-promo__copy strong").textContent,
    "MobiShop",
    "The live campaign must use saved app identity.",
  );
  assert.equal(
    smartBanner.querySelector(".mobishop-app-promo__coupon").textContent,
    "APP10",
    "The app-only coupon must appear in the website campaign.",
  );
  assert.ok(
    dom.window.document.querySelector(
      '[data-mobishop-app-promo-slot="inline"] .mobishop-app-promo--inline_banner',
    ),
  ...
  assert.equal(
    ...
    dom.window.document.querySelector(
      '[data-mobishop-app-promo-placement="after_header"]',
    ),
  ...
  assert.equal(
    ...
    "https://download.example/mobishop",
    "The generated desktop QR code must use the custom QR destination.",
  );
  assert.equal(
    desktopQr.querySelector(".mobishop-app-promo__action").href,
    "https://download.example/mobishop",
  ...
  assert.ok(
    missingRootDom.window.document.querySelector(
      "[data-mobishop-app-promo-root] .mobishop-app-promo--desktop_qr",
    ),
  ...
  assert.equal(
    delayedConfigDom.window.document.querySelector(
      "[data-mobishop-app-promo-root]",
    ).dataset.mobishopAppPromoStatus,
    "ready",
  ...
  assert.equal(
    messageOnlyBanner.querySelector(".mobishop-app-promo__action"),
    null,
  ...
  assert.ok(
    messageOnlyQr.querySelector(".mobishop-app-promo__qr-placeholder"),
  ...
  const messageOnlyFloating = messageOnlyDom.window.document.querySelector(
    ".mobishop-app-promo--floating_button",
  );
  assert.ok(
    messageOnlyFloating,
  ...
  const privateQrCard = privateQrTestDom.window.document.querySelector(
    ".mobishop-app-promo--desktop_qr.is-test",
  );
  assert.ok(
    privateQrCard,
  ...
  assert.ok(
    privateQrCard.querySelector(".mobishop-app-promo__qr-placeholder"),
    "A destinationless QR test must use an honest placeholder instead of a fake code.",
  );
  assert.equal(
    privateQrCard.querySelector(".mobishop-app-promo__action"),
    null,
  ...
  const privateFloatingButton =
    privateFloatingTestDom.window.document.querySelector(
      ".mobishop-app-promo--floating_button.is-test",
    );
  assert.ok(
  ...
  assert.ok(
    mobileConflictDom.window.document.querySelector(
      '[data-mobishop-app-promotion="bottom_sheet"]',
    ),
    "The enabled mobile bottom sheet must render.",
  );
  assert.equal(
    mobileConflictDom.window.document.querySelector(
      '[data-mobishop-app-promotion="popup"]',
    ),
    null,
