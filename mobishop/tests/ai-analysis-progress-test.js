...
assert.match(
  shellStyles,
  /\.mobishop-ai-progress-actions \.button-primary\{[^}]*background:#2f806e!important[^}]*color:#fff!important/,
  "The AI background action must keep the MobiShop color after the progress card leaves the AI page container.",
);
assert.match(
  shellStyles,
  /\.mobishop-ai-progress-actions \.button-primary:hover,\.mobishop-ai-progress-actions \.button-primary:focus\{[^}]*background:#236b59!important/,
  "The AI background action hover and focus states must use the darker MobiShop color.",
);

const dom = new JSDOM(
  `<!doctype html><body>
    <div class="mobishop-ai-progress-overlay is-docked" data-ai-progress-overlay>
  ...
  </body>`,
  {
    ...
    url: "https://example.com/wp-admin/admin.php?page=mobishop-ai-insights",
  },
);

const { window } = dom;
window.__MOBISHOP_AI_PROGRESS_TEST__ = true;
...
const hook = window.MobiShopAiProgressTest;
...
assert.equal(independentDock.dataset.mobishopBackgroundJob, "generate-offers");
assert.equal(
...
hook.render(overlay, {
  ...
  result_url: "https://example.com/wp-admin/admin.php?page=mobishop-ai-insights&ai_ready=1",
});
assert.equal(overlay.dataset.aiComplete, "1");
assert.match(overlay.querySelector("[data-ai-cancel-button]").textContent, /OK/);
assert.equal(overlay.querySelector("[data-ai-cancel-button]").classList.contains("is-confirm"), true, "Completed AI actions must use the MobiShop confirmation color.");
...
assert.equal(overlay.dataset.mobishopBackgroundJob, "generate-offers");
assert.equal(
  overlay,
  sameOverlay,
...
assert.deepEqual(
  JSON.parse(window.localStorage.getItem("mobishop_ai_progress_position_v1")),
  { left: 320, top: 240 },
