...
function completedCard(document) {
  ...
  card.className = "mobishop-cart-import-state is-complete";
  card.dataset.mobishopBackgroundJob = "abandoned-carts";
...
const dom = new JSDOM("<!doctype html><body><div data-mobishop-background-job-stack></div></body>", {
...
dom.window.document.querySelector("[data-mobishop-background-job-stack]").appendChild(firstCard);
...
assert.match(dom.window.localStorage.getItem("mobishopCompletedBackgroundJobNotices"), /abandoned-carts:1722535200/);
...
dom.window.document.querySelector("[data-mobishop-background-job-stack]").appendChild(repeatedCard);
...
dom.window.document.querySelector("[data-mobishop-background-job-stack]").appendChild(nextCard);
dom.window.document.dispatchEvent(new dom.window.CustomEvent("mobishop:cms-page-ready"));
