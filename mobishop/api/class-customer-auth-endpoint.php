...
final class MobiShop_Customer_Auth_Endpoint {

	private const SESSION_META_KEY = '_mobishop_customer_sessions_v1';
	private const TOKEN_PREFIX     = 'kma1';
	private const MAX_SESSIONS     = 5;
	private const SOCIAL_TTL       = 600;
	private const SOCIAL_CODE_TTL  = 180;
	private const SOCIAL_SCHEME    = 'mobishop-store-app';
	...
	public function login( WP_REST_Request $request ) {
		...
		if ( is_wp_error( $user ) || ! $user instanceof WP_User || ! $this->is_allowed_customer( $user ) ) {
			return new WP_Error(
				'mobishop_auth_invalid_credentials',
	...
	public function register_customer( WP_REST_Request $request ) {
		...
		if ( ! function_exists( 'wc_create_new_customer' ) ) {
			return new WP_Error(
				'mobishop_auth_woocommerce_unavailable',
		...
		if ( is_wp_error( $user_id ) ) {
			...
			return new WP_Error(
				'mobishop_auth_registration_failed',
		...
		if ( class_exists( 'MobiShop_Analytics' ) ) {
			MobiShop_Analytics::mark_mobile_registration( $user->ID );
		}
	...
	public function social_start( WP_REST_Request $request ) {
		...
		if ( '' === $request_id ) {
			return new WP_Error(
				'mobishop_auth_social_start_failed',
		...
		$callback_url = add_query_arg(
			'request_id',
			$request_id,
			rest_url( 'mobishop/v1/auth/social/callback' )
		);
	...
	public function capture_social_login( $user_id, $provider = null ): void {
		...
		if ( ! preg_match( '/^mobishop:([A-Za-z0-9_-]{43,128})$/', $tracker, $matches ) ) {
			return;
		}
	...
	public function social_callback( WP_REST_Request $request ) {
		...
		if ( ! $this->is_social_secret( $code ) || ! $this->is_social_secret( $state ) ) {
			return new WP_Error(
				'mobishop_auth_social_not_completed',
	...
	public function social_exchange( WP_REST_Request $request ) {
		...
		if ( ! is_array( $handoff ) ||
			...
			return new WP_Error(
				'mobishop_auth_social_invalid_handoff',
	...
	private function issue_session_response( WP_User $user ) {
		...
		$lifetime = (int) apply_filters(
			'mobishop_auth_session_lifetime',
			30 * DAY_IN_SECONDS,
			$user
		);
		...
		update_user_meta( $user->ID, self::SESSION_META_KEY, $sessions );
		if ( class_exists( 'MobiShop_Analytics' ) ) {
			MobiShop_Analytics::mark_mobile_customer( $user->ID );
		}
	...
	private function token_from_request( WP_REST_Request $request ): string {
		$token = trim( (string) $request->get_header( 'x-mobishop-session' ) );
		if ( '' !== $token ) {
	...
	private function token_from_server(): string {
		$token = isset( $_SERVER['HTTP_X_MOBISHOP_SESSION'] )
			? wp_unslash( $_SERVER['HTTP_X_MOBISHOP_SESSION'] )
			: '';
	...
	private function is_mobile_rest_request(): bool {
		$route = isset( $GLOBALS['wp']->query_vars['rest_route'] )
			...
			: '';
		if ( str_starts_with( $route, '/wc/store/' ) || str_starts_with( $route, '/mobishop/v1/' ) ) {
			return true;
		}
		$uri = isset( $_SERVER['REQUEST_URI'] )
			? (string) wp_unslash( $_SERVER['REQUEST_URI'] )
			: '';
		return str_contains( $uri, '/wp-json/wc/store/' ) || str_contains( $uri, '/wp-json/mobishop/v1/' );
	}
	...
	private function render_social_authorize_url( string $provider, string $callback_url, string $request_id ) {
		if ( ! in_array( $provider, array( 'google', 'facebook' ), true ) || ! shortcode_exists( 'nextend_social_login' ) ) {
			return new WP_Error(
				'mobishop_auth_social_unavailable',
		...
		$shortcode = sprintf(
			'[nextend_social_login provider="%s" redirect="%s" trackerdata="mobishop:%s"]',
			$provider,
		...
		if ( ! is_array( $target ) || ! is_array( $home ) || ! $this->same_origin_parts( $target, $home ) ) {
			return new WP_Error(
				'mobishop_auth_social_invalid_url',
	...
	private function social_key( string $type, string $value ): string {
		return 'mobishop_auth_social_' . sanitize_key( $type ) . '_' . substr(
