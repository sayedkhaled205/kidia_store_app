...
final class MobiShop_Customer_Account_Endpoint {
	private const ACCOUNT_PHONE_META = '_mobishop_account_phone';
	...
	public function update_profile( WP_REST_Request $request ) {
		...
		if ( array_key_exists( 'email', $values ) ) {
			...
			if ( ! is_email( $email ) ) {
				return new WP_Error(
					'mobishop_account_invalid_email',
		...
		if ( is_wp_error( $updated ) ) {
			return new WP_Error(
				'mobishop_account_update_failed',
	...
	public function update_address( WP_REST_Request $request ) {
		...
		if ( ! in_array( $type, array( 'billing', 'shipping' ), true ) ) {
			return new WP_Error(
				'mobishop_account_invalid_address',
	...
	private function current_customer() {
		...
		if ( ! class_exists( 'WC_Customer' ) ) {
			return new WP_Error(
				'mobishop_account_woocommerce_unavailable',
	...
	private function support_payload(): array {
		...
		$email = apply_filters(
			'mobishop_cms_support_email',
			$default_email
		);
		$phone = apply_filters(
			'mobishop_cms_support_phone',
			(string) get_option( 'woocommerce_store_phone', '' )
		);
		$whatsapp = apply_filters(
			'mobishop_cms_support_whatsapp',
			$phone
		);
		$contact_url = apply_filters(
			'mobishop_cms_support_url',
			home_url( '/contact-us/' )
		);
		return array(
	...
	private function persistence_error(): WP_Error {
		return new WP_Error(
			'mobishop_account_persistence_failed',
