(() => {
  ...
  const config = window.MobiShopWebsiteAnalytics || {};
  if (!config.endpoint) {
    return;
  }

  const cookieName = "mobishop_website_client";
  ...
  const sessionKey = "mobishop_website_session";
  const sessionId =
    window.sessionStorage?.getItem(sessionKey) ||
    `session-${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`;
  const queueKey = "mobishop_website_analytics_queue_v1";
  let flushing = false;
  let memoryQueue = [];
