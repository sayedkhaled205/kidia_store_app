...
final class MobiShop_Analytics {
	...
	private const DB_OPTION  = 'mobishop_analytics_db_version';
	private const MOBILE_META = '_mobishop_customer';
	private const WEBSITE_META = '_mobishop_website_customer';
	private const ORIGIN_META = '_mobishop_customer_origin';
	private const WEBSITE_IMPORT_OPTION = 'mobishop_website_cart_import_v4';
	private const WEBSITE_IMPORT_HOOK = 'mobishop_import_website_cart_sessions';
	private const WEBSITE_IMPORT_LOCK = 'mobishop_website_cart_import_lock_v4';
	...
	public function enqueue_website_tracker(): void {
		...
		wp_enqueue_script(
			'mobishop-website-analytics',
			MOBISHOP_URL . 'public/assets/website-analytics.js',
			array(),
			MOBISHOP_VERSION,
			true
		);
		wp_add_inline_script(
			'mobishop-website-analytics',
			'window.MobiShopWebsiteAnalytics=' . wp_json_encode(
				array(
					'endpoint' => esc_url_raw( rest_url( 'mobishop/v1/analytics/website-event' ) ),
					'context'  => $context,
				)
			) . ';',
			'before'
	...
	public function record_website_event_request( WP_REST_Request $request ) {
		...
		if ( ! in_array( $event, $allowed, true ) ) {
			return new WP_Error(
				'mobishop_website_analytics_event_invalid',
		...
		if ( '' === $client_id ) {
			return new WP_Error(
				'mobishop_website_analytics_client_missing',
		...
		if ( ! $this->allow_request( 'web-' . $client_id, 240 ) || ! $this->allow_request( 'web-ip-' . $this->request_ip(), 900 ) ) {
			return new WP_Error(
				'mobishop_website_analytics_rate_limited',
		...
		if ( 'failed' === $result ) {
			return new WP_Error(
				'mobishop_website_analytics_write_failed',
	...
	public function record_event( WP_REST_Request $request ) {
		...
		if ( ! in_array( $event, self::EVENTS, true ) ) {
			return new WP_Error(
				'mobishop_analytics_event_invalid',
		...
		if ( '' === $client_id ) {
			return new WP_Error(
				'mobishop_analytics_client_missing',
				__( 'A valid analytics client id is required.', 'mobishop' ),
				array( 'status' => 400 )
			);
		}
		if ( ! $this->allow_request( $client_id ) || ! $this->allow_request( 'ip-' . $this->request_ip(), 600 ) ) {
			return new WP_Error(
				'mobishop_analytics_rate_limited',
				__( 'Too many analytics events were received.', 'mobishop' ),
				array( 'status' => 429 )
			);
		}
		...
		if ( 'failed' === $result ) {
			return new WP_Error(
				'mobishop_analytics_write_failed',
				__( 'The analytics event could not be stored.', 'mobishop' ),
				array( 'status' => 500 )
			);
		}
	...
	public function capture_paid_order( $order_or_id ): void {
		...
		if ( ! $order instanceof WC_Order || 'mobile' === $order->get_meta( '_mobishop_order_source' ) ) {
			return;
		}
		if ( 'yes' === $order->get_meta( '_mobishop_website_analytics_recorded' ) ) {
			return;
		}
		...
		$order->update_meta_data( '_mobishop_website_analytics_recorded', 'yes' );
		$order->save();
	}
	...
	public function capture_completed_order( $order ): void {
		...
		$source  = 'mobile' === $order->get_meta( '_mobishop_order_source' ) ? 'mobile' : 'website';
	...
	public static function customer_sources( int $user_id ): array {
		...
		$mobile  = '1' === (string) get_user_meta( $user_id, self::MOBILE_META, true )
			|| metadata_exists( 'user', $user_id, '_mobishop_customer_sessions_v1' );
	...
	private static function reporting_source_sql( string $alias, string $source, bool $refund_aware = false ): string {
		...
		global $wpdb;
		$queries = array(
			"SELECT post_id AS order_id FROM {$wpdb->postmeta} WHERE meta_key = '_mobishop_order_source' AND meta_value = 'mobile'",
		);
		$hpos_meta = $wpdb->prefix . 'wc_orders_meta';
		if ( self::reporting_table_exists( $hpos_meta ) ) {
			$queries[] = "SELECT order_id FROM {$hpos_meta} WHERE meta_key = '_mobishop_order_source' AND meta_value = 'mobile'";
		}
		$mobile_orders = 'SELECT DISTINCT order_id FROM (' . implode( ' UNION ALL ', $queries ) . ') AS mobishop_sources';
	...
	private static function collect_orders_in_period( array $args, int $from, int $to, string $source, bool $stop_at_older_order ): array {
		...
		do {
			...
			foreach ( $batch as $order ) {
				...
				$order_source = 'mobile' === (string) $order->get_meta( '_mobishop_order_source' )
					? 'mobile'
					: 'website';
	...
	public static function commerce_cache_key( int $from, int $to, string $source = 'all' ): string {
		$source = in_array( $source, array( 'website', 'mobile' ), true ) ? $source : 'all';
		return 'mobishop_commerce_snapshot_v7_' . md5( $from . '|' . $to . '|' . $source );
	}
	...
	private static function summary_cache_key( int $from, int $to, string $source = 'all' ): string {
		$source = in_array( $source, array( 'website', 'mobile' ), true ) ? $source : 'all';
		return 'mobishop_analytics_summary_v3_' . md5( $from . '|' . $to . '|' . $source );
	}
	...
	private static function schedule_website_session_import( bool $from_runner = false ): void {
		if ( ! $from_runner && function_exists( 'as_has_scheduled_action' ) && as_has_scheduled_action( self::WEBSITE_IMPORT_HOOK, array(), 'mobishop' ) ) {
			return;
		}
		if ( function_exists( 'as_enqueue_async_action' ) ) {
			as_enqueue_async_action( self::WEBSITE_IMPORT_HOOK, array(), 'mobishop', false );
			return;
		}
	...
	private function website_event_cache_key( string $event, string $client_id, int $object_id, string $label ): string {
		return 'mobishop_web_event_' . substr( hash( 'sha256', $event . '|' . $client_id . '|' . $object_id . '|' . $label ), 0, 28 );
	}
	...
	private function website_client_id(): string {
		...
		$client_id = $this->identifier( $_COOKIE['mobishop_website_client'] ?? '' );
		if ( '' !== $client_id ) {
			return $client_id;
		}
		$client_id = 'web-' . wp_generate_uuid4();
		$_COOKIE['mobishop_website_client'] = $client_id;
		if ( ! headers_sent() ) {
			setcookie(
				'mobishop_website_client',
				$client_id,
				array(
	...
	private function allow_request( string $key, int $limit = 240 ): bool {
		$transient = 'mobishop_analytics_rate_' . substr( hash( 'sha256', $key ), 0, 24 );
	...
	private static function events_table(): string {
		global $wpdb;
		return $wpdb->prefix . 'mobishop_events';
	}

	private static function carts_table(): string {
		global $wpdb;
		return $wpdb->prefix . 'mobishop_carts';
	}
}
