 ...
 *
 * The engine intentionally uses first-party WooCommerce and MobiShop analytics
...
final class MobiShop_AI_Offer_Engine {
	...
	public static function recommendations( int $from, int $to, string $source = 'all' ): array {
		...
		$summary = MobiShop_Analytics::summary( $from, $to, $source );
		...
		$offers = apply_filters( 'mobishop_ai_offer_recommendations', $offers, $summary, $source, $from, $to );
	...
	public static function rotation_segments( int $from, int $to, string $source = 'all' ): array {
		...
		$commerce   = MobiShop_Analytics::commerce_snapshot( $from, $to, $source );
	...
	private static function recommendation_cache_key( int $from, int $to, string $source ): string {
		$source = in_array( $source, array( 'website', 'mobile' ), true ) ? $source : 'all';
		return 'mobishop_ai_offers_v6_' . md5( $from . '|' . $to . '|' . $source );
	}

	private static function rotation_cache_key( int $from, int $to, string $source ): string {
		$source = in_array( $source, array( 'website', 'mobile' ), true ) ? $source : 'all';
		return 'mobishop_ai_rotation_v3_' . md5( $from . '|' . $to . '|' . $source );
	}
	...
	public static function signal_catalog(): array {
		...
		$signals = apply_filters( 'mobishop_ai_offer_signals', $signals );
