...
final class MobiShop_App_Exporter {

	private const STATE_OPTION = 'mobishop_app_export_state_v1';
	private const ASYNC_HOOK = 'mobishop_process_app_build';
	private const START_TIMEOUT = 180;

	public function register(): void {
		add_action( 'admin_post_mobishop_export_app', array( $this, 'handle_export' ) );
		add_action( 'admin_post_mobishop_build_app', array( $this, 'handle_build' ) );
		add_action( 'admin_post_mobishop_download_apk', array( $this, 'handle_download_apk' ) );
		add_action( 'wp_ajax_mobishop_app_build_start', array( $this, 'handle_build_start' ) );
		add_action( 'wp_ajax_mobishop_app_build_status', array( $this, 'handle_build_status' ) );
		add_action( 'wp_ajax_mobishop_app_build_cancel', array( $this, 'handle_build_cancel' ) );
	...
	public static function configuration_hash(): string {
		$identity = ( new MobiShop_Setup_Wizard() )->identity();
		$payload  = array(
			'plugin_version' => defined( 'MOBISHOP_VERSION' ) ? MOBISHOP_VERSION : '',
			'site_url'       => home_url( '/' ),
			'identity'       => $identity,
			'push'           => MobiShop_Push_Service::client_configuration(),
		);
	...
	public static function manifest(): array {
		...
		return array(
			...
			'application'  => array(
				...
				'slug'          => $slug,
				'androidPackage' => 'app.mobishop.' . $package_key,
				'iosBundleId'   => 'app.mobishop.' . str_replace( '_', '', $package_key ),
				'language'      => (string) ( $identity['language'] ?? 'en' ),
				'direction'     => (string) ( $identity['direction'] ?? 'ltr' ),
				'appIconShape'  => (string) ( $identity['app_icon_shape'] ?? 'rounded_square' ),
				'fontFamily'    => (string) ( $identity['font_family'] ?? 'system' ),
				...
				'enabledPages'  => array_values( is_array( $identity['enabled_pages'] ?? null ) ? $identity['enabled_pages'] : array_keys( MobiShop_Setup_Wizard::setup_pages() ) ),
			),
			'store'        => array(
				'url'      => home_url( '/' ),
				'apiBase'  => rest_url( 'mobishop/v1/' ),
				'homeApi'  => rest_url( 'mobishop/v1/home-layout' ),
			),
			'push'         => $push,
			'build'        => array(
				'dartDefines' => array(
					...
					'PUSH_CONFIG_URL' => rest_url( 'mobishop/v1/push/config' ),
				),
	...
	public function handle_export(): void {
		...
		check_admin_referer( 'mobishop_export_app', 'mobishop_export_nonce' );
	...
	public function handle_build(): void {
		...
		check_admin_referer( 'mobishop_build_app', 'mobishop_build_nonce' );
	...
	/**
	 * Queues a real Android APK build with the MobiShop build service.
	 *
	...
	public function queue_build() {
		$license = new MobiShop_License_Manager();
	...
	private function dispatch_build( string $request_token ) {
		...
		$push_project = MobiShop_Push_Service::provision_project();
	...
	public function refresh_build( bool $force = false ) {
		...
		$response = ( new MobiShop_License_Manager() )->build_service_request( $path, $method );
		if ( is_wp_error( $response ) ) {
			return $response;
		}
	...
	public function handle_build_status(): void {
		...
		check_ajax_referer( 'mobishop_app_build_status', 'nonce' );
	...
	public function handle_build_start(): void {
		...
		check_ajax_referer( 'mobishop_build_app', 'nonce' );
	...
	public function handle_build_cancel(): void {
		...
		check_ajax_referer( 'mobishop_app_build_cancel', 'nonce' );
	...
	public function handle_download_apk(): void {
		...
		check_admin_referer( 'mobishop_download_apk', 'mobishop_download_nonce' );
	...
	private function build_request_payload( array $manifest, bool $provision_push ): array {
		...
		$version     = defined( 'MOBISHOP_VERSION' ) ? MOBISHOP_VERSION : '1.0.0';
	...
	private function redirect_to_artifact( string $url ): bool {
		...
		nocache_headers();
		return wp_redirect( $url, 302, 'MobiShop Build Download' ); // phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect
	}

	public function download(): void {
		$manifest  = self::manifest();
		$app_slug  = sanitize_file_name( (string) $manifest['application']['slug'] );
		$file_name = 'mobishop-' . $app_slug . '-build-package.zip';
		$files     = array(
		...
		$file_name = 'mobishop-' . $app_slug . '-app-config.json';
		$this->mark_exported( $file_name );
	...
	private function readme( array $manifest ): string {
		...
		return implode(
			"\n",
			array(
				'MobiShop application build package',
				'===================================',
				'',
				'This package is generated by the WordPress plugin and is ready for the MobiShop build pipeline.',
				...
				'',
				'MobiShop creates a private Push connection for this application during the build.',
	...
	private function normalize_build_response( array $response, string $hash, ?array $previous = null ): array {
		...
		foreach ( $artifacts as $artifact ) {
			...
			if ( '' !== $artifact_url && preg_match( '/mobishop-build-files\.zip(?:$|\?)/i', $artifact_name . $artifact_url ) ) {
				$download_url = $artifact_url;
				$file_name    = $artifact_name ?: 'mobishop-build-files.zip';
				break;
			}
		...
		if ( '' === $download_url && ! empty( $apk_fallback ) ) {
			...
			$file_name    = (string) ( $apk_fallback[1] ?: 'mobishop-app.apk' );
		}
