...
final class MobiShop_Website_App_Promotion {

	public const OPTION = 'mobishop_website_app_promotion_v1';
	private const METRICS_OPTION = 'mobishop_website_app_promotion_metrics_v1';
	private const PREVIEW_ACTION = 'mobishop_website_app_promotion_preview';
	private const PREVIEW_QUERY = 'mobishop_app_promotion_preview';
	private const TEST_ACTION = 'mobishop_website_app_promotion_test';
	private const TEST_QUERY = 'mobishop_app_promotion_test';
	private const TEST_NONCE_QUERY = 'mobishop_app_promotion_test_nonce';
	...
	public function register(): void {
		...
		add_shortcode( 'mobishop_app_promo', array( $this, 'shortcode' ) );
		add_action( 'admin_post_mobishop_save_website_app_promotion', array( $this, 'save' ) );
		add_action( 'wp_ajax_mobishop_app_promotion_event', array( $this, 'track_event' ) );
		add_action( 'wp_ajax_nopriv_mobishop_app_promotion_event', array( $this, 'track_event' ) );
	}

	/** @return array<string,mixed> */
	public static function defaults(): array {
		$identity = class_exists( 'MobiShop_Setup_Wizard' )
			? ( new MobiShop_Setup_Wizard() )->identity()
			: array();
	...
	public function enqueue_assets(): void {
		...
		wp_enqueue_script(
			'mobishop-website-app-promotion',
			MOBISHOP_URL . 'public/assets/website-app-promotion.js',
			array( 'mobishop-qrcode' ),
			MOBISHOP_VERSION . '-' . (string) filemtime( MOBISHOP_PATH . 'public/assets/website-app-promotion.js' ),
			true
		);
		wp_localize_script(
			'mobishop-website-app-promotion',
			'MobiShopAppPromotion',
	...
	private function frontend_config(): array {
		return array(
			...
			'nonce'        => wp_create_nonce( 'mobishop_app_promotion_event' ),
			'page'         => $this->page_context(),
			'loggedIn'     => is_user_logged_in(),
			'shortcode'    => '[mobishop_app_promo]',
	...
	public function render_after_header_slot(): void {
		...
		if (
			...
			echo '<div class="mobishop-app-promo-slot" data-mobishop-app-promo-slot="inline" data-mobishop-app-promo-placement="after_header"></div>';
		}
	}
	...
	public function render_footer(): void {
		...
		if (
			...
			echo '<div class="mobishop-app-promo-slot" data-mobishop-app-promo-slot="inline" data-mobishop-app-promo-placement="before_footer"></div>';
		}
		echo '<div class="mobishop-app-promo-root" data-mobishop-app-promo-root></div>';
		echo '<script type="application/json" data-mobishop-app-promo-config>';
	...
	public function shortcode(): string {
		...
		return '<div class="mobishop-app-promo-slot" data-mobishop-app-promo-slot="inline" data-mobishop-app-promo-placement="shortcode"></div>';
	}
	...
	public function save(): void {
		...
		check_admin_referer( 'mobishop_save_website_app_promotion', 'mobishop_promotion_nonce' );
		...
		wp_safe_redirect(
			add_query_arg(
				array(
					'page'            => 'mobishop-website-app-promotion',
					'promotion_saved' => '1',
				),
				admin_url( 'admin.php' )
			)
	...
	public function track_event(): void {
		check_ajax_referer( 'mobishop_app_promotion_event', 'nonce' );
		...
		$limit_key = 'mobishop_promo_' . md5( $address . '|' . $event . '|' . gmdate( 'Y-m-d-H' ) );
