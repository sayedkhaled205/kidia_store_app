<?php
/**
 * MobiShop license activation and verification.
 *
 * @package MobiShop
 */
...
final class MobiShop_License_Manager {
	...
	public function activate( string $license_key ) {
		...
		$response = $this->request(
			...
			array(
				...
				'site_url'        => home_url( '/' ),
				'plugin_version'  => MOBISHOP_VERSION,
				'nonce'           => $nonce,
			)
		);
	...
	public function status(): array {
		...
		return array(
			...
			'signature_configured' => defined( 'MOBISHOP_LICENSE_PUBLIC_KEY' ) && '' !== (string) MOBISHOP_LICENSE_PUBLIC_KEY,
		);
	}
	...
	/**
	 * Sends an authenticated request to the MobiShop APK build service.
	 *
	...
	public function build_service_request( string $path = '', string $method = 'GET', array $body = array() ) {
		...
		$base_url = (string) apply_filters( 'mobishop_build_api_base_url', self::BUILD_API_BASE_URL );
		$args     = array(
			'timeout' => 30,
			'method'  => $method,
			'headers' => array(
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error( 'build_service_unavailable', __( 'Could not contact the MobiShop build service. Please try again.', 'mobishop' ) );
		}
		...
		if ( $status < 200 || $status >= 300 || ! is_array( $data ) || ( isset( $data['ok'] ) && empty( $data['ok'] ) ) ) {
			$message = $this->remote_error_message(
				$data,
				__( 'The MobiShop build service rejected the request.', 'mobishop' )
			);
	 ...
	 *
	 * Firebase credentials stay on the MobiShop platform. The WordPress plugin
	...
	/**
	 * Sends an installation-bound request to the MobiShop Firebase service.
	 *
	 * @param string              $path   Firebase resource path, relative to /firebase.
	 * @param string              $method HTTP method.
	 * @param array<string,mixed> $body   Optional JSON request body.
	...
	public function firebase_service_request( string $path = '', string $method = 'GET', array $body = array() ) {
		...
		$base_url = (string) apply_filters( 'mobishop_firebase_api_base_url', self::FIREBASE_API_BASE_URL );
		$args     = array(
		...
		$response = wp_remote_request( untrailingslashit( $base_url ) . ( '/' === $path ? '' : $path ), $args );
		if ( is_wp_error( $response ) ) {
			return new WP_Error( 'push_service_unavailable', __( 'Could not contact the MobiShop Firebase service. Please try again.', 'mobishop' ) );
		}
		...
		if ( $status < 200 || $status >= 300 || ! is_array( $data ) || ( isset( $data['ok'] ) && empty( $data['ok'] ) ) ) {
			$message = is_array( $data ) && isset( $data['error']['message'] )
				...
				: __( 'The MobiShop Firebase service rejected the request.', 'mobishop' );
			$code = is_array( $data ) && isset( $data['error']['code'] )
				? sanitize_key( (string) $data['error']['code'] )
				: 'push_request_failed';
	 ...
	 * the service-account credentials used by MobiShop to send messages.
	 *
	...
	/**
	 * Extracts both the MobiShop error envelope and Laravel validation errors.
	 *
	 ...
	 *
	 * The production public key is supplied via MOBISHOP_LICENSE_PUBLIC_KEY.
	...
	private function validate_proof( $proof, string $nonce ) {
		...
		$public_key = defined( 'MOBISHOP_LICENSE_PUBLIC_KEY' )
			? (string) MOBISHOP_LICENSE_PUBLIC_KEY
			: '';
		if ( '' !== $public_key ) {
