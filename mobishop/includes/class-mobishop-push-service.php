 ...
 *
 * Every application receives an isolated Firebase project during the MobiShop
...
final class MobiShop_Push_Service {
	...
	private const LEGACY_SETTINGS_OPTION = 'mobishop_push_provider_settings';
	private const DEVICES_OPTION  = 'mobishop_push_devices_v1';
	...
	public function register(): void {
		...
		add_action( 'mobishop_trigger_push_automation', array( $this, 'trigger_automation' ), 10, 2 );
		add_action( 'mobishop_dispatch_automation_push', array( $this, 'dispatch_automation' ), 10, 2 );
		add_action( 'mobishop_scan_push_automations', array( $this, 'scan_automations' ) );
		...
		if ( ! wp_next_scheduled( 'mobishop_scan_push_automations' ) ) {
			wp_schedule_event( time() + 5 * MINUTE_IN_SECONDS, 'hourly', 'mobishop_scan_push_automations' );
		}
	}
	...
	public static function client_configuration(): array {
		...
		return array(
			...
			'configUrl'            => rest_url( 'mobishop/v1/push/config' ),
			'registrationUrl'      => rest_url( 'mobishop/v1/push/devices' ),
			'eventsUrl'            => rest_url( 'mobishop/v1/push/events' ),
	...
	public static function connection_status(): array {
		$license_active = class_exists( 'MobiShop_License_Manager' )
		...
		return array(
			...
			'reason'         => $connected
				...
				: ( $license_active
					...
					: __( 'Activate the MobiShop license, then build the application to enable Push.', 'mobishop' ) ),
	...
	public static function project_status( bool $force = false ): array {
		...
		$response = ( new MobiShop_License_Manager() )->firebase_service_request( 'project' );
	...
	public static function provision_project() {
		...
		$response = ( new MobiShop_License_Manager() )->firebase_service_request(
			'project',
			'POST',
			array(
				'app_name'        => sanitize_text_field( (string) ( $identity['app_name'] ?? get_bloginfo( 'name' ) ) ),
				'android_package' => 'app.mobishop.' . $package_key,
				'ios_bundle_id'   => 'app.mobishop.' . str_replace( '_', '', $package_key ),
			)
		);
	...
	public static function validate_connection() {
		return ( new MobiShop_License_Manager() )->firebase_service_request(
			'messages',
			'POST',
			array(
				'target_type'  => 'topic',
				'target'       => 'mobishop-connection-check',
				'title'        => 'MobiShop Push test',
				'body'         => 'Firebase connection validation',
				'data'         => array( 'source' => 'mobishop', 'type' => 'connection_check' ),
				'validate_only' => true,
			)
		);
	}
	...
	public function register_device( WP_REST_Request $request ) {
		...
		if ( strlen( $token ) < 16 || strlen( $token ) > 4096 ) {
			return new WP_Error( 'mobishop_push_token_invalid', __( 'A valid device token is required.', 'mobishop' ), array( 'status' => 400 ) );
		}
		...
		update_option( 'mobishop_push_registered_devices', count( $devices ), false );
	...
	public function record_event( WP_REST_Request $request ) {
		...
		if ( '' === $notification_id || ! in_array( $event, array( 'delivered', 'opened', 'converted' ), true ) ) {
			return new WP_Error( 'mobishop_push_event_invalid', __( 'A valid notification event is required.', 'mobishop' ), array( 'status' => 400 ) );
		}
	 ...
	 * do_action( 'mobishop_trigger_push_automation', $trigger, $context ).
	 *
	...
	public function trigger_automation( string $trigger, array $context = array() ): void {
		...
		wp_schedule_single_event( $when, 'mobishop_dispatch_automation_push', array( $payload, $log_key ) );
	...
	public function dispatch_automation( array $payload, string $log_key = '' ): void {
		...
		if ( $user_id > 0 && ! empty( $payload['automation']['stop_on_purchase'] ) ) {
			$last_purchase = absint( get_user_meta( $user_id, '_mobishop_last_purchase_at', true ) );
	...
	public function scan_automations(): void {
		...
		$carts = MobiShop_Analytics::abandoned_carts(
	...
	public function suppress_after_purchase( int $order_id ): void {
		...
		if ( $order instanceof WC_Order && $order->get_user_id() > 0 ) {
			update_user_meta( $order->get_user_id(), '_mobishop_last_purchase_at', time() );
		}
	}
