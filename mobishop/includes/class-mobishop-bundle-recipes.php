...
final class MobiShop_Bundle_Recipes {

	private const OPTION = 'mobishop_bundle_recipes_v1';

	public function register(): void {
		add_action( 'rest_api_init', array( $this, 'register_routes' ) );
		add_action( 'admin_post_mobishop_save_bundle_recipe', array( $this, 'save_recipe' ) );
	...
	public function get_bundle( WP_REST_Request $request ) {
		...
		if (
			...
			return new WP_Error( 'mobishop_bundle_not_found', __( 'Bundle not found.', 'mobishop' ), array( 'status' => 404 ) );
		}
	...
	public function claim_bundle( WP_REST_Request $request ) {
		...
		if ( ! is_array( $recipe ) || 'published' !== ( $recipe['status'] ?? 'draft' ) || ! self::is_active( $recipe ) ) {
			return new WP_Error( 'mobishop_bundle_unavailable', __( 'Bundle is not currently available.', 'mobishop' ), array( 'status' => 404 ) );
		}
		$channel = sanitize_key( (string) $request->get_param( 'channel' ) );
		$channel = in_array( $channel, array( 'website', 'mobile' ), true ) ? $channel : 'mobile';
		if ( ! in_array( (string) ( $recipe['channel'] ?? 'all' ), array( 'all', $channel ), true ) ) {
			return new WP_Error( 'mobishop_bundle_channel', __( 'Bundle is not available on this sales channel.', 'mobishop' ), array( 'status' => 403 ) );
		}
	...
	public function save_recipe(): void {
		...
		check_admin_referer( 'mobishop_save_bundle_recipe', 'mobishop_bundle_nonce' );
	...
	public function validate_bundle_coupon( bool $valid, WC_Coupon $coupon ): bool {
		...
		$recipe_id = sanitize_key( (string) get_post_meta( $coupon->get_id(), '_mobishop_bundle_recipe_id', true ) );
		if ( '' === $recipe_id ) {
			return true;
		}
		$recipes = self::all();
	...
	private function ensure_bundle_coupon( array $recipe ) {
		if ( ! class_exists( 'WC_Coupon' ) ) {
			return new WP_Error( 'mobishop_bundle_coupon_unavailable', __( 'WooCommerce coupons are unavailable.', 'mobishop' ), array( 'status' => 503 ) );
		}
		...
		$code = 'MOBISHOP-BUNDLE-' . strtoupper( substr( hash( 'sha256', (string) $recipe['id'] ), 0, 10 ) );
		...
		if ( $coupon_id <= 0 ) {
			return new WP_Error( 'mobishop_bundle_coupon_failed', __( 'Bundle discount could not be created.', 'mobishop' ), array( 'status' => 500 ) );
		}
		update_post_meta( $coupon_id, '_mobishop_bundle_recipe_id', (string) $recipe['id'] );
		if ( class_exists( 'MobiShop_Coupon_Channel' ) ) {
			MobiShop_Coupon_Channel::set( $coupon_id, (string) ( $recipe['channel'] ?? 'all' ) );
		}
		$recipes = self::all();
	...
	private function create_or_update_bundle_product( array $recipe ): int {
		...
		$id = $product->save();
		if ( $id > 0 ) {
			update_post_meta( $id, '_mobishop_bundle_recipe_id', $recipe['id'] );
		}
		return $id;
	}
}
