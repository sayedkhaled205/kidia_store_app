...
if ( class_exists( 'MobiShop_Layout_Store', false ) ) {
	return;
}

final class MobiShop_Layout_Store {

	private const OPTION_NAME = 'mobishop_home_layout_v5';

	private const LEGACY_OPTION_NAME = 'mobishop_home_layout_v4';

	private const EXCLUSIONS_OPTION = 'mobishop_home_layout_exclusions_v2';
	...
	/** PatPat-inspired MobiShop content order used by fresh stores and the preset action. */
	public function get_mobishop_patpat_layout(): array {
		$types = array( 'image_banner', 'category_grid', 'promo_strip', 'countdown', 'product_carousel', 'banner_grid', 'product_carousel', 'brand_carousel', 'product_grid' );
		$names = array( 'MobiShop Main Offer', 'Shop by Category', 'MobiShop Benefits', 'Flash Sale', 'New Arrivals', 'Seasonal Collections', 'Best Sellers', 'Our Brands', 'More for Kids' );
		...
		foreach ( $types as $index => $type ) {
			$block = ! empty( $pool[ $type ] ) ? array_shift( $pool[ $type ] ) : MobiShop_Block_Registry::create( $type, $index + 1 );
			...
			switch ( $type ) {
				case 'app_header':
					...
					if ( empty( $block['settings']['title'] ) ) { $block['settings']['title'] = 'MobiShop'; }
					break;
				case 'image_banner':
					$block['settings']['aspect_ratio'] = 1; $block['settings']['border_radius'] = 18; $block['settings']['button_label'] = __( 'SHOP NOW', 'mobishop' );
				...
				case 'category_grid':
					...
					break;
				case 'promo_strip':
					$block['settings']['text'] = __( 'New MobiShop offers every day', 'mobishop' ); $block['settings']['background_color'] = '#EAF6F2'; $block['settings']['text_color'] = '#1F6F61';
					break;
				case 'countdown':
					$block['settings']['title'] = __( 'Flash sale', 'mobishop' ); $block['settings']['ends_at'] = gmdate( 'Y-m-d\TH:i', time() + ( 3 * ( defined( 'DAY_IN_SECONDS' ) ? DAY_IN_SECONDS : 86400 ) ) );
					break;
				case 'banner_grid':
					$block['settings']['title'] = __( 'Seasonal collections', 'mobishop' ); $block['settings']['columns'] = 2; $block['settings']['gap'] = 10; $block['settings']['border_radius'] = 16; $block['settings']['image_fit'] = 'cover';
					break;
				case 'brand_carousel':
					$block['settings']['title'] = __( 'Our brands', 'mobishop' ); $block['settings']['item_width'] = 82;
					break;
			}
			if ( in_array( $type, array( 'product_carousel', 'product_grid' ), true ) ) {
				$block['settings']['source'] = 'on_sale';
				if ( 4 === $index ) { $block['settings']['source'] = 'latest'; }
				if ( 6 === $index ) { $block['settings']['source'] = 'featured'; }
				$block['settings']['title'] = 4 === $index ? __( 'New arrivals', 'mobishop' ) : ( 6 === $index ? __( 'Best sellers', 'mobishop' ) : __( 'More for kids', 'mobishop' ) );
	...
	private function create_default_layout(): array {
		...
		foreach ( $types as $index => $type ) {
			$block = MobiShop_Block_Registry::create(
				$type,
				$index + 1
			);
			...
			if ( 'quick_links' === $type ) {
				...
				$block['settings'] = array_merge( $block['settings'], array( 'aspect_ratio' => 1, 'border_radius' => 18, 'button_label' => __( 'SHOP NOW', 'mobishop' ) ) );
	...
	private function normalize_layout(
		...
		foreach ( array_values( $layout ) as $index => $raw_block ) {
			...
			if (
				empty( $type )
				|| ! MobiShop_Block_Registry::exists( $type )
			) {
				continue;
			}
			...
			if ( empty( $id ) ) {
				$id = MobiShop_Block_Registry::generate_id(
					$type
				);
			}
			...
			if ( null === $item ) {
				...
				if ( null !== $source ) {
					...
					$block['settings'] = wp_parse_args(
						...
						MobiShop_Block_Registry::defaults( $type )
					);
					$block['status'] = 'draft';
					$preserved[] = $block;
					continue;
				...
				$block['settings'] = MobiShop_Block_Registry::defaults(
					$type
				);
				$block['status'] = 'draft';
				$preserved[] = $block;
	...
	private function get_default_name(
		...
		$definition = MobiShop_Block_Registry::get(
			$type
		);
