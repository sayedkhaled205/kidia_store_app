...
if ( class_exists( 'MobiShop_Countdown_Block', false ) ) {
	return;
}

final class MobiShop_Countdown_Block extends MobiShop_Block {
	...
	public function get_description(): string {
		return __( 'Offer countdown timer.', 'mobishop' );
	}
    ...
    <div class="mobishop-builder-grid">
		...
		<div class="mobishop-builder-field"><label>Background Color</label><input type="color" name="blocks[<?php echo esc_attr( $index ); ?>][settings][background_color]" value="<?php echo esc_attr( $settings['background_color'] ); ?>"></div>
		<div class="mobishop-builder-field"><label>Text Color</label><input type="color" name="blocks[<?php echo esc_attr( $index ); ?>][settings][text_color]" value="<?php echo esc_attr( $settings['text_color'] ); ?>"></div>
		<div class="mobishop-builder-field"><label>Timer Box Color</label><input type="color" name="blocks[<?php echo esc_attr( $index ); ?>][settings][box_color]" value="<?php echo esc_attr( $settings['box_color'] ); ?>"></div>
		<div class="mobishop-builder-field mobishop-countdown-setting mobishop-countdown-setting--layout"><label><?php esc_html_e( 'Layout Style', 'mobishop' ); ?></label><select class="mobishop-countdown-layout" name="blocks[<?php echo esc_attr( $index ); ?>][settings][layout_style]"><?php foreach ( array( 'cards' => 'Cards', 'circles' => 'Circles', 'flip_clock' => 'Flip Clock', 'minimal_inline' => 'Minimal Inline', 'split_labels' => 'Split Labels' ) as $value => $label ) : ?><option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $settings['layout_style'] ); ?>><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select><span class="mobishop-countdown-layout-preview is-<?php echo esc_attr( $settings['layout_style'] ); ?>" aria-hidden="true"><i data-countdown-unit="days">08<small>D</small></i><i data-countdown-unit="hours" <?php echo $settings['show_hours'] ? '' : 'hidden'; ?>>12<small>H</small></i><i data-countdown-unit="minutes" <?php echo $settings['show_minutes'] ? '' : 'hidden'; ?>>24<small>M</small></i><i data-countdown-unit="seconds" <?php echo $settings['show_seconds'] ? '' : 'hidden'; ?>>36<small>S</small></i></span></div>
		<div class="mobishop-builder-field mobishop-countdown-setting mobishop-countdown-setting--visible-units">
			<label><?php esc_html_e( 'Visible time units', 'mobishop' ); ?></label>
			<select class="mobishop-countdown-visible-units" name="blocks[<?php echo esc_attr( $index ); ?>][settings][visible_units]">
				<?php foreach ( array(
					'days' => __( 'Days only', 'mobishop' ),
					'days_hours' => __( 'Days + Hours', 'mobishop' ),
					'days_hours_minutes' => __( 'Days + Hours + Minutes', 'mobishop' ),
					'days_hours_minutes_seconds' => __( 'Days + Hours + Minutes + Seconds', 'mobishop' ),
				) as $value => $label ) : ?>
					<option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $settings['visible_units'] ); ?>><?php echo esc_html( $label ); ?></option>
				<?php endforeach; ?>
			</select>
		</div>
		<div class="mobishop-countdown-actions">
			<div class="mobishop-builder-field"><label>Action Type</label><select name="blocks[<?php echo esc_attr( $index ); ?>][settings][action_type]"><?php foreach ( array( '' => 'No Action', 'product' => 'Product', 'category' => 'Category', 'collection' => 'Collection', 'brand' => 'Brand', 'brands' => 'All Brands', 'search' => 'Search', 'external' => 'External URL' ) as $value => $label ) : ?><option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $settings['action_type'] ); ?>><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select></div>
			<div class="mobishop-builder-field"><label>Action Value</label><input type="text" name="blocks[<?php echo esc_attr( $index ); ?>][settings][action_value]" value="<?php echo esc_attr( $settings['action_value'] ); ?>"></div>
		</div>
