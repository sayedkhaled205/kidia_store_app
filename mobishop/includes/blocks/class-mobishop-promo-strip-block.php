...
if ( class_exists( 'MobiShop_Promo_Strip_Block', false ) ) {
	return;
}

final class MobiShop_Promo_Strip_Block extends MobiShop_Block {
	...
	<div class="mobishop-builder-field mobishop-promo-action-setting mobishop-promo-action-setting--text">
	...
	<div class="mobishop-builder-field mobishop-promo-action-setting mobishop-promo-action-setting--width"><label><?php esc_html_e( 'Width (%)', 'mobishop' ); ?></label><input type="number" min="10" max="100" name="blocks[<?php echo esc_attr( (string) $index ); ?>][settings][width]" value="<?php echo esc_attr( (string) $settings['width'] ); ?>" placeholder="100"></div>
	<div class="mobishop-builder-field mobishop-promo-action-setting mobishop-promo-action-setting--height"><label><?php esc_html_e( 'Height', 'mobishop' ); ?></label><input type="number" min="20" max="240" name="blocks[<?php echo esc_attr( (string) $index ); ?>][settings][height]" value="<?php echo esc_attr( (string) $settings['height'] ); ?>" placeholder="Auto"></div>
	<div class="mobishop-builder-field mobishop-promo-action-setting mobishop-promo-action-setting--enable-transition"><label><?php esc_html_e( 'Rotating Messages', 'mobishop' ); ?></label><label class="mobishop-page-master-toggle"><input class="mobishop-promo-transition-toggle" type="checkbox" name="blocks[<?php echo esc_attr( (string) $index ); ?>][settings][enable_transition]" value="1" <?php checked( true, $settings['enable_transition'] ); ?>><span class="mobishop-toggle-state"></span></label></div>
	<div class="mobishop-promo-transition-fields" <?php echo $settings['enable_transition'] ? '' : 'hidden'; ?>>
		<div class="mobishop-promo-messages" data-next-index="<?php echo esc_attr( (string) count( $settings['messages'] ) ); ?>">
			<?php foreach ( $settings['messages'] as $message_index => $message ) : ?><div class="mobishop-promo-message-row"><input type="text" name="blocks[<?php echo esc_attr( (string) $index ); ?>][settings][messages][<?php echo esc_attr( (string) $message_index ); ?>]" value="<?php echo esc_attr( $message ); ?>"><button type="button" class="button mobishop-remove-promo-message"><?php esc_html_e( 'Remove', 'mobishop' ); ?></button></div><?php endforeach; ?>
		</div>
		<button type="button" class="button mobishop-add-promo-message"><?php esc_html_e( 'Add Message', 'mobishop' ); ?></button>
		<div class="mobishop-builder-grid">
			<div class="mobishop-builder-field"><label><?php esc_html_e( 'Transition Effect', 'mobishop' ); ?></label><select name="blocks[<?php echo esc_attr( (string) $index ); ?>][settings][transition_effect]"><?php foreach ( array( 'fade' => 'Fade', 'slide_up' => 'Slide Up', 'slide_left' => 'Slide Left', 'scale' => 'Scale' ) as $value => $label ) : ?><option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $settings['transition_effect'] ); ?>><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select></div>
			<div class="mobishop-builder-field"><label><?php esc_html_e( 'Change Every (seconds)', 'mobishop' ); ?></label><input type="number" min="1" max="60" name="blocks[<?php echo esc_attr( (string) $index ); ?>][settings][change_every]" value="<?php echo esc_attr( (string) $settings['change_every'] ); ?>"></div>
			<div class="mobishop-builder-field"><label><?php esc_html_e( 'Transition Duration (ms)', 'mobishop' ); ?></label><input type="number" min="100" max="5000" step="100" name="blocks[<?php echo esc_attr( (string) $index ); ?>][settings][transition_duration]" value="<?php echo esc_attr( (string) $settings['transition_duration'] ); ?>"></div>
		</div>
	</div>

	<div class="mobishop-builder-field mobishop-promo-action-setting mobishop-promo-action-setting--background">
	...
	<div class="mobishop-builder-field mobishop-promo-action-setting mobishop-promo-action-setting--text-color">
		...
		<div class="mobishop-builder-field mobishop-promo-action-setting mobishop-promo-action-setting--action-type">
		...
		<div class="mobishop-builder-field mobishop-promo-action-setting mobishop-promo-action-setting--action-value">
