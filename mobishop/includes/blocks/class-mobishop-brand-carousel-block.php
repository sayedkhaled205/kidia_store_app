...
if ( class_exists( 'MobiShop_Brand_Carousel_Block', false ) ) {
	return;
}

final class MobiShop_Brand_Carousel_Block extends MobiShop_Block {
    ...
    <div class="mobishop-builder-grid">
		...
		<div class="mobishop-builder-field"><label>Layout</label><select name="blocks[<?php echo esc_attr( $index ); ?>][settings][layout]"><option value="carousel" <?php selected( 'carousel', $settings['layout'] ); ?>>Horizontal Row</option><option value="grid" <?php selected( 'grid', $settings['layout'] ); ?>>Grid</option></select></div>
		<div class="mobishop-builder-field"><label>Columns</label><input type="number" min="2" max="6" name="blocks[<?php echo esc_attr( $index ); ?>][settings][columns]" value="<?php echo esc_attr( (string) $settings['columns'] ); ?>"></div>
		<div class="mobishop-builder-field"><label>Logo Shape</label><select name="blocks[<?php echo esc_attr( $index ); ?>][settings][image_shape]"><option value="circle" <?php selected( 'circle', $settings['image_shape'] ); ?>>Circle</option><option value="rounded" <?php selected( 'rounded', $settings['image_shape'] ); ?>>Rounded</option><option value="square" <?php selected( 'square', $settings['image_shape'] ); ?>>Square</option></select></div>
		<div class="mobishop-builder-field"><label>Gap</label><input type="number" min="0" max="32" name="blocks[<?php echo esc_attr( $index ); ?>][settings][gap]" value="<?php echo esc_attr( (string) $settings['gap'] ); ?>"></div>
		<div class="mobishop-builder-field"><label><input type="checkbox" name="blocks[<?php echo esc_attr( $index ); ?>][settings][show_names]" value="1" <?php checked( true, $settings['show_names'] ); ?>> Show Brand Names</label></div>
