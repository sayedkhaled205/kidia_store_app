...
function runMergeControlsContractTest() {
  ...
  assert.match(categoryPreview, /margin_bottom[^\n]+margin_top/, "Category preview must pull elements together instead of adding positive margins.");
	...
	assert.match(chromeTemplate, /kidia-chrome-setting--section-layout[\s\S]*kidia-section-layout-component/, "Header and Footer must feed their five settings into the shared Section Layout component.");
	assert.match(settingsSections, /classList\.contains\("kidia-section-layout-component"\)[\s\S]*addHeadings\(container\)/, "The shared Section Layout component must also initialize inside Header and Footer.");
  ...
  assert.match(pageStore, /product_wishlist_background_size[^\n]+array\(\), 20, 64/, "Wishlist backgrounds must support compact sizes down to 20px.");
	...
	const categoryShared = ["margin_top", "margin_bottom", "space_up", "space_down"].map(function (key) { return `<label class="kidia-section-layout-field">${key}<input name="category_general[${key}]" value="0"></label>`; }).join("") + `<label class="kidia-section-layout-field">element_background_color<input type="color" name="category_general[element_background_color]" value="#ffffff"></label>`;
	...
	assert.equal(sectionDom.window.document.querySelector("#category-fields .kidia-section-background-value").value, "#FFFFFF", "The canonical background control must show an editable hex value beside the picker.");
...
function runChromeComposerTest() {
  ...
  assert.equal(window.document.querySelectorAll(".kidia-chrome-row").length, 2, "Home header must support two draggable rows.");
	...
	const categoryTemplate = readAsset("../pages/category-builder.php");
	assert.doesNotMatch(categoryTemplate, /type="range"/, "Category settings must use editable value fields instead of sliders.");
	assert.match(categoryTemplate, /kidia-section-layout-field[\s\S]*category_general\[margin_top\][\s\S]*kidia-section-layout-field[\s\S]*category_general\[margin_bottom\][\s\S]*kidia-section-layout-field[\s\S]*category_general\[space_up\][\s\S]*kidia-section-layout-field[\s\S]*category_general\[space_down\][\s\S]*kidia-section-layout-field[\s\S]*category_general\[element_background_color\]/, "Category must provide the same five fields to the shared Section Layout component.");
	assert.match(readAsset("category-builder.css"), /kidia-category-general-fields\s*\{[^}]*direction:rtl;[^}]*text-align:right;/, "Every Category settings section must start from the right.");
	...
	const homeBuilderCss = readAsset("home-builder.css");
	assert.match(homeBuilderCss, /kidia-builder-block__actions\s*\{[^}]*grid-template-columns:\s*74px 92px 38px 92px;[^}]*width:\s*311px;[^}]*direction:\s*ltr;/, "Every outer element card must reserve the same fixed action slots.");
	assert.match(homeBuilderCss, /kidia-delete-block\s*\{\s*grid-column:\s*1;[^}]*\}[\s\S]*kidia-duplicate-block\s*\{\s*grid-column:\s*2;[^}]*\}[\s\S]*kidia-builder-switch--card\s*\{[^}]*grid-column:\s*4;/, "Outer card actions must always follow Remove, Duplicate, dropdown, and On/Off.");
	assert.doesNotMatch(homeBuilderCss, /data-type="product_carousel"\]\s+\.kidia-builder-block__actions/, "No element type may shift the shared outer-card actions independently.");
