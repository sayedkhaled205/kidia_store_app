...
function runMergeControlsContractTest() {
  ...
  assert.match(categoryPreview, /margin_bottom[^\n]+margin_top/, "Category preview must pull elements together instead of adding positive margins.");
	...
	assert.match(flutterPreviewBridge, /#kidia-page-elements > \.kidia-page-card[\s\S]*layout\.elements/, "The Flutter bridge must preserve the visible card order when it serializes the page.");
	...
	assert.match(pageStore, /product_summary[\s\S]*price_color[\s\S]*#2F806E[\s\S]*name_color[\s\S]*#1D1D1D/, "Product Information must expose independent price and product-name colors with the established visual defaults.");
	assert.match(pageStore, /show_name[\s\S]*Show product name[\s\S]*name_price_gap[\s\S]*Space between price and product name[\s\S]*20[\s\S]*0, 64/, "Product Information must expose a clear name On/Off field and adjustable price-to-name spacing while retaining the current 20px default.");
	assert.match(pageStore, /variations[\s\S]*title_color[\s\S]*option_text_color[\s\S]*group_gap[\s\S]*0, 48/, "Variations must expose title color, option text color, and adjustable spacing between groups.");
	assert.match(pageStore, /product_tabs[\s\S]*show_tabs[\s\S]*Show Overview \/ Reviews \/ Recommend/, "Product Tabs must expose a clear On/Off field for the complete Overview, Reviews, and Recommend strip.");
	assert.match(productScreenSource, /product_tabs'\)\.enabled[\s\S]*boolean\('show_tabs', true\)/, "Flutter must remove the whole Product Tabs strip when its On/Off field is disabled.");
	assert.match(productScreenSource, /priceColor[\s\S]*price_color[\s\S]*nameColor[\s\S]*name_color[\s\S]*color: priceColor[\s\S]*color: nameColor/, "Flutter must apply the configured product price and name colors.");
	assert.match(productScreenSource, /namePriceGap[\s\S]*name_price_gap[\s\S]*show_name[\s\S]*show_price[\s\S]*namePriceGap/, "Flutter must apply the configured price-to-name gap only when both visible fields need separation.");
	assert.match(productScreenSource, /titleColor[\s\S]*title_color[\s\S]*optionTextColor[\s\S]*option_text_color[\s\S]*groupGap[\s\S]*group_gap[\s\S]*EdgeInsets\.only\(bottom: groupGap\)/, "Flutter must apply all three requested Variation appearance controls.");
	assert.match(pagePreview, /show_tabs[\s\S]*price_color[\s\S]*show_name[\s\S]*name_price_gap[\s\S]*name_color[\s\S]*title_color[\s\S]*option_text_color[\s\S]*group_gap/, "The Product Builder preview must react to every new Product control.");
