...
function runMergeControlsContractTest() {
  ...
  assert.match(pageStore, /product_wishlist_background_size[^\n]+array\(\), 20, 64/, "Wishlist backgrounds must support compact sizes down to 20px.");
	...
	const sectionDom = new JSDOM(`<!doctype html><html><body><div class="kidia-page-builder"><section data-element="product_grid"><div id="product-fields" class="kidia-page-fields">${catalogProductMarkup}${sharedMarkup}<div class="kidia-page-field"><label id="page-boolean"><input type="checkbox" name="layout[elements][0][settings][catalog_extra_toggle]" checked> Extra</label></div></div></section></div><div class="kidia-builder-wrap"><section data-type="product_carousel"><div id="carousel-fields" class="kidia-builder-settings-content">${carouselMarkup}<div class="kidia-builder-field"><label id="home-boolean"><input type="checkbox" name="blocks[0][settings][show_price]"> Show price</label></div></div></section><section data-type="product_grid"><div id="home-grid-fields" class="kidia-builder-settings-content">${productGridMarkup}</div></section><div class="kidia-builder-settings-content">${nestedTitlePair}</div></div><div class="kidia-category-builder"><div id="category-fields" class="kidia-category-general-fields">${categoryLayout}${categoryShared}<label id="category-boolean"><input type="checkbox" name="category_general[show_arrow]"> Show arrow</label><label id="master-boolean" class="kidia-page-master-toggle"><input type="checkbox" name="category_element_enabled"><span class="kidia-toggle-state"></span></label></div></div></body></html>`, { runScripts: "outside-only" });
	...
	assert.deepEqual(Array.from(sectionDom.window.document.querySelectorAll("#category-fields > .kidia-settings-section-title"), function (heading) { return heading.textContent; }), ["Layout & Spacing"], "Category must merge Visibility & Display, Products & Data, and General Settings into Layout & Spacing.");
	assert.equal(sectionDom.window.document.querySelectorAll('#category-fields > label:not(.kidia-page-master-toggle)').length, 4, "Category Layout & Spacing must preserve Layout, Open categories, Grid columns, and Show arrow exactly once.");
...
function runChromeComposerTest() {
  ...
  assert.equal(window.document.querySelectorAll(".kidia-chrome-row").length, 2, "Home header must support two draggable rows.");
	...
	assert.match(readAsset("category-builder.css"), /kidia-category-card\s*\{[^}]*grid-template-columns:24px 58px minmax\(190px,1fr\) minmax\(132px,160px\) minmax\(160px,190px\) 92px 44px;[^}]*grid-template-rows:auto;/, "Every desktop Category card must reserve the same single-row columns for its name, image buttons, visibility, and expand control.");
	assert.match(readAsset("category-builder.css"), /kidia-category-image-actions\s*\{[^}]*grid-column:4 \/ 6;[^}]*grid-row:1;[^}]*grid-template-columns:minmax\(132px,160px\) minmax\(160px,190px\)/, "Every Category card must keep both image buttons in fixed columns on the main row.");
