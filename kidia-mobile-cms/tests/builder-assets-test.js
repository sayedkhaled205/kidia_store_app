...
function runMergeControlsContractTest() {
  ...
  assert.match(categoryPreview, /margin_bottom[^\n]+margin_top/, "Category preview must pull elements together instead of adding positive margins.");
	...
	assert.match(readAsset("admin-theme.css"), /\.kidia-section-layout-panel\s*\{[\s\S]*display:\s*block\s*!important;[\s\S]*grid-column:\s*1 \/ -1\s*!important;[\s\S]*direction:\s*rtl\s*!important;/, "The independent Section Layout panel must not inherit the previous section's layout or direction.");
	...
	assert.match(readAsset("chrome-layout.css"), /data-setting="show_cart_badge"\]\s*\{[^}]*grid-column:2;[^}]*align-items:center;[^}]*justify-self:stretch;/, "Show cart item count must stay centered above the second Cart Settings column without overlapping the icon choices.");
	...
	assert.doesNotMatch(readAsset("home-builder.css"), /\.kidia-product-icon-field--quick_add_position[^}]*grid-row:\s*1\s*\/\s*5/, "The icon position field must not reserve a full column across four rows.");
	...
	assert.match(readAsset("admin-theme.css"), /Every boolean setting uses the same On\/Off control instead of a tick box[\s\S]*\.kidia-unified-boolean > input\[type="checkbox"\][\s\S]*border-radius:\s*999px[\s\S]*content:\s*"Off"[\s\S]*content:\s*"On"/, "Every Builder must render boolean values as the shared On/Off switch rather than tick boxes.");
  ...
  assert.match(pageStore, /product_wishlist_background_size[^\n]+array\(\), 20, 64/, "Wishlist backgrounds must support compact sizes down to 20px.");
	...
	const sectionDom = new JSDOM(`<!doctype html><html><body><div class="kidia-page-builder"><section data-element="product_grid"><div id="product-fields" class="kidia-page-fields">${quickMarkup}${sharedMarkup}<div class="kidia-page-field"><label id="page-boolean"><input type="checkbox" name="layout[elements][0][settings][show_name]" checked> Show name</label></div></div></section></div><div class="kidia-builder-wrap"><section data-type="product_carousel"><div id="carousel-fields" class="kidia-builder-settings-content">${carouselMarkup}<div class="kidia-builder-field"><label id="home-boolean"><input type="checkbox" name="blocks[0][settings][show_price]"> Show price</label></div></div></section><section data-type="product_grid"><div id="home-grid-fields" class="kidia-builder-settings-content">${productGridMarkup}</div></section><div class="kidia-builder-settings-content">${nestedTitlePair}</div></div><div class="kidia-category-builder"><div id="category-fields" class="kidia-category-general-fields">${categoryShared}<label id="category-boolean"><input type="checkbox" name="category_general[show_arrow]"> Show arrow</label><label id="master-boolean" class="kidia-page-master-toggle"><input type="checkbox" name="category_element_enabled"><span class="kidia-toggle-state"></span></label></div></div></body></html>`, { runScripts: "outside-only" });
