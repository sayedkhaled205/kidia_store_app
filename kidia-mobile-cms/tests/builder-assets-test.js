...
function runMergeControlsContractTest() {
  ...
  const settingsSections = readAsset("settings-sections.js");
	...
	assert.match(heroBlockSource, /kidia-repeatable-field--image-url[\s\S]*kidia-repeatable-field--media[\s\S]*kidia-repeatable-field--title[\s\S]*kidia-repeatable-field--subtitle[\s\S]*kidia-repeatable-field--button-label[\s\S]*kidia-repeatable-field--button-link[\s\S]*kidia-repeatable-field--image-clickable/, "Slide must expose the approved Hero Slider fields in the fixed three-column card layout.");
