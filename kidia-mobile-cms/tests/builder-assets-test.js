...
function runMergeControlsContractTest() {
  ...
  assert.match(categoryPreview, /margin_bottom[^\n]+margin_top/, "Category preview must pull elements together instead of adding positive margins.");
	...
	assert.match(readAsset("home-builder.css"), /\.kidia-banner-editor-item > \.kidia-hero-block-item__header\s*\{[^}]*justify-content:\s*flex-start;[^}]*direction:\s*rtl;[^}]*\}[\s\S]*\.kidia-banner-editor-item > \.kidia-hero-block-item__header \.kidia-repeatable-item-actions\s*\{[^}]*margin-inline-start:\s*0;/, "Banner headers must match the compact Slider header alignment.");
