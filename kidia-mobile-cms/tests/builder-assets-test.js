...
function runMergeControlsContractTest() {
  ...
  assert.match(categoryPreview, /margin_bottom[^\n]+margin_top/, "Category preview must pull elements together instead of adding positive margins.");
	...
	assert.match(chromeTemplate, /class="kidia-fixed-chrome-identity"/, "Fixed Header and Footer identity must have a stable ordering hook.");
	assert.match(readAsset("chrome-layout.css"), /\.kidia-fixed-chrome-card > \.kidia-page-card__header\s*\{\s*direction:ltr;\s*\}[\s\S]*kidia-chrome-transfer-actions\s*\{\s*order:1;[^}]*direction:ltr;[\s\S]*kidia-fixed-chrome-expand\s*\{\s*order:2;[\s\S]*kidia-page-master-toggle\s*\{\s*order:3;/, "Fixed Header and Footer must run Copy, Paste, expand, then On/Off from the left.");
	...
	assert.match(settingsSections, /homeBlock\.dataset\.type === "product_carousel"[\s\S]*content_data[\s\S]*carousel_actions[\s\S]*card_layout[\s\S]*carousel_visibility[\s\S]*quick_add[\s\S]*carousel_wishlist/, "Product Carousel must use its explicit six-section settings map instead of heuristic grouping.");
	assert.match(readAsset("home-builder.css"), /\.kidia-hero-block-item__header\s*\{[\s\S]*direction:\s*rtl;[\s\S]*width:\s*100%;/, "Repeatable cards must retain the shared default header structure.");
	assert.match(readAsset("home-builder.css"), /\.kidia-slider-editor-item > \.kidia-hero-block-item__header,[\s\S]*\.kidia-banner-editor-item > \.kidia-hero-block-item__header\s*\{[\s\S]*justify-content:\s*flex-start;[\s\S]*direction:\s*ltr;/, "The complete Slide and Banner header group must start from the far left.");
	assert.match(readAsset("home-builder.css"), /\.kidia-slider-editor-item > \.kidia-hero-block-item__header \.kidia-repeatable-item-actions,[\s\S]*margin-inline-start:\s*0;[\s\S]*flex:\s*0 0 auto;[\s\S]*direction:\s*ltr;/, "Slide and Banner headers must run title, Remove, Add, then On/Off from the left.");
	...
	assert.match(readAsset("home-builder.css"), /data-type="product_carousel"[^\{]*\.kidia-builder-settings-content\s*\{[\s\S]*grid-template-columns:\s*repeat\(3,[\s\S]*direction:\s*rtl;/, "Product Carousel sections must use the shared three-field RTL layout.");
  ...
  assert.match(pageStore, /product_wishlist_background_size[^\n]+array\(\), 20, 64/, "Wishlist backgrounds must support compact sizes down to 20px.");
	...
	const carouselMarkup = carouselKeys.map(function (key) { return `<div class="kidia-builder-field"><label>${key}</label><input name="blocks[0][settings][${key}]" value="0"></div>`; }).join("") + sharedMarkup.replaceAll("layout[elements][0]", "blocks[0]");
	const sectionDom = new JSDOM(`<!doctype html><html><body><div class="kidia-page-builder"><section data-element="product_grid"><div id="product-fields" class="kidia-page-fields">${quickMarkup}${sharedMarkup}</div></section></div><div class="kidia-builder-wrap"><section data-type="product_carousel"><div id="carousel-fields" class="kidia-builder-settings-content">${carouselMarkup}</div></section><div class="kidia-builder-settings-content">${nestedTitlePair}</div></div><div class="kidia-category-builder"><div id="category-fields" class="kidia-category-general-fields">${categoryShared}</div></div></body></html>`, { runScripts: "outside-only" });
