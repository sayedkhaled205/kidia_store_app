...
function runMergeControlsContractTest() {
  ...
  assert.match(categoryPreview, /margin_bottom[^\n]+margin_top/, "Category preview must pull elements together instead of adding positive margins.");
	...
	assert.match(pageStore, /private const VERSION = 24;/, "Page availability and the current builder controls must use the current layout schema.");
...
function runHeaderPositionAndProductApplyAllContractTest() {
	...
	assert.match(store, /VERSION = 24[\s\S]*saved\['version'\][\s\S]*cart_offset_y[\s\S]*orders_offset_y[\s\S]*current_offset - 2/, "Existing Cart and Orders icons must migrate upward by two pixels exactly once.");
