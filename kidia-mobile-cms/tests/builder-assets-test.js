...
function runHomeBuilderTest() {
  ...
  assert.match(builderCss, /grid-template-columns:\s*350px minmax\(0, 1fr\)/, "The editor must reserve the larger canonical phone preview without crowding the settings.");
	assert.match(builderCss, /\.kidia-mobile-preview__device\s*\{[\s\S]*?width:\s*322px;[\s\S]*?margin-inline:\s*auto;/, "The Home phone must use the balanced intermediate viewport centered beside the element editor.");
...
function runMergeControlsContractTest() {
  ...
  assert.match(categoryPreview, /margin_bottom[^\n]+margin_top/, "Category preview must pull elements together instead of adding positive margins.");
	...
	assert.match(readAsset("home-builder.css"), /\.kidia-mobile-preview__device\s*\{[^}]*width:\s*322px;/, "The Home phone must use the balanced intermediate preview width.");
	...
	assert.match(readAsset("page-builder.css"), /\.kidia-page-phone\s*\{[^}]*width:322px;[^}]*margin-inline:auto;/, "Every page phone must use the same balanced width and remain centered.");
	...
	assert.match(readAsset("category-builder.css"), /\.kidia-category-phone\s*\{[^}]*width:322px;[^}]*margin-inline:auto;/, "The Category phone must use the same balanced width and remain centered.");
	...
	assert.match(pageBuilderCss, /\.kidia-card-actions,[\s\S]*display:grid!important;[\s\S]*grid-template-columns:82px 82px 44px 88px;[\s\S]*gap:2px;[\s\S]*direction:ltr!important;/, "Every closed element card must keep its four actions compact and close together.");
	...
	assert.match(pageBuilderCss, /\.kidia-flutter-preview\s*\{[^}]*width:360px;[^}]*height:800px;[^}]*zoom:\.85;/, "Flutter must keep the canonical mobile viewport and scale the complete interactive screen to the balanced frame.");
	...
	assert.match(pageBuilderCss, /\.kidia-page-card__header > \.kidia-card-actions,[\s\S]*\.kidia-builder-block__header > \.kidia-card-actions\s*\{\s*position:absolute;\s*top:50%;\s*left:10px;\s*margin:0;\s*transform:translateY\(-50%\);\s*\}/, "Every desktop closed-card action strip must share the same slightly-left physical anchor instead of depending on RTL flex distribution.");
	...
	assert.match(pageBuilderCss, /\.kidia-page-card__header > \.kidia-card-actions,[\s\S]*left:10px;/, "Header, Footer, and element action groups must share the same slightly-left physical anchor.");
	...
	assert.match(pageBuilderCss, /\.kidia-card-actions \.button\.kidia-card-action\s*\{[^}]*height:38px;[^}]*padding:0 8px;[^}]*border-radius:7px!important;[^}]*font-size:12px;/, "Every Header, Footer, and element card action button must share one size and shape.");
	assert.match(pageBuilderCss, /\.kidia-card-actions \.kidia-card-action--toggle\s*\{[^}]*width:88px;[^}]*height:38px;[^}]*direction:rtl;/, "Every exterior card toggle must occupy the exact same slot and dimensions.");
