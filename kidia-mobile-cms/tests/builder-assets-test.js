...
function runMergeControlsContractTest() {
  ...
  assert.match(pageStore, /product_wishlist_background_size[^\n]+array\(\), 20, 64/, "Wishlist backgrounds must support compact sizes down to 20px.");
	...
	assert.equal((checkoutSuggestions.match(/kidia-settings-section-title--checkout-suggestions/g) || []).length, 1, "Checkout Suggestions must merge Products, Image and Actions into one real section.");
	assert.doesNotMatch(checkoutSuggestions, />Products & Data<|>Image Settings<|>Actions & Navigation</, "Checkout Suggestions must not render the three old section headings.");
