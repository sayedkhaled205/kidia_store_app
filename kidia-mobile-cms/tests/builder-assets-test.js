...
function runMergeControlsContractTest() {
  ...
  const settingsSections = readAsset("settings-sections.js");
	...
	assert.match(heroBlockSource, /kidia-slider-item-toggle[\s\S]*kidia-toggle-state/, "Each Slide must use the shared On/Off toggle in its header.");
	assert.doesNotMatch(heroBlockSource, /kidia-builder-field--toggle[\s\S]*Show/, "Slide cards must not keep the old Show checkbox field.");
	assert.match(readAsset("home-builder.css"), /\.kidia-slider-editor-item\s*>\s*\.kidia-hero-block-item__header\s*\{[^}]*grid-row:\s*1;[^}]*order:\s*-10;/, "The Slide header must remain above all card fields.");
	...
	assert.match(readAsset("category-builder.css"), /\.kidia-category-image-actions \.button\s*\{[^}]*width:\s*110px;[^}]*min-width:\s*110px;/, "Category and Subcategory image buttons must use the compact non-overlapping width.");
	assert.match(readAsset("category-builder.css"), /\.kidia-category-visibility\.kidia-page-master-toggle\s*\{[^}]*min-width:\s*80px;[^}]*justify-self:\s*end;/, "Category and Subcategory On/Off controls must stay clear of the image buttons.");
	assert.match(chromeTemplate, /footer_general_order[\s\S]*height[\s\S]*style[\s\S]*shadow[\s\S]*horizontal_padding[\s\S]*top_radius[\s\S]*side_spacing_percent[\s\S]*label_size[\s\S]*icon_size[\s\S]*icon_label_gap[\s\S]*border_width[\s\S]*border_color[\s\S]*inactive_color[\s\S]*active_color/, "Footer General Settings must follow the requested fixed order.");
	assert.match(chromeTemplate, /kidia-footer-toggle-row[\s\S]*hide_on_scroll[\s\S]*safe_area[\s\S]*show_labels/, "The three Footer On/Off controls must share the final General Settings row.");
	assert.match(readAsset("page-builder.css"), /\.kidia-footer-toggle-row\s*\{[^}]*grid-template-columns:\s*repeat\(3/, "Footer On/Off controls must render as one three-column row.");
