...
function homeBlock(type, index, settings, name = type) {
  const category = ["hero_slider", "image_banner", "banner_grid", "video_banner"].includes(type)
    ...
    : ["category_grid", "product_carousel", "product_grid", "brand_carousel", "bundle_collection"].includes(type)
      ...
      : "content";
  return `
    <div class="kidia-builder-block is-collapsed" data-type="${type}" data-library-id="${type}_${index}" data-element-category="${category}">
...
function homeMarkup() {
  ...
  return `<!doctype html><html><body>
    ...
    <div id="kidia-element-picker" hidden aria-hidden="true">
      <nav class="kidia-element-category-filter">
        <button type="button" data-kidia-element-category="all">All</button>
        <button type="button" data-kidia-element-category="products">Products</button>
      </nav>
      <div class="kidia-element-group" data-element-category="layout">Layout</div>
...
function runHomeBuilderTest() {
  ...
  assert.match(builderCss, /grid-template-columns:\s*350px minmax\(0, 1fr\)/, "The editor must reserve the larger canonical phone preview without crowding the settings.");
	assert.match(builderCss, /\.kidia-mobile-preview__device\s*\{[\s\S]*?width:\s*304px;[\s\S]*?margin-inline:\s*auto;/, "The Home phone must use the smaller balanced viewport centered beside the element editor.");
  ...
  assert.match(builderCss, /\.kidia-builder-list\s*\{[\s\S]*?display:\s*flex;[\s\S]*?flex-direction:\s*column;/, "Added Home elements must render as compact horizontal rows.");
  assert.match(builderCss, /\.kidia-builder-block__header\s*\{[\s\S]*?min-height:\s*66px;[\s\S]*?flex-direction:\s*row;/, "Every collapsed Home element must match the compact Header and Footer row pattern.");
...
function runMergeControlsContractTest() {
  ...
  const settingsSections = readAsset("settings-sections.js");
	...
	assert.match(heroBlockSource, /kidia-repeatable-field--image-url[\s\S]*kidia-repeatable-field--media[\s\S]*kidia-repeatable-field--title[\s\S]*kidia-repeatable-field--subtitle[\s\S]*kidia-repeatable-field--button-label[\s\S]*kidia-repeatable-field--button-position[\s\S]*kidia-repeatable-field--button-link/, "Slide must expose every field used by the current button-link three-column card layout.");
	assert.doesNotMatch(heroBlockSource.slice(heroBlockSource.indexOf("private function render_item")), /kidia-repeatable-field--action-(?:type|value)/, "Hero Slider must not regress to the superseded Action Type and Action Value controls.");
	...
	assert.match(readAsset("home-builder.css"), /\.kidia-slider-editor-item\s*>\s*\.kidia-repeatable-field--button-link,[\s\S]*grid-row:\s*4;/, "Current Slider button fields must shift below the protected header row.");
	assert.match(readAsset("home-builder.css"), /\.kidia-repeatable-field--button-link\s*\{\s*grid-column:\s*1;\s*grid-row:\s*3;\s*\}[\s\S]*\.kidia-repeatable-field--button-position\s*\{\s*grid-column:\s*2;\s*grid-row:\s*3;\s*\}/, "Button Link and Button Position must replace the old action fields without changing the approved grid.");
  ...
  assert.match(categoryPreview, /margin_bottom[^\n]+margin_top/, "Category preview must pull elements together instead of adding positive margins.");
	...
	assert.match(readAsset("home-builder.css"), /\.kidia-mobile-preview__device\s*\{[^}]*width:\s*304px;/, "The Home phone must use the smaller balanced preview width.");
	...
	assert.match(readAsset("page-builder.css"), /\.kidia-page-phone\s*\{[^}]*width:304px;[^}]*margin-inline:auto;/, "Every page phone must use the same smaller width and remain centered.");
	...
	assert.match(readAsset("category-builder.css"), /\.kidia-category-phone\s*\{[^}]*width:304px;[^}]*margin-inline:auto;/, "The Category phone must use the same smaller width and remain centered.");
	...
	assert.match(readAsset("home-builder.css"), /\.kidia-mobile-preview__screen\s*\{[^}]*height:\s*640px;[^}]*touch-action:\s*pan-y;/, "Home preview must use the smaller mobile height while preserving touch scrolling.");
	assert.match(pageBuilderCss, /\.kidia-page-phone__screen\s*\{[^}]*height:640px;[^}]*touch-action:pan-y;/, "Every page preview must use the same smaller mobile height while preserving scrolling.");
	assert.match(categoryBuilderCss, /\.kidia-category-phone__screen\s*\{[^}]*height:640px;[^}]*touch-action:pan-y;/, "Category preview must use the same smaller mobile height while preserving scrolling.");
	assert.match(pageBuilderCss, /\.kidia-flutter-preview\s*\{[^}]*width:360px;[^}]*height:800px;[^}]*touch-action:pan-y;[^}]*zoom:\.8;/, "Flutter must keep the canonical mobile viewport and scale the complete interactive screen without disabling scrolling.");
	...
	assert.match(homeBlockTemplate, /kidia-builder-block__actions kidia-card-actions[\s\S]*kidia-card-action--primary[\s\S]*kidia-card-action--secondary[\s\S]*kidia-card-action--expand[\s\S]*kidia-card-action--toggle/, "All Home top-level elements must use the canonical Header and Footer action order.");
	...
	assert.match(pageBuilder, /kidia-builder-switch kidia-builder-switch--card kidia-card-action kidia-card-action--toggle[\s\S]*kidia-builder-switch__track[\s\S]*kidia-builder-switch__state/, "Every shared page element must use the exact Header and Footer card toggle component.");
	assert.match(categoryBuilder, /kidia-builder-switch kidia-builder-switch--card kidia-card-action kidia-card-action--toggle[\s\S]*kidia-builder-switch__track[\s\S]*kidia-builder-switch__state/, "Category must use the exact Header and Footer card toggle component.");
	assert.match(pageBuilderCss, /\.kidia-page-builder :is\(\.kidia-page-card,\.kidia-fixed-chrome-card\),[\s\S]*border:1px solid #d8e5e1;[\s\S]*border-radius:10px;[\s\S]*background:#fff;/, "Every page, Home, Category, Header, and Footer card must share one exterior shell.");
	...
	assert.match(pageBuilderCss, /\.kidia-card-actions \.kidia-card-action--toggle\s*\{[^}]*width:104px;[^}]*height:34px;[^}]*gap:5px;[^}]*overflow:visible;[^}]*direction:ltr;/, "Every exterior card toggle must occupy the same unclipped slot with the label close to the switch.");
  ...
  assert.match(pageStore, /product_wishlist_background_size[^\n]+array\(\), 20, 64/, "Wishlist backgrounds must support compact sizes down to 20px.");
	...
	assert.match(homeBuilderCss, /\.kidia-builder-switch input\[type="checkbox"\]\s*\{[\s\S]*clip-path:\s*inset\(50%\);[\s\S]*appearance:\s*none\s*!important;/, "Home switches must fully hide the native WordPress checkbox without changing their saved values.");
	assert.match(homeBuilderCss, /\.kidia-builder-switch input\[type="checkbox"\]::before,[\s\S]*::after\s*\{[\s\S]*content:\s*none\s*!important;/, "Native WordPress checkbox marks must never leak behind the custom On/Off switch.");
...
function runPageBuilderTest() {
  ...
  assert.match(homeBlockSource, /imageSwipeEnabled[\s\S]*ProductCard/, "Home Product Grid and Product Carousel must pass image-swipe settings into the shared product card.");
	...
	assert.match(pageTemplateSource, /\$kidia_toolbar_show_restore = true/, "Every shared page must request the defaults action from the common toolbar.");
...
function runChromeComposerTest() {
  ...
  const presetLayout = JSON.parse(card.querySelector(".kidia-chrome-layout-json").value);
  ...
  const layoutInput = card.querySelector(".kidia-chrome-layout-json");
  layoutInput.value = layout;
  card.querySelector(".kidia-chrome-composer").dispatchEvent(new window.Event("kidia:chrome-layout-replaced"));
...
function runHeaderPositionAndProductApplyAllContractTest() {
	...
	assert.match(shellCss, /body\.kidia-cms-builder-screen #wpcontent\{[^}]*overflow:hidden!important;/, "The WordPress content viewport must not scroll behind a Builder.");
	assert.match(shellCss, /body\.kidia-cms-builder-screen #wpbody\{[^}]*overflow-y:scroll!important;[^}]*scrollbar-gutter:stable;/, "Every Builder must reserve the same plugin scrollbar rail as the other CMS pages.");
	...
	assert.match(shellScript, /function resetDocumentScroll\(\)[\s\S]*history\.scrollRestoration = 'manual'[\s\S]*window\.scrollTo[\s\S]*syncBuilderScreen[\s\S]*resetDocumentScroll\(\)/, "Every CMS view, including Builders, must discard restored WordPress document scroll before its workspace appears.");
