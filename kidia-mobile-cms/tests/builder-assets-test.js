...
function runMergeControlsContractTest() {
  ...
  assert.match(categoryPreview, /margin_bottom[^\n]+margin_top/, "Category preview must pull elements together instead of adding positive margins.");
	...
	assert.match(pageBuilderCss, /\.kidia-card-action--primary\s*\{\s*grid-column:1;\s*\}[\s\S]*\.kidia-card-action--secondary\s*\{\s*grid-column:2;\s*\}[\s\S]*\.kidia-card-action--expand\s*\{\s*grid-column:3;\s*\}[\s\S]*\.kidia-card-action--toggle\s*\{\s*grid-column:4;\s*\}/, "Primary, secondary, expand, and On/Off controls must keep the same slots on every closed card.");
