...
assert.match(plugin, /Version:\s+1\.32\.0/, "The plugin header must be version 1.32.0.");
assert.match(
  plugin,
...
for (const endpoint of ["/activate", "/verify", "/deactivate"]) {
...
assert.match(admin, /admin_post_kidia_mobile_deactivate_license/, "The deactivation handler must be registered.");
