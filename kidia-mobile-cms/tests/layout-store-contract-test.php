...
kidia_assert( 'quick_links' === $preset[0]['type'], 'The PatPat content must begin with four circular shortcuts under the fixed logo/search header.' );
kidia_assert( 4 === $preset[0]['settings']['columns'], 'The first shortcut row must use four equal columns.' );
kidia_assert( 'image_banner' === $preset[1]['type'], 'The PatPat content must place the main Kidia banner directly under the shortcuts.' );
kidia_assert( 1.0 === (float) $preset[1]['settings']['aspect_ratio'], 'The main Kidia banner must use the tall PatPat proportion.' );
kidia_assert( 'category_grid' === $preset[2]['type'] && 3 === $preset[2]['settings']['columns'], 'Three circular Kidia categories must follow the main banner.' );
...
kidia_assert( array( 'quick_links', 'image_banner', 'category_grid' ) === array_column( array_slice( $default_layout, 0, 3 ), 'type' ), 'Fresh Home Pages must use the PatPat shortcut, tall banner and three-category opening order.' );
