...
assert.match(shellTemplate, /data-build-persistent[\s\S]*data-build-modal[\s\S]*role="dialog"[\s\S]*data-build-message[\s\S]*data-build-progress[\s\S]*data-build-cancel/, "The build progress card must live in the permanent CMS shell and keep its cancel action across page changes.");
...
assert.match(dashboardTemplate, /\.kidia-app-build__card-button\s*\{[^}]*min-height:\s*120px;[^}]*background:\s*#2f806e;/, "The complete build card button must use the light WooMobile color and fill the journey card.");
...
assert.match(shellCss, /body\.kidia-cms-plugin-page:not\(\.kidia-cms-builder-screen\) #wpbody\{[^}]*height:calc\(100vh - 32px\)!important;[^}]*overflow-y:scroll!important;[^}]*scrollbar-gutter:stable;[^}]*direction:ltr;/, "The plugin workspace must own the stable inner desktop scrollbar beside the RTL WordPress menu.");
...
assert.doesNotMatch(shellCss, /#adminmenuback,[^}]*#adminmenuwrap\{[^}]*position:fixed!important;/, "Woo Mobile CMS must leave WordPress menu scrolling to the native outer document.");
