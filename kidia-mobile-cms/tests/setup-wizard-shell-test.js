...
assert.match(fs.readFileSync(path.join(root, 'includes/blocks/class-kidia-mobile-quick-links-block.php'), 'utf8'), /array\(\s*'carousel',\s*'grid',\s*'service'\s*\)/, "Quick Links must preserve the Family Pop service layout through PHP sanitization.");
assert.match(fs.readFileSync(path.join(root, 'includes/blocks/class-kidia-mobile-banner-grid-block.php'), 'utf8'), /array\(\s*'equal',\s*'featured',\s*'mosaic',\s*'carousel'\s*\)/, "Banner Grid must preserve the horizontal Explore Trends layout through PHP sanitization.");
...
assert.match(admin, /\$abandoned_import_state\s*=\s*Kidia_Mobile_Analytics::website_session_import_status\(\)/, "Every full CMS load must restore the saved Abandoned Cart job state.");
assert.match(shellTemplate, /data-kidia-background-job-stack[\s\S]*data-kidia-background-job="app-build"[\s\S]*data-kidia-background-job="abandoned-carts"/, "The permanent shell must render Build and Abandoned Cart progress inside one shared stack.");
...
assert.match(wizardCss, /\.kidia-theme-modal__select\{[^}]*margin-top:0!important;[^}]*border-color:#236b59!important;[^}]*background:#2f806e!important/, "Theme selection must stay directly below the page buttons and retain the plugin action color independently of the chosen theme.");
...
assert.match(wizardCss, /kidia-theme-modal__device\{[^}]*--kidia-theme-preview-scale:\.72[^}]*width:calc\(360px \* var\(--kidia-theme-preview-scale\) \+ 10px\)[^}]*height:calc\(800px \* var\(--kidia-theme-preview-scale\) \+ 10px\)[^}]*padding:0[^}]*overflow:hidden[^}]*border:5px solid #2f806e/, "The complete phone must keep one continuous green border outside the clipped curved screen.");
assert.match(wizardCss, /kidia-theme-modal__device iframe\{[^}]*top:0[^}]*left:0[^}]*border-radius:21px/, "The scaled Flutter screen must sit inside the frame without covering its curved border.");
...
assert.match(wizardCss, /kidia-setup-apply\{[^}]*display:inline-flex!important[^}]*flex-direction:row!important[^}]*flex-wrap:nowrap!important[^}]*align-items:center!important[^}]*white-space:nowrap\}[\s\S]*kidia-setup-apply \.dashicons\{[^}]*flex:0 0 20px[^}]*line-height:20px[^}]*transform:none!important/, "Finish label and check icon must remain centered beside each other on one row.");
...
assert.match(shellCss, /@media\(min-width:783px\)\{[\s\S]*body\.kidia-cms-plugin-page #adminmenuwrap\{[^}]*position:fixed!important;[^}]*inset-block-start:32px!important;[^}]*inset-block-end:0!important;[^}]*inset-inline-start:auto!important;[^}]*overflow-y:scroll!important;[^}]*overscroll-behavior:contain;[^}]*scrollbar-gutter:stable;/, "The black WordPress menu must keep its own contained desktop scrollbar without direction-dependent positioning.");
assert.match(shellCss, /body\.kidia-cms-plugin-page #adminmenuwrap\{[^}]*overflow-y:scroll!important;[^}]*direction:ltr;/, "The WordPress menu scrollbar must sit on the physical right side without moving the menu.");
assert.match(shellCss, /html\[dir="ltr"\] body\.kidia-cms-plugin-page #adminmenuwrap\{[^}]*left:0!important;[^}]*right:auto!important;/, "The LTR WordPress menu must remain pinned to the physical left.");
assert.match(shellCss, /html\[dir="rtl"\] body\.kidia-cms-plugin-page #adminmenuwrap,[^}]*\.rtl body\.kidia-cms-plugin-page #adminmenuwrap\{[^}]*left:auto!important;[^}]*right:0!important;/, "The RTL WordPress menu must remain pinned to the physical right while its scrollbar moves right.");
assert.match(shellCss, /html\[dir="ltr"\] body\.kidia-cms-plugin-page #adminmenu\{[^}]*direction:ltr;/, "The WordPress menu itself must remain LTR when the admin locale is LTR.");
assert.match(shellCss, /html\[dir="rtl"\] body\.kidia-cms-plugin-page #adminmenu,[^}]*\.rtl body\.kidia-cms-plugin-page #adminmenu\{[^}]*direction:rtl;/, "The WordPress menu itself must remain RTL after moving its scrollbar.");
