...
assert.match(service, /'family_pop'\s*===\s*\$profile[\s\S]*'overlay_strength'\]\s*=\s*0[\s\S]*'text_color'\]\s*=\s*'#000000'[\s\S]*'button_color'\]\s*=\s*'#000000'[\s\S]*'button_text_color'\]\s*=\s*'#FFFFFF'/, "Family Pop banner-grid overlays must use black copy and a black button with white text.");
