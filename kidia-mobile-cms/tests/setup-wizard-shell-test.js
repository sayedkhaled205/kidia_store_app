...
assert.match(familyTheme, /'category_design'[\s\S]*'card_gap'\s*=>\s*12[\s\S]*'card_height'\s*=>\s*84[\s\S]*'card_style'\s*=>\s*'outlined'[\s\S]*'card_radius'\s*=>\s*16[\s\S]*'image_size'\s*=>\s*68[\s\S]*'image_shape'\s*=>\s*'rounded'/, "Family Pop categories must use the compact rounded PatPat list cards.");
