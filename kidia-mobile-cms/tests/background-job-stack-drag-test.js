...
const markup = `<!doctype html><body>
  <div data-kidia-background-job-stack>
    <div data-kidia-background-job="app-build"><div class="kidia-ai-progress-card">Build <button type="button">OK</button></div></div>
    <div data-kidia-background-job="generate-offers"><div class="kidia-ai-progress-card">Generate Offer</div></div>
    <section class="kidia-background-job-card" data-kidia-background-job="abandoned-carts">Generate Abandoned</section>
  </div>
</body>`;
...
const stack = dom.window.document.querySelector("[data-kidia-background-job-stack]");
installGeometry(stack);
dom.window.eval(script);

const cards = Array.from(dom.window.document.querySelectorAll("[data-kidia-background-job]"));
cards.forEach((card, index) => {
  const before = stack.getBoundingClientRect();
  const handle = card.querySelector(".kidia-ai-progress-card") || card;
  ...
  assert.notEqual(after.left, before.left, `${card.dataset.kidiaBackgroundJob} must drag the shared stack.`);
  assert.notEqual(after.top, before.top, `${card.dataset.kidiaBackgroundJob} must move all cards vertically.`);
  assert.equal(stack.classList.contains("is-dragging"), false, "Dragging state must end after pointerup.");
});

const saved = JSON.parse(dom.window.localStorage.getItem("kidiaBackgroundJobStackPositionV1"));
...
const restoredStack = restoredDom.window.document.querySelector("[data-kidia-background-job-stack]");
installGeometry(restoredStack);
restoredDom.window.localStorage.setItem("kidiaBackgroundJobStackPositionV1", JSON.stringify(saved));
