...
assert.match(
  template,
  /kidia-header-preset__row[\s\S]*kidia-header-preset__column[\s\S]*kidia-header-preset__item--/,
  "Preset previews must render real rows, columns, and items.",
);
assert.match(
  css,
  /\.kidia-header-presets\s*\{[^}]*box-sizing:border-box;[^}]*width:100%;[^}]*max-width:100%;[^}]*overflow:hidden;/,
  "The preset section must contain its grid.",
);
assert.match(
  css,
  /\.kidia-header-presets__grid\s*\{[^}]*repeat\(auto-fit,minmax\(min\(142px,100%\),1fr\)\)/,
  "Preset cards must wrap responsively without overflowing.",
);
assert.match(
  css,
  /@media\(max-width:480px\)\{\.kidia-header-presets__grid\{grid-template-columns:1fr\}\}/,
