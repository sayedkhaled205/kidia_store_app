	...
	<section class="kidia-data-panel kidia-data-panel--<?php echo esc_attr( $store_tab ); ?>">
		...
		<?php elseif ( 'categories' === $store_tab ) : ?>
			<div class="kidia-category-section"><h3><?php esc_html_e( 'Main categories', 'mobishop' ); ?></h3><p><?php esc_html_e( 'Top-level categories are kept separate from their subcategories.', 'mobishop' ); ?></p><div class="kidia-category-list">
				<?php foreach ( $parent_categories as $category ) : $category_link = get_term_link( $category ); ?><article><div class="kidia-category-thumb"><?php echo wp_kses_post( $category_image( $category ) ); ?></div><div><strong><?php echo esc_html( $category->name ); ?></strong><small><?php echo esc_html( sprintf( _n( '%d product', '%d products', $category->count, 'mobishop' ), $category->count ) ); ?></small><div class="kidia-row-actions"><a href="<?php echo esc_url( get_edit_term_link( $category->term_id, 'product_cat' ) ); ?>"><?php esc_html_e( 'Edit', 'mobishop' ); ?></a><?php if ( ! is_wp_error( $category_link ) ) : ?><button type="button" data-copy-link="<?php echo esc_attr( $category_link ); ?>"><?php esc_html_e( 'Copy link', 'mobishop' ); ?></button><?php endif; ?></div></div></article><?php endforeach; ?>
			</div></div>
			<div class="kidia-category-section is-subcategories"><h3><?php esc_html_e( 'Subcategories', 'mobishop' ); ?></h3>
		...
		<?php elseif ( 'discounts' === $store_tab ) : ?>
			<form class="kidia-coupon-filters" method="get" data-kidia-instant-filter="coupons">
				...
				<button class="button button-primary" type="submit" data-kidia-instant-submit-fallback><?php esc_html_e( 'Filter', 'mobishop' ); ?></button>
			...
			<?php foreach ( $coupons as $coupon_post ) : $coupon = class_exists( 'WC_Coupon' ) ? new WC_Coupon( $coupon_post->ID ) : null; if ( ! $coupon ) { continue; } $expires = $coupon->get_date_expires(); $starts = get_post_datetime( $coupon_post ); $limit = $coupon->get_usage_limit(); $used = $coupon->get_usage_count(); $remaining = $limit ? max( 0, $limit - $used ) : null; $product_ids = $coupon->get_product_ids(); $excluded_ids = $coupon->get_excluded_product_ids(); $categories = $coupon->get_product_categories(); $excluded_categories = $coupon->get_excluded_product_categories(); $channel = Kidia_Mobile_Coupon_Channel::get( $coupon_post->ID ); $channel_labels = Kidia_Mobile_Coupon_Channel::labels(); ?>
				<tr><td><code><?php echo esc_html( $coupon->get_code() ); ?></code><small class="kidia-coupon-badges"><b><?php echo esc_html( ucfirst( $coupon_post->post_status ) ); ?></b><b><?php echo esc_html( $channel_labels[ $channel ] ); ?></b><?php if ( $coupon->get_individual_use() ) : ?><b><?php esc_html_e( 'Individual', 'mobishop' ); ?></b><?php endif; ?></small></td>
				...
				<td><div class="kidia-row-actions kidia-coupon-actions"><a href="<?php echo esc_url( get_edit_post_link( $coupon_post->ID ) ); ?>"><?php esc_html_e( 'Edit', 'mobishop' ); ?></a><button type="button" data-copy-text="<?php echo esc_attr( $coupon->get_code() ); ?>"><?php esc_html_e( 'Copy code', 'mobishop' ); ?></button><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-kidia-instant-action="coupon-channel"><input type="hidden" name="action" value="kidia_mobile_set_coupon_channel"><input type="hidden" name="coupon_id" value="<?php echo esc_attr( (string) $coupon_post->ID ); ?>"><?php wp_nonce_field( 'kidia_mobile_set_coupon_channel_' . $coupon_post->ID, 'kidia_mobile_coupon_channel_nonce' ); ?><select name="coupon_channel" data-kidia-saved-value="<?php echo esc_attr( $channel ); ?>" aria-label="<?php esc_attr_e( 'Coupon sales channel', 'mobishop' ); ?>"><?php foreach ( $channel_labels as $channel_key => $channel_label ) : ?><option value="<?php echo esc_attr( $channel_key ); ?>" <?php selected( $channel, $channel_key ); ?>><?php echo esc_html( $channel_label ); ?></option><?php endforeach; ?></select><button type="submit" data-kidia-instant-submit-fallback><?php esc_html_e( 'Save channel', 'mobishop' ); ?></button></form></div></td></tr>
		...
		<?php elseif ( 'orders' === $store_tab ) : ?>
			<div class="kidia-data-summary"><div><small><?php esc_html_e( 'Orders in period', 'mobishop' ); ?></small><b><?php echo esc_html( (string) count( $orders ) ); ?></b></div><div><small><?php esc_html_e( 'Revenue in period', 'mobishop' ); ?></small><b><?php echo wp_kses_post( $money( $order_revenue ) ); ?></b></div></div>
		...
		<?php elseif ( 'reports' === $store_tab ) : ?>
			<?php if ( ! $store_reporting_ready ) : ?>
				<div class="kidia-reporting-ready"><span class="dashicons dashicons-chart-bar"></span><div><h3><?php esc_html_e( 'Choose a period, then generate the reports', 'mobishop' ); ?></h3><p><?php esc_html_e( 'Generate prepares the indexed WooCommerce reporting source once. After that, periods and channels open immediately and new orders, refunds and status changes update automatically.', 'mobishop' ); ?></p></div></div>
			<?php else : ?>
			<div data-kidia-live-store-data="reports" aria-live="polite">
			<div class="kidia-data-summary is-four"><div><small><?php esc_html_e( 'Orders', 'mobishop' ); ?></small><b><?php echo esc_html( (string) absint( $reporting_snapshot['all_orders'] ?? 0 ) ); ?></b></div><div><small><?php esc_html_e( 'Net sales', 'mobishop' ); ?></small><b><?php echo wp_kses_post( $money( $order_revenue ) ); ?></b></div><div><small><?php esc_html_e( 'Average order value', 'mobishop' ); ?></small><b><?php echo wp_kses_post( $money( $order_average ) ); ?></b></div><div><small><?php esc_html_e( 'Items sold', 'mobishop' ); ?></small><b><?php echo esc_html( (string) $order_units ); ?></b></div></div>
			<div class="kidia-report-grid">
				<section><header><h3><?php esc_html_e( 'Order status', 'mobishop' ); ?></h3><p><?php esc_html_e( 'See where fulfilment or cancellations affect sales.', 'mobishop' ); ?></p></header><div class="kidia-metric-list"><?php $report_order_count = absint( $reporting_snapshot['all_orders'] ?? 0 ); foreach ( $order_statuses as $status => $status_count ) : ?><div><span><?php echo esc_html( wc_get_order_status_name( $status ) ); ?></span><b><?php echo esc_html( (string) $status_count ); ?></b><i style="--kidia-progress:<?php echo esc_attr( $report_order_count ? (string) round( 100 * $status_count / $report_order_count ) : '0' ); ?>%"></i></div><?php endforeach; ?></div></section>
				<section><header><h3><?php esc_html_e( 'Best-selling products', 'mobishop' ); ?></h3><p><?php esc_html_e( 'Prioritise products producing revenue in the selected source and period.', 'mobishop' ); ?></p></header><div class="kidia-ranked-list"><?php $rank = 0; foreach ( $product_performance as $product_id => $product_row ) : ++$rank; ?><div><b><?php echo esc_html( (string) $rank ); ?></b><span><strong><?php echo esc_html( $product_row['name'] ); ?></strong><small><?php echo esc_html( sprintf( __( '%d units', 'mobishop' ), $product_row['units'] ) ); ?></small></span><em><?php echo wp_kses_post( $money( $product_row['revenue'] ) ); ?></em></div><?php endforeach; ?></div></section>
			</div>
			</div>
			<?php endif; ?>
