...
$tab_url = static function ( string $tab, ?string $source = null ) use ( $store_tab, $store_source, $date_preset, $date_from, $date_to ): string {
	...
	return add_query_arg(
		array(
			...
			'store_tab'    => $tab,
			'store_source' => null === $source ? ( 'analytics' === $tab ? 'mobile' : $store_source ) : $source,
...
$source_tabs = array( 'customers', 'orders', 'reports', 'abandoned-carts' );
...
$analytics_opens = $event( 'app_open' );
...
<div class="wrap kidia-data-page">
	<header class="kidia-data-hero">
		<div><span class="dashicons dashicons-database"></span><div><h1><?php esc_html_e( 'Store Data', 'kidia-mobile-cms' ); ?></h1><p><?php esc_html_e( 'Live commerce data from your website and mobile app, organised for clear sales decisions.', 'kidia-mobile-cms' ); ?></p></div></div>
	...
	<nav class="kidia-data-tabs">
		<?php foreach ( $store_tabs as $key => $item ) : ?>
	...
	<section class="kidia-data-panel kidia-data-panel--<?php echo esc_attr( $store_tab ); ?>">
		<div class="kidia-data-panel__head">
			<div>
				...
				<?php elseif ( 'analytics' === $store_tab ) : ?>
					<span class="kidia-source-lock"><span class="dashicons dashicons-smartphone"></span><?php esc_html_e( 'Mobile App only', 'kidia-mobile-cms' ); ?></span>
				<?php endif; ?>
			</div>
		...
		<?php if ( 'products' === $store_tab ) : ?>
			<div class="kidia-data-table-wrap"><table class="kidia-data-table"><thead><tr><th><?php esc_html_e( 'Product', 'kidia-mobile-cms' ); ?></th><th><?php esc_html_e( 'SKU', 'kidia-mobile-cms' ); ?></th><th><?php esc_html_e( 'Stock', 'kidia-mobile-cms' ); ?></th><th><?php esc_html_e( 'Price', 'kidia-mobile-cms' ); ?></th><th><?php esc_html_e( 'Status', 'kidia-mobile-cms' ); ?></th><th><?php esc_html_e( 'Actions', 'kidia-mobile-cms' ); ?></th></tr></thead><tbody>
				<?php foreach ( $products as $product ) : $link = get_permalink( $product->get_id() ); ?><tr><td><div class="kidia-data-identity"><?php echo wp_kses_post( $product->get_image( 'thumbnail' ) ); ?><strong><?php echo esc_html( $product->get_name() ); ?></strong></div></td><td><code><?php echo esc_html( $product->get_sku() ?: '—' ); ?></code></td><td><?php echo esc_html( wc_get_stock_html( $product ) ? wp_strip_all_tags( wc_get_stock_html( $product ) ) : __( 'Not managed', 'kidia-mobile-cms' ) ); ?></td><td><?php echo wp_kses_post( $product->get_price_html() ); ?></td><td><span class="kidia-status"><?php echo esc_html( ucfirst( $product->get_status() ) ); ?></span></td><td><div class="kidia-row-actions"><a href="<?php echo esc_url( get_edit_post_link( $product->get_id() ) ); ?>"><?php esc_html_e( 'Edit', 'kidia-mobile-cms' ); ?></a><a href="<?php echo esc_url( $link ); ?>" target="_blank" rel="noopener"><?php esc_html_e( 'View', 'kidia-mobile-cms' ); ?></a><button type="button" data-copy-link="<?php echo esc_attr( $link ); ?>"><?php esc_html_e( 'Copy link', 'kidia-mobile-cms' ); ?></button></div></td></tr><?php endforeach; ?>
			</tbody></table></div>
		...
		<?php elseif ( 'categories' === $store_tab ) : ?>
			<div class="kidia-category-section"><h3><?php esc_html_e( 'Main categories', 'kidia-mobile-cms' ); ?></h3><p><?php esc_html_e( 'Top-level categories are kept separate from their subcategories.', 'kidia-mobile-cms' ); ?></p><div class="kidia-category-list">
				<?php foreach ( $parent_categories as $category ) : $category_link = get_term_link( $category ); ?><article><span class="dashicons dashicons-category"></span><div><strong><?php echo esc_html( $category->name ); ?></strong><small><?php echo esc_html( sprintf( _n( '%d product', '%d products', $category->count, 'kidia-mobile-cms' ), $category->count ) ); ?></small></div><div class="kidia-row-actions"><a href="<?php echo esc_url( get_edit_term_link( $category->term_id, 'product_cat' ) ); ?>"><?php esc_html_e( 'Edit', 'kidia-mobile-cms' ); ?></a><?php if ( ! is_wp_error( $category_link ) ) : ?><button type="button" data-copy-link="<?php echo esc_attr( $category_link ); ?>"><?php esc_html_e( 'Copy link', 'kidia-mobile-cms' ); ?></button><?php endif; ?></div></article><?php endforeach; ?>
			</div></div>
			<div class="kidia-category-section is-subcategories"><h3><?php esc_html_e( 'Subcategories', 'kidia-mobile-cms' ); ?></h3>
				<?php foreach ( $parent_categories as $parent ) : $children = $subcategory_groups[ $parent->term_id ] ?? array(); if ( empty( $children ) ) { continue; } ?><section><h4><?php echo esc_html( $parent->name ); ?><b><?php echo esc_html( (string) count( $children ) ); ?></b></h4><div class="kidia-category-list"><?php foreach ( $children as $category ) : $category_link = get_term_link( $category ); ?><article><span class="dashicons dashicons-arrow-left-alt2"></span><div><strong><?php echo esc_html( $category->name ); ?></strong><small><?php echo esc_html( sprintf( _n( '%d product', '%d products', $category->count, 'kidia-mobile-cms' ), $category->count ) ); ?></small></div><div class="kidia-row-actions"><a href="<?php echo esc_url( get_edit_term_link( $category->term_id, 'product_cat' ) ); ?>"><?php esc_html_e( 'Edit', 'kidia-mobile-cms' ); ?></a><?php if ( ! is_wp_error( $category_link ) ) : ?><button type="button" data-copy-link="<?php echo esc_attr( $category_link ); ?>"><?php esc_html_e( 'Copy link', 'kidia-mobile-cms' ); ?></button><?php endif; ?></div></article><?php endforeach; ?></div></section><?php endforeach; ?>
			</div>
		...
		<?php elseif ( 'customers' === $store_tab ) : ?>
			...
			<div class="kidia-customer-list"><?php foreach ( $customers as $customer ) : $sources = Kidia_Mobile_Analytics::customer_sources( $customer->ID ); ?><article><?php echo get_avatar( $customer->ID, 48 ); ?><div><strong><?php echo esc_html( $customer->display_name ); ?></strong><small><?php echo esc_html( $customer->user_email ); ?></small><span class="kidia-source-badges"><?php if ( $sources['website'] ) : ?><b class="is-website"><span class="dashicons dashicons-desktop"></span><?php esc_html_e( 'Website', 'kidia-mobile-cms' ); ?></b><?php endif; ?><?php if ( $sources['mobile'] ) : ?><b class="is-mobile"><span class="dashicons dashicons-smartphone"></span><?php esc_html_e( 'Mobile', 'kidia-mobile-cms' ); ?></b><?php endif; ?></span></div><div><b><?php echo esc_html( function_exists( 'wc_get_customer_order_count' ) ? (string) wc_get_customer_order_count( $customer->ID ) : '0' ); ?></b><small><?php esc_html_e( 'Orders', 'kidia-mobile-cms' ); ?></small></div><div><b><?php echo wp_kses_post( function_exists( 'wc_get_customer_total_spent' ) ? $money( (float) wc_get_customer_total_spent( $customer->ID ) ) : '—' ); ?></b><small><?php esc_html_e( 'Total spent', 'kidia-mobile-cms' ); ?></small></div><a class="button" href="<?php echo esc_url( get_edit_user_link( $customer->ID ) ); ?>"><?php esc_html_e( 'Profile', 'kidia-mobile-cms' ); ?></a></article><?php endforeach; ?></div>
		...
		<?php elseif ( 'analytics' === $store_tab ) : ?>
			...
			<div class="kidia-data-summary is-four kidia-analytics-kpis">
				<div><small><?php esc_html_e( 'App visitors', 'kidia-mobile-cms' ); ?></small><b><?php echo esc_html( (string) $analytics['visitors'] ); ?></b><span class="is-<?php echo esc_attr( $visitor_change[1] ); ?>"><?php echo esc_html( $visitor_change[0] . '%' ); ?> <?php esc_html_e( 'vs previous period', 'kidia-mobile-cms' ); ?></span></div>
				...
				<div><small><?php esc_html_e( 'Mobile purchases', 'kidia-mobile-cms' ); ?></small><b><?php echo esc_html( (string) $analytics_purchase['count'] ); ?></b><span class="is-<?php echo esc_attr( $purchase_change[1] ); ?>"><?php echo esc_html( $purchase_change[0] . '%' ); ?> <?php esc_html_e( 'vs previous period', 'kidia-mobile-cms' ); ?></span></div>
				<div><small><?php esc_html_e( 'Tracked mobile revenue', 'kidia-mobile-cms' ); ?></small><b><?php echo wp_kses_post( $money( (float) $analytics_purchase['value'] ) ); ?></b><span class="is-<?php echo esc_attr( $revenue_change[1] ); ?>"><?php echo esc_html( $revenue_change[0] . '%' ); ?> <?php esc_html_e( 'vs previous period', 'kidia-mobile-cms' ); ?></span></div>
			</div>
			...
			<div class="kidia-analytics-grid">
				<section class="kidia-funnel"><header><h3><?php esc_html_e( 'Mobile sales funnel', 'kidia-mobile-cms' ); ?></h3><p><?php esc_html_e( 'Find the step where customers leave before buying.', 'kidia-mobile-cms' ); ?></p></header><?php
				$funnel = array(
					array( __( 'Opened app', 'kidia-mobile-cms' ), $analytics_opens['unique'], 100 ),
					array( __( 'Viewed product', 'kidia-mobile-cms' ), $analytics_views['unique'], $rate( $analytics_views['unique'], $analytics_opens['unique'] ) ),
					array( __( 'Added to cart', 'kidia-mobile-cms' ), $analytics_cart['unique'], $rate( $analytics_cart['unique'], $analytics_opens['unique'] ) ),
					array( __( 'Started checkout', 'kidia-mobile-cms' ), $analytics_checkout['unique'], $rate( $analytics_checkout['unique'], $analytics_opens['unique'] ) ),
					array( __( 'Purchased', 'kidia-mobile-cms' ), $analytics_purchase['unique'], $rate( $analytics_purchase['unique'], $analytics_opens['unique'] ) ),
				);
				...
				<section><header><h3><?php esc_html_e( 'Registration funnel', 'kidia-mobile-cms' ); ?></h3><p><?php esc_html_e( 'Measure how many people finish creating an account.', 'kidia-mobile-cms' ); ?></p></header><div class="kidia-decision-card"><strong><?php echo esc_html( $rate( $analytics_signups['count'], $analytics_registration_starts['count'] ) . '%' ); ?></strong><span><?php esc_html_e( 'registration completion', 'kidia-mobile-cms' ); ?></span><small><?php echo esc_html( sprintf( __( '%1$d completed from %2$d starts', 'kidia-mobile-cms' ), $analytics_signups['count'], $analytics_registration_starts['count'] ) ); ?></small></div><div class="kidia-decision-card"><strong><?php echo esc_html( $rate( $analytics_purchase['unique'], $analytics_opens['unique'] ) . '%' ); ?></strong><span><?php esc_html_e( 'visitor-to-purchase conversion', 'kidia-mobile-cms' ); ?></span></div></section>
				<section><header><h3><?php esc_html_e( 'Top viewed products', 'kidia-mobile-cms' ); ?></h3><p><?php esc_html_e( 'Products getting attention in the app.', 'kidia-mobile-cms' ); ?></p></header><div class="kidia-ranked-list"><?php $rank = 0; foreach ( $analytics['top_products'] as $row ) : ++$rank; ?><div><b><?php echo esc_html( (string) $rank ); ?></b><span><strong><?php echo esc_html( $row['event_label'] ); ?></strong><small><?php echo esc_html( sprintf( __( '%d unique viewers', 'kidia-mobile-cms' ), $row['unique_clients'] ) ); ?></small></span><em><?php echo esc_html( (string) $row['event_count'] ); ?></em></div><?php endforeach; ?></div></section>
				...
				<section><header><h3><?php esc_html_e( 'Category demand', 'kidia-mobile-cms' ); ?></h3><p><?php esc_html_e( 'Categories attracting the most mobile attention.', 'kidia-mobile-cms' ); ?></p></header><div class="kidia-ranked-list"><?php $rank = 0; foreach ( $analytics['top_categories'] as $row ) : ++$rank; ?><div><b><?php echo esc_html( (string) $rank ); ?></b><span><strong><?php echo esc_html( $row['event_label'] ); ?></strong><small><?php echo esc_html( sprintf( __( '%d unique viewers', 'kidia-mobile-cms' ), $row['unique_clients'] ) ); ?></small></span><em><?php echo esc_html( (string) $row['event_count'] ); ?></em></div><?php endforeach; ?></div></section>
			...
			<div class="kidia-opportunities"><h3><?php esc_html_e( 'Sales opportunities', 'kidia-mobile-cms' ); ?></h3><div>
				...
				<?php if ( $analytics_checkout['unique'] > $analytics_purchase['unique'] ) : ?><article><span class="dashicons dashicons-cart"></span><div><strong><?php esc_html_e( 'Recover checkout drop-off', 'kidia-mobile-cms' ); ?></strong><p><?php echo esc_html( sprintf( __( '%d mobile shoppers started checkout without a tracked purchase. Check delivery cost, payment methods and abandoned carts.', 'kidia-mobile-cms' ), $analytics_checkout['unique'] - $analytics_purchase['unique'] ) ); ?></p></div></article><?php endif; ?>
				...
				<?php if ( 0 === $analytics_opens['count'] ) : ?><article><span class="dashicons dashicons-info-outline"></span><div><strong><?php esc_html_e( 'Tracking is ready', 'kidia-mobile-cms' ); ?></strong><p><?php esc_html_e( 'Metrics will appear after customers use the updated mobile app. No placeholder values are shown.', 'kidia-mobile-cms' ); ?></p></div></article><?php endif; ?>
			</div></div>
		...
		<?php else : ?>
			<div class="kidia-settings-list"><?php foreach ( array( __( 'General store settings', 'kidia-mobile-cms' ) => 'general', __( 'Payments', 'kidia-mobile-cms' ) => 'checkout', __( 'Shipping', 'kidia-mobile-cms' ) => 'shipping', __( 'Tax', 'kidia-mobile-cms' ) => 'tax', __( 'Emails', 'kidia-mobile-cms' ) => 'email' ) as $label => $settings_tab ) : ?><a href="<?php echo esc_url( add_query_arg( 'tab', $settings_tab, $manage_urls['settings'] ) ); ?>"><span class="dashicons dashicons-admin-generic"></span><strong><?php echo esc_html( $label ); ?></strong><span class="dashicons dashicons-arrow-left-alt2"></span></a><?php endforeach; ?></div>
		<?php endif; ?>
	</section>
</div>
