	...
	<section class="kidia-data-panel kidia-data-panel--<?php echo esc_attr( $store_tab ); ?>">
		...
		<?php if ( 'products' === $store_tab ) : ?>
			<form class="kidia-product-search" method="get" data-kidia-instant-filter="products">
				...
				<button class="button button-primary" type="submit" data-kidia-instant-submit-fallback><?php esc_html_e( 'Search', 'kidia-mobile-cms' ); ?></button>
		...
		<?php elseif ( 'discounts' === $store_tab ) : ?>
			<form class="kidia-coupon-filters" method="get" data-kidia-instant-filter="coupons">
				...
				<button class="button button-primary" type="submit" data-kidia-instant-submit-fallback><?php esc_html_e( 'Filter', 'kidia-mobile-cms' ); ?></button>
			...
			<?php foreach ( $coupons as $coupon_post ) : $coupon = class_exists( 'WC_Coupon' ) ? new WC_Coupon( $coupon_post->ID ) : null; if ( ! $coupon ) { continue; } $expires = $coupon->get_date_expires(); $starts = get_post_datetime( $coupon_post ); $limit = $coupon->get_usage_limit(); $used = $coupon->get_usage_count(); $remaining = $limit ? max( 0, $limit - $used ) : null; $product_ids = $coupon->get_product_ids(); $excluded_ids = $coupon->get_excluded_product_ids(); $categories = $coupon->get_product_categories(); $excluded_categories = $coupon->get_excluded_product_categories(); $channel = Kidia_Mobile_Coupon_Channel::get( $coupon_post->ID ); $channel_labels = Kidia_Mobile_Coupon_Channel::labels(); ?>
				...
				<td><div class="kidia-row-actions kidia-coupon-actions"><a href="<?php echo esc_url( get_edit_post_link( $coupon_post->ID ) ); ?>"><?php esc_html_e( 'Edit', 'kidia-mobile-cms' ); ?></a><button type="button" data-copy-text="<?php echo esc_attr( $coupon->get_code() ); ?>"><?php esc_html_e( 'Copy code', 'kidia-mobile-cms' ); ?></button><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-kidia-instant-action="coupon-channel"><input type="hidden" name="action" value="kidia_mobile_set_coupon_channel"><input type="hidden" name="coupon_id" value="<?php echo esc_attr( (string) $coupon_post->ID ); ?>"><?php wp_nonce_field( 'kidia_mobile_set_coupon_channel_' . $coupon_post->ID, 'kidia_mobile_coupon_channel_nonce' ); ?><select name="coupon_channel" data-kidia-saved-value="<?php echo esc_attr( $channel ); ?>" aria-label="<?php esc_attr_e( 'Coupon sales channel', 'kidia-mobile-cms' ); ?>"><?php foreach ( $channel_labels as $channel_key => $channel_label ) : ?><option value="<?php echo esc_attr( $channel_key ); ?>" <?php selected( $channel, $channel_key ); ?>><?php echo esc_html( $channel_label ); ?></option><?php endforeach; ?></select><button type="submit" data-kidia-instant-submit-fallback><?php esc_html_e( 'Save channel', 'kidia-mobile-cms' ); ?></button></form></div></td></tr>
		...
		<?php elseif ( 'reports' === $store_tab ) : ?>
			<?php if ( ! $store_reporting_ready ) : ?>
				<div class="kidia-reporting-ready"><span class="dashicons dashicons-chart-bar"></span><div><h3><?php esc_html_e( 'Choose a period, then generate the reports', 'kidia-mobile-cms' ); ?></h3><p><?php esc_html_e( 'Generate prepares the indexed WooCommerce reporting source once. After that, periods and channels open immediately and new orders, refunds and status changes update automatically.', 'kidia-mobile-cms' ); ?></p></div></div>
			<?php else : ?>
			<div data-kidia-live-store-data="reports" aria-live="polite">
			<div class="kidia-data-summary is-four"><div><small><?php esc_html_e( 'Orders', 'kidia-mobile-cms' ); ?></small><b><?php echo esc_html( (string) absint( $reporting_snapshot['all_orders'] ?? 0 ) ); ?></b></div><div><small><?php esc_html_e( 'Net sales', 'kidia-mobile-cms' ); ?></small><b><?php echo wp_kses_post( $money( $order_revenue ) ); ?></b></div><div><small><?php esc_html_e( 'Average order value', 'kidia-mobile-cms' ); ?></small><b><?php echo wp_kses_post( $money( $order_average ) ); ?></b></div><div><small><?php esc_html_e( 'Items sold', 'kidia-mobile-cms' ); ?></small><b><?php echo esc_html( (string) $order_units ); ?></b></div></div>
			<div class="kidia-report-grid">
				<section><header><h3><?php esc_html_e( 'Order status', 'kidia-mobile-cms' ); ?></h3><p><?php esc_html_e( 'See where fulfilment or cancellations affect sales.', 'kidia-mobile-cms' ); ?></p></header><div class="kidia-metric-list"><?php $report_order_count = absint( $reporting_snapshot['all_orders'] ?? 0 ); foreach ( $order_statuses as $status => $status_count ) : ?><div><span><?php echo esc_html( wc_get_order_status_name( $status ) ); ?></span><b><?php echo esc_html( (string) $status_count ); ?></b><i style="--kidia-progress:<?php echo esc_attr( $report_order_count ? (string) round( 100 * $status_count / $report_order_count ) : '0' ); ?>%"></i></div><?php endforeach; ?></div></section>
		...
		<?php elseif ( 'analytics' === $store_tab ) : ?>
			<?php if ( ! $store_reporting_ready ) : ?>
				<div class="kidia-reporting-ready"><span class="dashicons dashicons-chart-line"></span><div><h3><?php esc_html_e( 'Choose a period, then generate analytics', 'kidia-mobile-cms' ); ?></h3><p><?php esc_html_e( 'Nothing is calculated while you choose the dates. Generate once, then every period and source reads the live indexed data immediately.', 'kidia-mobile-cms' ); ?></p></div></div>
			<?php else : ?>
			...
			<div class="kidia-data-summary is-four kidia-analytics-kpis">
				...
				<div><small><?php esc_html_e( 'Orders', 'kidia-mobile-cms' ); ?></small><b><?php echo esc_html( (string) $analytics_purchase['count'] ); ?></b><span class="is-<?php echo esc_attr( $purchase_change[1] ); ?>"><?php echo esc_html( $purchase_change[0] . '%' ); ?> <?php esc_html_e( 'vs previous period', 'kidia-mobile-cms' ); ?></span></div>
				<div><small><?php esc_html_e( 'Net sales', 'kidia-mobile-cms' ); ?></small><b><?php echo wp_kses_post( $money( (float) $analytics_purchase['value'] ) ); ?></b><span class="is-<?php echo esc_attr( $revenue_change[1] ); ?>"><?php echo esc_html( $revenue_change[0] . '%' ); ?> <?php esc_html_e( 'vs previous period', 'kidia-mobile-cms' ); ?></span></div>
			</div>
			<div class="kidia-audience-strip"><div><small><?php esc_html_e( 'New visitors', 'kidia-mobile-cms' ); ?></small><b><?php echo esc_html( (string) $analytics['new_users'] ); ?></b></div><div><small><?php esc_html_e( 'Returning visitors', 'kidia-mobile-cms' ); ?></small><b><?php echo esc_html( (string) $analytics['returning_users'] ); ?></b></div><div><small><?php esc_html_e( 'Logins', 'kidia-mobile-cms' ); ?></small><b><?php echo esc_html( (string) $event( 'login' )['count'] ); ?></b></div><div><small><?php esc_html_e( 'Searches', 'kidia-mobile-cms' ); ?></small><b><?php echo esc_html( (string) $event( 'search' )['count'] ); ?></b></div><div><small><?php esc_html_e( 'Average order value', 'kidia-mobile-cms' ); ?></small><b><?php echo wp_kses_post( $money( (float) $analytics_purchase['average_value'] ) ); ?></b></div></div>
			<div class="kidia-analytics-grid">
				...
				$funnel = array(
					array( 'mobile' === $store_source ? __( 'Opened app', 'kidia-mobile-cms' ) : ( 'website' === $store_source ? __( 'Visited store', 'kidia-mobile-cms' ) : __( 'Visitors / app opens', 'kidia-mobile-cms' ) ), $analytics_closed_funnel['visitors'], 100 ),
					array( __( 'Viewed product', 'kidia-mobile-cms' ), $analytics_closed_funnel['viewed_product'], $rate( $analytics_closed_funnel['viewed_product'], $analytics_closed_funnel['visitors'] ) ),
					array( __( 'Added to cart', 'kidia-mobile-cms' ), $analytics_closed_funnel['added_to_cart'], $rate( $analytics_closed_funnel['added_to_cart'], $analytics_closed_funnel['visitors'] ) ),
					array( __( 'Started checkout', 'kidia-mobile-cms' ), $analytics_closed_funnel['started_checkout'], $rate( $analytics_closed_funnel['started_checkout'], $analytics_closed_funnel['visitors'] ) ),
					array( __( 'Purchased', 'kidia-mobile-cms' ), $analytics_closed_funnel['purchased'], $rate( $analytics_closed_funnel['purchased'], $analytics_closed_funnel['visitors'] ) ),
				);
				...
				<section><header><h3><?php esc_html_e( 'Registration funnel', 'kidia-mobile-cms' ); ?></h3><p><?php esc_html_e( 'Measure how many people finish creating an account.', 'kidia-mobile-cms' ); ?></p></header><div class="kidia-decision-card"><strong><?php echo esc_html( $rate( $analytics_signups['unique'], $analytics_registration_starts['unique'] ) . '%' ); ?></strong><span><?php esc_html_e( 'registration completion', 'kidia-mobile-cms' ); ?></span><small><?php echo esc_html( sprintf( __( '%1$d completed from %2$d starts', 'kidia-mobile-cms' ), $analytics_signups['unique'], $analytics_registration_starts['unique'] ) ); ?></small></div><div class="kidia-decision-card"><strong><?php echo esc_html( $rate( $analytics_closed_funnel['purchased'], $analytics_closed_funnel['visitors'] ) . '%' ); ?></strong><span><?php esc_html_e( 'visitor-to-purchase conversion', 'kidia-mobile-cms' ); ?></span></div></section>
			...
			<div class="kidia-opportunities"><h3><?php esc_html_e( 'Sales opportunities', 'kidia-mobile-cms' ); ?></h3><div>
				<?php if ( $analytics_closed_funnel['viewed_product'] > 0 && $rate( $analytics_closed_funnel['added_to_cart'], $analytics_closed_funnel['viewed_product'] ) < 15 ) : ?><article><span class="dashicons dashicons-lightbulb"></span><div><strong><?php esc_html_e( 'Improve product-to-cart conversion', 'kidia-mobile-cms' ); ?></strong><p><?php echo esc_html( sprintf( __( 'Only %s of product viewers added an item. Review price, photos and the add-to-cart offer on highly viewed products.', 'kidia-mobile-cms' ), $rate( $analytics_closed_funnel['added_to_cart'], $analytics_closed_funnel['viewed_product'] ) . '%' ) ); ?></p></div></article><?php endif; ?>
				<?php if ( $analytics_closed_funnel['started_checkout'] > $analytics_closed_funnel['purchased'] ) : ?><article><span class="dashicons dashicons-cart"></span><div><strong><?php esc_html_e( 'Recover checkout drop-off', 'kidia-mobile-cms' ); ?></strong><p><?php echo esc_html( sprintf( __( '%d shoppers started checkout without a tracked purchase. Check delivery cost, payment methods and abandoned carts.', 'kidia-mobile-cms' ), $analytics_closed_funnel['started_checkout'] - $analytics_closed_funnel['purchased'] ) ); ?></p></div></article><?php endif; ?>
