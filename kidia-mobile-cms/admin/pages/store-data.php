	...
	<section class="kidia-data-panel kidia-data-panel--<?php echo esc_attr( $store_tab ); ?>">
		...
		<?php elseif ( 'discounts' === $store_tab ) : ?>
			...
			<?php foreach ( $coupons as $coupon_post ) : $coupon = class_exists( 'WC_Coupon' ) ? new WC_Coupon( $coupon_post->ID ) : null; if ( ! $coupon ) { continue; } $expires = $coupon->get_date_expires(); $starts = get_post_datetime( $coupon_post ); $limit = $coupon->get_usage_limit(); $used = $coupon->get_usage_count(); $remaining = $limit ? max( 0, $limit - $used ) : null; $product_ids = $coupon->get_product_ids(); $excluded_ids = $coupon->get_excluded_product_ids(); $categories = $coupon->get_product_categories(); $excluded_categories = $coupon->get_excluded_product_categories(); ?>
				<tr><td><code><?php echo esc_html( $coupon->get_code() ); ?></code><small class="kidia-coupon-badges"><b><?php echo esc_html( ucfirst( $coupon_post->post_status ) ); ?></b><?php if ( $coupon->get_individual_use() ) : ?><b><?php esc_html_e( 'Individual', 'kidia-mobile-cms' ); ?></b><?php endif; ?></small></td>
				...
				<td><strong><?php echo esc_html( $starts ? $starts->format_i18n( get_option( 'date_format' ) ) : '—' ); ?></strong><small><?php echo esc_html( $expires ? sprintf( __( 'Ends %s', 'kidia-mobile-cms' ), $expires->date_i18n( get_option( 'date_format' ) ) ) : __( 'No expiry', 'kidia-mobile-cms' ) ); ?></small></td>
				<td><div class="kidia-row-actions kidia-coupon-actions"><a href="<?php echo esc_url( get_edit_post_link( $coupon_post->ID ) ); ?>"><?php esc_html_e( 'Edit', 'kidia-mobile-cms' ); ?></a><button type="button" data-copy-text="<?php echo esc_attr( $coupon->get_code() ); ?>"><?php esc_html_e( 'Copy code', 'kidia-mobile-cms' ); ?></button></div></td></tr>
		...
		<?php elseif ( 'abandoned-carts' === $store_tab ) : ?>
			...
			<form class="kidia-recovery-builder" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<details open><summary><span class="dashicons dashicons-megaphone"></span><?php esc_html_e( 'Send recovery offer to selected carts', 'kidia-mobile-cms' ); ?></summary><div class="kidia-recovery-controls">
					...
					<label class="is-wide"><span><?php esc_html_e( 'Notification title', 'kidia-mobile-cms' ); ?></span><input type="text" name="recovery_title" maxlength="100" value="<?php esc_attr_e( 'Your cart is waiting', 'kidia-mobile-cms' ); ?>" required></label>
					<label class="is-wide"><span><?php esc_html_e( 'Message — use {coupon} to insert the personal code', 'kidia-mobile-cms' ); ?></span><textarea name="recovery_message" rows="2" maxlength="500" required><?php esc_html_e( 'Complete your order with code {coupon} before your personal offer expires.', 'kidia-mobile-cms' ); ?></textarea></label>
					...
					<label class="kidia-recovery-check"><input type="checkbox" name="recovery_restrict_products" value="1" checked><span><?php esc_html_e( 'Restrict each coupon to products in that customer’s cart', 'kidia-mobile-cms' ); ?></span></label>
					<button class="button button-primary" type="submit"><span class="dashicons dashicons-paper-plane"></span><?php esc_html_e( 'Create coupons & send', 'kidia-mobile-cms' ); ?></button>
				</div></details>
				<div class="kidia-data-table-wrap"><table class="kidia-data-table kidia-abandoned-table"><thead><tr><th><input type="checkbox" data-select-all-carts aria-label="<?php esc_attr_e( 'Select all carts', 'kidia-mobile-cms' ); ?>"></th><th><?php esc_html_e( 'Customer', 'kidia-mobile-cms' ); ?></th><th><?php esc_html_e( 'Items', 'kidia-mobile-cms' ); ?></th><th><?php esc_html_e( 'Value', 'kidia-mobile-cms' ); ?></th><th><?php esc_html_e( 'Source', 'kidia-mobile-cms' ); ?></th><th><?php esc_html_e( 'Last activity', 'kidia-mobile-cms' ); ?></th><th><?php esc_html_e( 'Status', 'kidia-mobile-cms' ); ?></th></tr></thead><tbody><?php foreach ( $abandoned_carts as $cart ) : ?><tr><td><input type="checkbox" name="cart_ids[]" value="<?php echo esc_attr( (string) $cart['id'] ); ?>" <?php disabled( empty( $cart['customer_email'] ) ); ?> aria-label="<?php esc_attr_e( 'Select cart', 'kidia-mobile-cms' ); ?>"></td><td><strong><?php echo esc_html( $cart['customer_name'] ?: __( 'Guest', 'kidia-mobile-cms' ) ); ?></strong><small><?php echo esc_html( $cart['customer_email'] ?: __( 'Email required for a personal coupon', 'kidia-mobile-cms' ) ); ?></small></td><td><strong><?php echo esc_html( (string) $cart['item_count'] ); ?></strong><small><?php echo esc_html( implode( ', ', array_slice( array_filter( array_column( $cart['items'], 'name' ) ), 0, 3 ) ) ); ?></small></td><td><?php echo wp_kses_post( $money( (float) $cart['cart_total'] ) ); ?></td><td><span class="kidia-source-badge is-<?php echo esc_attr( $cart['source'] ); ?>"><?php echo esc_html( $source_labels[ $cart['source'] ] ?? ucfirst( $cart['source'] ) ); ?></span></td><td><?php echo esc_html( get_date_from_gmt( $cart['last_activity_at'], get_option( 'date_format' ) . ' ' . get_option( 'time_format' ) ) ); ?><small><?php echo esc_html( human_time_diff( strtotime( $cart['last_activity_at'] . ' UTC' ), time() ) . ' ' . __( 'ago', 'kidia-mobile-cms' ) ); ?></small></td><td><span class="kidia-status is-<?php echo esc_attr( $cart['status'] ); ?>"><?php echo esc_html( ucfirst( $cart['status'] ) ); ?></span><?php if ( ! empty( $cart['order_id'] ) ) : ?><small>#<?php echo esc_html( (string) $cart['order_id'] ); ?></small><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div>
			</form>
