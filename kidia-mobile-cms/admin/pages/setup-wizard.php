...
<div class="wrap kidia-setup-wrap">
	<div class="kidia-setup-hero">
		<div>
			<span class="kidia-setup-eyebrow"><?php esc_html_e( 'Woomobi CMS', 'mobishop' ); ?></span>
		...
		<div class="kidia-setup-progress" aria-label="<?php esc_attr_e( 'Setup progress', 'mobishop' ); ?>">
	...
	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="kidia-setup-form">
		...
		<section class="kidia-setup-step is-active" data-step="1" data-step-kind="identity">
			...
			<div class="kidia-setup-identity-grid">
				...
				<div class="kidia-setup-logo-field">
					...
					<button type="button" class="button kidia-setup-choose-logo"><?php esc_html_e( 'Choose logo', 'mobishop' ); ?></button>
				</div>
			</div>
		</section>
			...
			<div class="kidia-page-selection-grid">
				<?php foreach ( $setup_pages as $page_key => $page_details ) : ?>
					...
					<label class="kidia-page-choice <?php echo $is_required ? 'is-required' : ''; ?>">
						...
						<span class="kidia-page-choice__status"><?php echo $is_required ? esc_html__( 'Required', 'mobishop' ) : esc_html__( 'Optional', 'mobishop' ); ?></span>
						<?php if ( ! $is_required ) : ?><span class="kidia-page-choice__switch" aria-hidden="true"></span><?php endif; ?>
					</label>
				<?php endforeach; ?>
			</div>
			<p class="kidia-page-selection-note"><span class="dashicons dashicons-info-outline"></span><?php esc_html_e( 'The complete theme will be installed only on the selected pages. Unselected pages stay hidden and keep their saved design.', 'mobishop' ); ?></p>
		</section>
				...
				<?php foreach ( $themes as $key => $theme ) : ?>
					...
					<label class="kidia-theme-card" data-theme-key="<?php echo esc_attr( $key ); ?>" data-theme-name="<?php echo esc_attr( (string) $theme['name'] ); ?>" data-theme-hero="<?php echo esc_url( $hero_url ); ?>" data-theme-copy="<?php echo esc_attr( wp_json_encode( array_values( $theme['sample_copy'] ) ) ); ?>" style="--theme-primary:<?php echo esc_attr( $theme['primary'] ); ?>;--theme-soft:<?php echo esc_attr( $theme['soft'] ); ?>;--theme-ink:<?php echo esc_attr( $theme['ink'] ); ?>;--theme-surface:<?php echo esc_attr( $theme['surface'] ); ?>">
						...
						<div class="kidia-theme-card-copy">
							...
							<button type="button" class="button kidia-theme-preview-button"><?php esc_html_e( 'Preview full theme', 'mobishop' ); ?></button>
						</div>
					</label>
				<?php endforeach; ?>
			</div>
		...
		<section class="kidia-setup-step" data-step="<?php echo esc_attr( (string) $setup_step ); ?>">
			...
			<div class="kidia-export-ready-card is-ready">
				...
				<div>
					...
					<p><?php esc_html_e( 'WooMobile will apply the selected theme, identity and pages, then open the visual page customizer.', 'mobishop' ); ?></p>
					<ul>
		...
		<div class="kidia-setup-actions">
			<button type="button" class="button kidia-setup-back" hidden><?php esc_html_e( 'Back', 'mobishop' ); ?></button>
			<div></div>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=kidia-mobile-cms' ) ); ?>" class="button kidia-setup-skip"><?php esc_html_e( 'Exit setup', 'mobishop' ); ?></a>
			<button type="button" class="button button-primary kidia-setup-next"><?php esc_html_e( 'Continue', 'mobishop' ); ?></button>
			<button type="submit" name="finish_setup" value="1" class="button button-primary kidia-setup-apply" hidden><span class="dashicons dashicons-yes-alt" aria-hidden="true"></span><?php esc_html_e( 'Finish', 'mobishop' ); ?></button>
		</div>
	</form>
	...
	<div class="kidia-theme-modal" hidden>
		...
		<div class="kidia-theme-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="kidia-theme-modal-title">
			<button type="button" class="kidia-theme-modal__close" data-theme-modal-close aria-label="<?php esc_attr_e( 'Close preview', 'mobishop' ); ?>"><span class="dashicons dashicons-no-alt"></span></button>
			<div class="kidia-theme-modal__heading"><span><?php esc_html_e( 'Complete theme preview', 'mobishop' ); ?></span><h2 id="kidia-theme-modal-title" data-theme-modal-name></h2><p><?php esc_html_e( 'Browse every real Flutter page generated from this theme’s own settings and store imagery.', 'mobishop' ); ?></p></div>
			<div class="kidia-theme-modal__workspace">
				<div class="kidia-theme-modal__controls">
					<nav class="kidia-theme-modal__pages" aria-label="<?php esc_attr_e( 'Theme preview pages', 'mobishop' ); ?>">
					...
					<button type="button" class="button button-primary kidia-theme-modal__select"><?php esc_html_e( 'Use this complete theme', 'mobishop' ); ?></button>
				</div>
