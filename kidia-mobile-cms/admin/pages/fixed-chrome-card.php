...
<section class="kidia-fixed-chrome-card kidia-page-card kidia-page-card--locked" data-chrome-part="<?php echo esc_attr( $chrome_part ); ?>">
	<div class="kidia-page-card__header">
		...
		<label class="kidia-page-master-toggle"><input type="hidden" name="layout[<?php echo esc_attr( $chrome_part ); ?>][enabled]" value="0"><input type="checkbox" name="layout[<?php echo esc_attr( $chrome_part ); ?>][enabled]" value="1" <?php checked( ! empty( $chrome_component['enabled'] ) ); ?>><span><?php esc_html_e( 'Show', 'kidia-mobile-cms' ); ?></span></label>
	...
	<div class="kidia-page-card__body" hidden><div class="kidia-page-fields">
		<?php foreach ( $chrome_fields as $field ) :
			...
			<?php if ( 'checkbox' === $field['type'] ) : ?><label class="kidia-page-toggle"><input type="hidden" name="<?php echo esc_attr( $name ); ?>" value="0"><input type="checkbox" name="<?php echo esc_attr( $name ); ?>" value="1" <?php checked( ! empty( $value ) ); ?>><span></span><b><?php echo esc_html( ! empty( $value ) ? __( 'Visible', 'kidia-mobile-cms' ) : __( 'Hidden', 'kidia-mobile-cms' ) ); ?></b></label>
