...
<section class="kidia-fixed-chrome-card kidia-page-card kidia-page-card--locked" data-element="<?php echo esc_attr( $chrome_part ); ?>" data-chrome-part="<?php echo esc_attr( $chrome_part ); ?>">
	...
	<div class="kidia-page-card__body" hidden>
		...
		<div class="kidia-chrome-settings">
			<section class="kidia-chrome-setting kidia-chrome-icons-panel"><h3><?php esc_html_e( '1. Icon Shapes', 'kidia-mobile-cms' ); ?></h3><p><?php esc_html_e( 'Only items placed in the rows above appear here.', 'kidia-mobile-cms' ); ?></p><div class="kidia-page-fields">
			<?php foreach ( $footer_icon_symbols as $item => $symbols ) : $variant_key = $item . '_icon_variant'; $variant_field = null; foreach ( $chrome_fields as $candidate ) { if ( $candidate['key'] === $variant_key ) { $variant_field = $candidate; } } if ( ! $variant_field ) { continue; } $selected_variant = (string) ( $chrome_settings[ $variant_key ] ?? $variant_field['default'] ); ?>
				<div class="kidia-chrome-icon-choice" data-item-section="<?php echo esc_attr( $item ); ?>" hidden><strong><?php echo esc_html( $chrome_items[ $item ] ?? ucfirst( $item ) ); ?></strong><div class="kidia-chrome-icon-options" role="radiogroup" aria-label="<?php echo esc_attr( $chrome_items[ $item ] ?? ucfirst( $item ) ); ?>"><?php foreach ( $variant_field['options'] as $option => $option_label ) : ?><button type="button" class="kidia-chrome-icon-option <?php echo $selected_variant === (string) $option ? 'is-selected' : ''; ?>" data-icon-value="<?php echo esc_attr( $option ); ?>" title="<?php echo esc_attr( $option_label ); ?>" aria-pressed="<?php echo $selected_variant === (string) $option ? 'true' : 'false'; ?>"><span class="dashicons <?php echo esc_attr( $symbols[ $option ] ?? 'dashicons-marker' ); ?>"></span></button><?php endforeach; ?></div><select class="kidia-chrome-icon-select screen-reader-text" name="<?php echo esc_attr( $chrome_prefix . '[settings][' . $variant_key . ']' ); ?>"><?php foreach ( $variant_field['options'] as $option => $option_label ) : ?><option value="<?php echo esc_attr( $option ); ?>" <?php selected( $selected_variant, (string) $option ); ?>><?php echo esc_html( $option_label ); ?></option><?php endforeach; ?></select></div>
			<?php endforeach; ?>
			</div></section>
		<section class="kidia-chrome-setting"><h3><?php esc_html_e( '2. Icon Size & Text', 'kidia-mobile-cms' ); ?></h3><?php foreach ( $chrome_items as $item => $label ) : ?><div class="kidia-page-fields" data-item-section="<?php echo esc_attr( $item ); ?>" hidden><?php foreach ( $chrome_fields as $field ) { $key = $field['key']; $mapped = $item_field( $chrome_part, $key ); $is_size_text = false !== strpos( $key, 'size' ) || false !== strpos( $key, 'label' ) || in_array( $key, array( 'title', 'subtitle', 'search_placeholder', 'show_voice_search' ), true ); if ( $mapped === $item && $is_size_text && false === strpos( $key, 'variant' ) ) { $render_chrome_field( $field, $chrome_settings[ $key ] ?? $field['default'], $chrome_prefix . '[settings][' . $key . ']' ); } } ?></div><?php endforeach; ?><div class="kidia-page-fields"><?php foreach ( $chrome_fields as $field ) { if ( in_array( $field['key'], array( 'icon_size', 'label_size', 'icon_label_gap', 'show_labels' ), true ) ) { $render_chrome_field( $field, $chrome_settings[ $field['key'] ] ?? $field['default'], $chrome_prefix . '[settings][' . $field['key'] . ']' ); } } ?></div></section>
		<section class="kidia-chrome-setting kidia-chrome-setting--general <?php echo 'footer' === $chrome_part ? 'kidia-chrome-footer-general' : ''; ?>"><h3><?php esc_html_e( '3. Colors & Appearance', 'kidia-mobile-cms' ); ?></h3><div class="kidia-page-fields"><?php foreach ( $chrome_fields as $field ) { $key = $field['key']; $mapped = $item_field( $chrome_part, $key ); $already = 'layout_json' === $key || false !== strpos( $key, 'variant' ) || false !== strpos( $key, 'size' ) || false !== strpos( $key, 'label' ) || in_array( $key, array( 'title', 'subtitle', 'search_placeholder', 'show_voice_search' ), true ); if ( ! $already && ( 'general' === $mapped || 0 !== strpos( $key, 'show_' ) ) ) { ?><div <?php if ( 'general' !== $mapped ) : ?>data-item-section="<?php echo esc_attr( $mapped ); ?>" hidden<?php endif; ?>><?php $render_chrome_field( $field, $chrome_settings[ $key ] ?? $field['default'], $chrome_prefix . '[settings][' . $key . ']' ); ?></div><?php } } ?></div></section>
		</div>
	</div>
</section>
