(function () {
	...
	function renderBlock(block) {
		...
		case "hero_slider":
			...
			return '<div class="kidia-preview-block kidia-preview-hero" style="aspect-ratio:' + ratio + '">' + (image ? '<img src="' + image + '" alt="">' : '<div class="kidia-preview-placeholder">' + name + "</div>") + ((item.title || item.subtitle) ? '<div class="kidia-preview-hero__copy"><strong>' + escapeHtml(item.title || "") + "</strong><small>" + escapeHtml(item.subtitle || "") + "</small></div>" : "") + (items.length > 1 ? '<div class="kidia-preview-dots">' + items.map(function (_, index) { return "<i" + (index === 0 ? ' class="is-active"' : "") + "></i>"; }).join("") + "</div>" : "") + "</div>";
		...
		case "text_block":
			...
			return '<div class="kidia-preview-text" style="background:' + background + ";color:" + textColor + ";text-align:" + (settings.alignment === "left" || settings.alignment === "center" ? settings.alignment : "right") + '"><strong>' + escapeHtml(settings.title || name) + "</strong><p>" + escapeHtml(settings.content || "") + "</p></div>";
