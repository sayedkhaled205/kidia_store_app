(() => {
  (function() {
    ...
    function serializeBlocks() {
      return getBlocks().map(function(block, index) {
        const value = function(selector) {
          var _a2;
          return ((_a2 = block.querySelector(selector)) == null ? void 0 : _a2.value) || "";
        };
        ...
        block.querySelectorAll('[name*="[settings]"]').forEach(function(input) {
          ...
          if (!match) return;
          const path = Array.from(match[1].matchAll(/\[([^\]]*)\]/g)).map(function(part) {
            return part[1];
          });
          let cursor = settings;
        ...
        return {
          ...
          enabled: (enabled == null ? void 0 : enabled.checked) ? "1" : "",
    ...
    function renderPreview() {
      ...
      previewContent.innerHTML = blocks.map(function(block) {
        ...
        if (block.type === "hero_slider") {
          ...
          const image = safeImage((_a2 = items[0]) == null ? void 0 : _a2.image_url);
    ...
    function openCreateModal(type, label) {
      ...
      window.setTimeout(function() {
        createNameInput == null ? void 0 : createNameInput.focus();
      }, 40);
    }
    ...
    function removeEmptyState() {
      var _a2;
      (_a2 = document.getElementById("kidia-builder-empty")) == null ? void 0 : _a2.remove();
    }
    ...
    function findBlockByReference(type, libraryId) {
      return getBlocks().find(function(block) {
        return block.dataset.type === type && block.dataset.libraryId === libraryId;
      }) || null;
    }
    function appendTemplate(templateId, type, libraryId, name, isNew) {
      ...
      const html = template.innerHTML.replaceAll("__INDEX__", String(index)).replaceAll("987654321", String(index)).replaceAll("__ORDER__", String(index + 1)).replaceAll("__BLOCK_ID__", blockId).replaceAll("__LIBRARY_ID__", resolvedLibraryId).replaceAll("__BLOCK_NAME__", "");
      removeEmptyState();
    ...
    function duplicateBlock(block) {
      ...
      const type = ((_a2 = clone.querySelector(".kidia-block-type")) == null ? void 0 : _a2.value) || "block";
    ...
    (_a = document.getElementById("kidia-add-element")) == null ? void 0 : _a.addEventListener("click", openPicker);
    (_b = document.getElementById("kidia-collapse-all")) == null ? void 0 : _b.addEventListener("click", function() {
      collapseAll(true);
    });
    (_c = document.getElementById("kidia-expand-all")) == null ? void 0 : _c.addEventListener("click", function() {
      collapseAll(false);
    });
    (_d = document.getElementById("kidia-create-element-submit")) == null ? void 0 : _d.addEventListener(
      "click",
      createElement
    );
    createNameInput == null ? void 0 : createNameInput.addEventListener("keydown", function(event) {
      if (event.key === "Enter") {
    ...
    builder.addEventListener("click", function(event) {
      ...
      if (target.closest(".kidia-delete-block")) {
        const name = ((_b2 = (_a2 = block.querySelector(".kidia-block-name")) == null ? void 0 : _a2.textContent) == null ? void 0 : _b2.trim()) || labels.untitled || "Untitled Element";
        const message = labels.deleteConfirm || "Remove this element from the Home page?";
      ...
      if (target.closest(".kidia-select-banner-image, .kidia-select-media, .kidia-select-app-header-logo")) {
        if (!((_c2 = window.wp) == null ? void 0 : _c2.media)) return;
        const frame = window.wp.media({ title: "Choose image", button: { text: "Use image" }, multiple: false });
        frame.on("select", function() {
          var _a3;
          const attachment = (_a3 = frame.state().get("selection").first()) == null ? void 0 : _a3.toJSON();
          if (!(attachment == null ? void 0 : attachment.url)) return;
          const field = target.closest(".kidia-builder-field");
          const input = field == null ? void 0 : field.querySelector(".kidia-banner-image-url, .kidia-media-url, .kidia-app-header-logo-url");
          const preview = field == null ? void 0 : field.querySelector(".kidia-banner-image-preview, .kidia-media-preview");
      ...
      if (target.closest(".kidia-hero-gallery__select")) {
        if (!((_d2 = window.wp) == null ? void 0 : _d2.media)) return;
        ...
        frame.on("select", function() {
          ...
          frame.state().get("selection").each(function(model) {
            ...
            Object.keys(fields).forEach(function(fieldName) {
              ...
              input.type = "hidden";
              input.dataset.heroField = fieldName;
              input.name = "blocks[" + blockIndex + "][settings][items][" + itemIndex + "][" + fieldName + "]";
              input.value = fields[fieldName];
              item.appendChild(input);
            });
            const remove = document.createElement("button");
            remove.type = "button";
            remove.className = "button-link-delete kidia-hero-gallery__remove";
      ...
      if (target.closest(".kidia-hero-gallery__remove")) {
        (_e = target.closest(".kidia-hero-gallery__item")) == null ? void 0 : _e.remove();
        updateIndexes();
        markDirty();
        renderPreview();
        return;
    ...
    builder.addEventListener("change", function(event) {
      ...
      if (target.classList.contains("kidia-block-status-select")) {
        const block = target.closest(".kidia-builder-block");
        const hidden = block == null ? void 0 : block.querySelector(".kidia-block-status");
        const badge = block == null ? void 0 : block.querySelector(".kidia-builder-status");
    ...
    builder.addEventListener("input", function(event) {
      ...
      if (target.classList.contains("kidia-block-name-input")) {
        const name = (_a2 = target.closest(".kidia-builder-block")) == null ? void 0 : _a2.querySelector(".kidia-block-name");
        if (name) {
    ...
    builder.addEventListener("pointerdown", function(event) {
      ...
      const block = handle == null ? void 0 : handle.closest(".kidia-builder-block");
      if (block) {
    ...
    searchInput == null ? void 0 : searchInput.addEventListener("input", function() {
