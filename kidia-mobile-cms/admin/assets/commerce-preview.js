(function () {
	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		function checkoutFields() {
			...
			if (pageEnabled && !pageEnabled.checked) return '<section class="kidia-commerce-fields is-disabled"><strong>Checkout fields are disabled</strong></section>';
			...
			form.querySelectorAll("[data-checkout-field]").forEach(function (row) {
				...
				var control = type && type.value === "textarea"
					? '<div class="kidia-commerce-preview-input is-textarea">' + escapeHtml(placeholder && placeholder.value ? placeholder.value : labelText) + '</div>'
					: type && type.value === "checkbox"
						? '<div class="kidia-commerce-preview-check"><i></i>' + escapeHtml(labelText + mark) + '</div>'
						: '<div class="kidia-commerce-preview-input">' + escapeHtml(labelText + mark) + '</div>';
				groups[groupKey].push(control);
			});
			var labels = { billing: "Billing details", shipping: "Shipping details", order: "Order details" };
			return Object.keys(groups).map(function (group) {
				return groups[group].length ? '<section class="kidia-commerce-fields"><strong>' + labels[group] + '</strong>' + groups[group].join("") + '</section>' : "";
			}).join("");
		}
		function draw() {
			...
			var checkoutContent = '<section class="kidia-commerce-context"><strong>Order summary</strong><span>2 items · Delivery and payment</span></section>' + checkoutFields() + '<button class="kidia-commerce-checkout" type="button">Place order</button>';
			var relatedContent = '<section class="kidia-commerce-context"><div class="kidia-commerce-product-photo"><span class="dashicons dashicons-format-image"></span></div><strong>Kidia product</strong><span>EGP 499</span></section>' + (enabled ? '<section class="kidia-app-section kidia-commerce-section"><h3>' + escapeHtml(value("title", "You may also like")) + '</h3><div class="kidia-app-product-grid">' + cards + '</div></section>' : '');
			preview.innerHTML = '<div class="kidia-app-status">9:41 <span>● ◒ ▰</span></div><header class="kidia-app-header"><span class="dashicons dashicons-arrow-left-alt2"></span><strong>' + (checkout ? 'Checkout' : 'Product details') + '</strong><span class="dashicons dashicons-cart"></span></header><main class="kidia-commerce-screen' + (checkout ? ' is-checkout-' + design : '') + '">' + (checkout ? checkoutContent : relatedContent) + '</main>';
