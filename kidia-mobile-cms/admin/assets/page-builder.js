(function ($) {
	...
	function previewElement(card) {
		...
		if (id === "product_summary") {
			return '<section class="kidia-app-summary">' + (checked(card, "show_badge", false) ? '<span class="kidia-app-sale">خصم</span>' : "") + '<div style="display:flex;justify-content:space-between;align-items:center">' + (checked(card, "show_price", true) ? '<div class="kidia-app-price" style="color:' + color(card, "price_color", "#2F806E") + '"><b>' + money(first) + '</b>' + (checked(card, "show_regular_price", true) ? '<del>520 ج.م</del>' : "") + '</div>' : "") + (checked(card, "show_rating", true) ? '<div class="kidia-app-stars">★★★★★ ' + (checked(card, "show_review_count", true) ? '<u>252</u>' : '') + '</div>' : "") + '</div>' + (checked(card, "show_name", true) ? '<h2 style="margin-top:' + (checked(card, "show_price", true) ? number(card, "name_price_gap", 20) : 0) + 'px;color:' + color(card, "name_color", "#1D1D1D") + '">' + escapeHtml(first.name || "Toddler Girl Dress") + '</h2>' : "") + (checked(card, "show_sku", false) ? '<small>SKU: KIDIA-001</small>' : "") + (checked(card, "show_stock", false) ? '<small class="kidia-app-stock">متوفر</small>' : "") + '</section>';
		}
		if (id === "variations") {
			return '<section class="kidia-app-variations" style="--variation-title-color:' + color(card, "title_color", "#1D1D1D") + ';--variation-option-color:' + color(card, "option_text_color", "#1F2933") + ';--variation-group-gap:' + number(card, "group_gap", 18) + 'px"><h3 style="color:var(--variation-title-color)">اللون <span>وردي</span></h3><div style="color:var(--variation-option-color)"><button class="is-selected">وردي</button><button>أصفر</button></div><div style="height:var(--variation-group-gap)"></div><h3 style="color:var(--variation-title-color)">المقاس</h3><div style="color:var(--variation-option-color)"><button>2Y</button><button class="is-selected">3Y</button><button>4Y</button></div></section>';
		}
		if (id === "purchase_bar") {
