(function ($) {
	...
	function quickAdd(card) { var variant=value(card,"quick_add_icon_variant","bag"),style=value(card,"quick_add_icon_style","outline"),name=variant==="cart"?"cart":variant==="basket"?"basket":"bag",size=Math.max(16,Math.min(36,number(card,"quick_add_icon_size",22))),shell=Math.max(28,Math.min(64,number(card,"quick_add_background_size",40))),background=checked(card,"quick_add_show_background",true)?color(card,"quick_add_background_color","#FFFFFF"):"transparent";return '<span class="kidia-app-quick-add is-'+escapeHtml(style)+'" style="width:'+shell+'px;height:'+shell+'px;border-radius:'+number(card,"quick_add_radius",24)+'px;background:'+background+';color:'+color(card,"quick_add_icon_color","#1F2933")+';--quick-add-icon-size:'+size+'px">'+icon(name)+(variant==="bag"?'<b>+</b>':'')+'</span>'; }
	function productWishlist(card) { var variant=value(card,"product_wishlist_icon_variant","heart"),size=Math.max(16,Math.min(36,number(card,"product_wishlist_icon_size",20))),shell=Math.max(28,Math.min(64,number(card,"product_wishlist_background_size",40))),background=checked(card,"product_wishlist_show_background",true)?color(card,"product_wishlist_background_color","#FFFFFF"):"transparent";return '<span class="kidia-app-product-wishlist is-'+escapeHtml(value(card,"product_wishlist_icon_style","outline"))+'" style="width:'+shell+'px;height:'+shell+'px;border-radius:'+number(card,"product_wishlist_radius",24)+'px;background:'+background+';color:'+color(card,"product_wishlist_icon_color","#1F2933")+';--wishlist-icon-size:'+size+'px">'+icon(variant==="bookmark"?"bookmark":"heart")+'</span>'; }
