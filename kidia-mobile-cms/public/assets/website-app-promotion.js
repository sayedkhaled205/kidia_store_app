(() => {
  ...
  const initializePromotion = () => {
    ...
    if (
      ...
      root.dataset.kidiaAppPromoStatus = "filtered";
      return true;
    }
    ...
    const icon = () => {
      ...
      if (settings.logo_url) {
        ...
        holder.append(create("span", "kidia-app-promo__phone-icon"));
      }
      return holder;
    };
      ...
      window.setTimeout(() => {
        ...
        if (campaign === "smart_banner") {
          document.documentElement.classList.remove(
            "kidia-app-promo-has-bottom-banner",
          );
        }
    ...
    const qrPlaceholder = () => {
      const placeholder = create(
        "span",
        "kidia-app-promo__qr-placeholder",
      );
    ...
    const build = (displayType, campaign) => {
      ...
      const node =
        displayType === "floating_button"
          ? create(
              "button",
              `kidia-app-promo kidia-app-promo--${displayType}`,
            )
          : create(
              "section",
              `kidia-app-promo kidia-app-promo--${displayType}`,
    ...
    const show = (displayType, campaign, slot = null) => {
      ...
      if (["popup", "bottom_sheet"].includes(displayType)) {
        ...
        node
          .querySelector(".kidia-app-promo__dismiss")
    ...
    const showInline = () => {
      ...
      if (!slots.length && isTestMode) {
        const slot = create("div", "kidia-app-promo-slot");
        slot.dataset.kidiaAppPromoSlot = "inline";
        const host =
    ...
    if (isTestMode) {
      document.documentElement.classList.add("kidia-app-promo-test-mode");
    ...
    root.dataset.kidiaAppPromoTest = testCampaign;
    return true;
  };
