...
final class Kidia_Mobile_Recovery_Campaigns {

	private const DB_VERSION = '1';
	private const DB_OPTION  = 'kidia_mobile_recovery_db_version';

	public function register(): void {
		add_action( 'init', array( $this, 'maybe_install' ), 4 );
		add_action( 'admin_post_kidia_mobile_create_recovery_campaign', array( $this, 'create_campaign' ) );
		add_action( 'kidia_mobile_dispatch_recovery_push', array( $this, 'dispatch_scheduled' ), 10, 2 );
	...
	public function create_campaign(): void {
		...
		$carts       = Kidia_Mobile_Analytics::carts_by_ids( $ids );
		...
		foreach ( $carts as $cart ) {
			...
			$code = 'KIDIA-' . absint( $cart['id'] ) . '-' . strtoupper( wp_generate_password( 6, false, false ) );
			...
			if ( $scheduled ) {
				wp_schedule_single_event( $scheduled, 'kidia_mobile_dispatch_recovery_push', array( $campaign_id, $payload ) );
			} else {
	...
	public function attribute_order( $order_or_id ): void {
		...
		foreach ( $rows as $row ) {
			...
			$order->update_meta_data( '_kidia_recovery_campaign_id', absint( $row['id'] ) );
			$order->save();
		}
	}
	...
	private static function table(): string {
		global $wpdb;
		return $wpdb->prefix . 'kidia_mobile_recovery_campaigns';
	}
}
